...
impl Window {
    ...
    /// Capture one paint-only base per view for Linux GPU texture retention.
    ///
    /// Existing layer scopes and unsupported primitives retain direct painting.
    /// Bounds must conservatively enclose the closure's clipped output. Input
    /// handlers and element trees must remain outside this paint-only closure.
    #[cfg(target_os = "linux")]
    pub fn paint_retained_layer(&mut self, bounds: Bounds<Pixels>, paint: impl FnOnce(&mut Self)) {
        use std::sync::atomic::{AtomicU64, Ordering};
        static REVISION: AtomicU64 = AtomicU64::new(1);
        self.invalidator.debug_assert_paint();
        let clipped = bounds.intersect(&self.content_mask().bounds);
        if clipped.is_empty() || self.next_frame.scene.inside_paint_layer() {
            paint(self);
            return;
        }
        let bounds = self.cover_bounds(clipped);
        let content_mask = self.snapped_content_mask();
        let outer = std::mem::take(&mut self.next_frame.scene);
        paint(self);
        let mut inner = std::mem::replace(&mut self.next_frame.scene, outer);
        if !inner.supports_retained_capture() {
            self.next_frame.scene.replay(0..inner.len(), &inner);
            return;
        }
        inner.finish();
        let owner = self.current_view();
        // The renderer caches one texture per owner; a second layer would
        // evict the first every frame and silently paint direct.
        debug_assert!(
            !self
                .next_frame
                .scene
                .retained_layers
                .iter()
                .any(|layer| layer.owner == owner),
            "a view may paint at most one retained layer per frame"
        );
        let layer = crate::RetainedLayer {
            owner,
            revision: REVISION.fetch_add(1, Ordering::Relaxed),
            bounds,
            content_mask,
            order: 0,
            scene: std::sync::Arc::new(inner),
        };
        self.next_frame
            .scene
            .insert_primitive(crate::Primitive::RetainedLayer(layer));
    }
