...
struct WgpuResources {
    ...
    #[cfg(target_os = "linux")]
    retained: retained_gpu::LayerCache,
    #[cfg(target_os = "linux")]
    retained_revisions: retained_gpu::RevisionHistory,
    #[cfg(target_os = "linux")]
    gpu_timings: Option<crate::gpu_timing::GpuTimings>,
}
...
impl WgpuRenderer {
    ...
    fn new_internal(
        ...
        #[cfg(target_os = "linux")]
        if std::env::var_os("GPUI_RETAINED_LAYER_PROBE").is_some() {
            log::info!(
                "retained layer target: format={surface_format:?} alpha={alpha_mode:?} max_dimension={max_texture_size} supported_alpha={:?}",
        ...
        #[cfg(target_os = "linux")]
        let gpu_timings = crate::gpu_timing::GpuTimings::new(&device, &queue);
        #[cfg(target_os = "linux")]
        if let Some(timings) = &gpu_timings {
            log::info!(
                "dev GPU timings renderer index={} width={} height={}",
                timings.index(),
        ...
        let resources = WgpuResources {
            ...
            #[cfg(target_os = "linux")]
            retained_revisions: Default::default(),
            #[cfg(target_os = "linux")]
            gpu_timings,
        };
    ...
    pub fn draw(&mut self, scene: &Scene) -> bool {
        ...
        #[cfg(target_os = "linux")]
        if let Some(timings) = &self.resources().gpu_timings {
            timings.collect(&self.resources().device);
        }
        ...
        #[cfg(target_os = "linux")]
        if self.prepare_retained_layers(scene, globals) {
            // Capture submissions use their own globals before this main write.
        ...
        loop {
            ...
            let timing = self
                ...
                .gpu_timings
                .as_ref()
                .and_then(|timings| timings.begin(crate::gpu_timing::Kind::Main));
            {
                let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                    ...
                    timestamp_writes: timing
                        .as_ref()
                        .and_then(|timing| timing.writes(true, scene.paths.is_empty())),
                ...
                for batch in scene.batches() {
                    let ok = match batch {
                        ...
                        PrimitiveBatch::Paths(range) => {
                            #[cfg(target_os = "linux")]
                            let last_paths = range.end == scene.paths.len();
                            ...
                            pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                                ...
                                timestamp_writes: timing
                                    .as_ref()
                                    .and_then(|timing| timing.writes(false, last_paths)),
                                color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                                    view: &frame_view,
                                    resolve_target: None,
                                    ops: wgpu::Operations {
                        ...
                        #[cfg(target_os = "linux")]
                        PrimitiveBatch::RetainedLayers(range) => {
                            scene.retained_layers[range].iter().all(|layer| {
                                self.draw_retained_layer(layer, &mut instance_offset, &mut pass)
                            })
                        }
