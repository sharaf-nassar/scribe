...
impl CosmicTextSystemState {
    ...
    fn load_family(
        ...
        for (font_id, postscript_name) in families {
            ...
            // @lat: [[rendering#GPUI Cell-Accurate Paint Path#Font Fallbacks#Color Emoji Face Admission]]
            // Emoji faces need not contain Latin glyphs to be valid fallbacks.
...
fn check_is_known_emoji_font(postscript_name: &str) -> bool {
    // PostScript names, not family names: fontdb reports nameID 6.
    matches!(
        postscript_name,
        "NotoColorEmoji" | "TwemojiMozilla" | "AppleColorEmoji" | "SegoeUIEmoji"
    )
}
