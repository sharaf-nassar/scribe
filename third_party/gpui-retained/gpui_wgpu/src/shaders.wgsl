...
fn fs_quad(input: QuadVarying) -> @location(0) vec4<f32> {
    ...
    let background_color = gradient_color(quad.background, input.position.xy + globals.viewport_origin, quad.bounds,
...
fn fs_path_rasterization(input: PathRasterizationVarying) -> @location(0) vec4<f32> {
    ...
    let color = gradient_color(background, input.position.xy + globals.viewport_origin, bounds,
...
fn fs_poly_sprite(input: PolySpriteVarying) -> @location(0) vec4<f32> {
    ...
    let distance = quad_sdf(input.position.xy + globals.viewport_origin, sprite.bounds, sprite.corner_radii);
...
// --- retained premultiplied layers --- //

struct RetainedLayerParams {
    bounds: Bounds,
    content_mask: Bounds,
}
@group(1) @binding(0) var<storage, read> b_retained_layers: array<RetainedLayerParams>;
...
fn vs_retained_layer(@builtin(vertex_index) vertex_id: u32, @builtin(instance_index) instance_id: u32) -> RetainedLayerVarying {
    ...
    out.position = to_device_position(unit_vertex, layer.bounds);
    out.texture_position = unit_vertex;
    out.clip_distances = distance_from_clip_rect(unit_vertex, layer.bounds, layer.content_mask);
    return out;
}

@fragment
fn fs_retained_layer(input: RetainedLayerVarying) -> @location(0) vec4<f32> {
    let color = textureSample(t_sprite, s_sprite, input.texture_position);
    if (any(input.clip_distances < vec4<f32>(0.0))) {
        return vec4<f32>(0.0);
    }
    // Capture already applied opacity, text gamma, and premultiplication.
    return color;
}
