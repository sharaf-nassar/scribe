...
| A11Y-05 | Status chrome | The status bar, pane status strip, and prompt/status chrome are anonymous text/div trees. Its update CTA has only a pointer click handler, and connection/error/update state has no concise accessible status channel. | Assistive technology misses important terminal-window state and cannot invoke the update CTA. | P2 | `scribe-38e.112` |
