...
## Client messages (56 variants, 50 reachable)
...
| `MoveWorkspace` | existing-window workspace move transaction | unit | `main.rs::TerminalView::execute_palette_action` `MoveWorkspaceToWindow` arm and `TerminalView::finish_workspace_drag` cross-window release → `TerminalView::begin_workspace_move` → `ipc_bridge.rs::IpcSink::move_workspace` | required |
...
**Reachability:** 50 of 56 rows name a live-path symbol; 6 are unwired and 0
...
## Server messages (76 variants, 69 reachable)
...
| `AgentPromptDismiss` | withdrawal of a consent prompt that can no longer be answered | unit | `main.rs::dispatch_server_message` → `main.rs::on_agent_prompt_dismiss` → `main.rs::AgentPromptSlot::dismiss` → `TerminalView::poll_agent_prompt` → `TerminalView::close_withdrawn_agent_prompt` closes the modal without a decision | required |
...
| `WorkspaceMoveResult` | existing-window workspace move outcome | unit | `main.rs::on_workspace_move_result` → `workspace_transfer.rs::WorkspaceMoves::receive_result` → `TerminalView::apply_workspace_move_feedback` | required |
...
**Reachability:** 69 of 76 rows name a live-path symbol; 7 are unwired and 0
...
| URL, path, and OSC 8 hyperlink handling | `US1-3` | Ctrl+click open, OSC 8 status-bar preview, scheme gate | visual-E2E | `main.rs::TerminalView::press_opens_link` and `TerminalView::route_osc8_activation` → `url_detect::{open_url_observed,open_path_observed,open_uri_unguarded_observed}` / `url_detect::is_allowed_osc8_target`, with rejected schemes raised as `dialog::DisallowedSchemeDialog`; `TerminalView::refresh_osc8_hover` → `status_bar::build_model`; Ctrl+Shift+U → `AnnotationDemo` → `TerminalElement::with_annotation` — `tests/e2e/visual/terminal-links.sh`, `tests/e2e/visual/overlays.sh` | required |
...
| Overlay chrome polish | `US3-5` | rounded corners, drop shadow, hover/pressed | visual-E2E | `command_palette::CommandPaletteView::render`, `dialog::DialogView::render`, and `context_menu::ContextMenuView::render` apply `.rounded_*()` + `.shadow_*()` on the live overlay layer; Ctrl+Shift+U → `AnnotationDemo` → `TerminalElement::with_annotation` supplies the in-grid annotation demo in place of the retired link box — `tests/e2e/visual/overlays.sh`, `tests/e2e/visual/dialogs.sh` | required |
...
behaviour), the user-facing parity surface is **222 rows, of which 209 are
reachable (94%)** and 13 are not. **1 of those 222** rows — `HookEvent`, whose
named symbol is `scribe-hook-helper`'s `main` — is out-of-client by design, so
the in-client figure is **208 of 222**.
