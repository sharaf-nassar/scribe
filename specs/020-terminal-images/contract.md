...
| Classic placement | Source crop `x/y/w/h`, cell span `c/r`, pixel offset `X/Y`, cursor policy `C=0/1`, signed 32-bit `z`; clip to viewport and margins | Relative-parent `P/Q`, interaction/tooling extensions, undefined-overlap parity |
| Placeholder placement | `U=1`, `U+10EEEE`, official row/column/MSB diacritics, 8/24/32-bit image identity, underline placement id, background alpha color, specified left-cell inheritance | Horizontal-scroll inference beyond the official rules or treating rendered placeholder pixels as classic placements |
| Delete | Lowercase soft and uppercase data-freeing forms of `a/A`, `i/I`, `p/P`, `x/X`, `y/Y`, `z/Z`; any delete aborts an incomplete transfer | `c/C`, `n/N`, `q/Q`, `r/R` selectors |
