...
- `crates/scribe-client/src/gpui_image_lifecycle.rs` — bounded per-view source identities, translated-bounds/content-mask crop, projected GPU accounting, deterministic eviction, and explicit atlas cleanup.
- `crates/scribe-client/src/terminal_element.rs` — consume the chosen source crop helper for placeholder resolution, clipping, z-order phases, and GPUI image paint.
...
`ImagePlacement` contains protocol kind, image/placement identity, terminal anchor, source crop, destination cell geometry, pixel offsets, z-index, scroll/margin association, cursor-movement behavior, optional persistent logical cell clip, and placeholder metadata. Unicode placeholder cells stay in terminal content; placement state maps their encoded color/diacritic identity to an image definition.
...
- Ten owned ASCII-hex fixtures cover the frozen Kitty/Sixel matrix without importing an external test suite; `tests/e2e/fixtures/terminal-images/contract.json` solely owns their paths and expected outcomes.
...
- **GPUI limitations:** revision `f96212f2c50f54d93712fa130d6226b1ce7d76b5` has no source-UV field or public live byte accounting. Mitigation: the completed spike uses one bounded source `RenderImage`, translates/scales its full bounds under `Window::with_content_mask`, charges texture plus staging independently, and needs neither cached crop variants nor a GPUI patch. See [`gpui-lifecycle-decision.md`](gpui-lifecycle-decision.md).
...
| Spike bounded Sixel and Kitty decode | P0 | Protocol contract | Historical feasibility work established the fork and budget decisions in `decoder-decision.md`. Its executable was retired after the production Kitty and Sixel adversarial gates superseded it. |
| Production GPUI crop, upload, and eviction | P0 | Protocol contract; macOS path approval | `tests/e2e/visual/terminal-image-renderer.sh` uses translated bounds plus a content mask over the production bounded source cache; proves upload reuse, `drop_image`, recovery reupload, dimension caps, and unattended lifecycle on Linux, while [`gpui-lifecycle-decision.md`](gpui-lifecycle-decision.md) defines native Metal assertions; `just e2e-visual terminal-image-renderer.sh` passes. |
...
| Build authoritative server image state engine | P1 | Parsers; Sixel decoder; Kitty normalization; common model | Add `SessionTerminal`, canonical RGBA definitions/placements, xterm Sixel grid effects, deterministic quotas, ordered decode barrier, cancellation, and viewerless retention; `just e2e-func terminal-image-server-state.sh` proves split-transfer publication and cross-session eviction isolation converge through the assembled production path. |
...
| Render layered GPUI image placements | P1 | Client live scene; GPUI spike | Add per-view `RenderImage` cache, chosen crop path, classic/placeholder mapping, six paint phases, Sixel chronology, effective envelope clip/scroll/margin/resize/delete/DPI semantics, eviction, and final-reference `drop_image`; `just e2e-visual terminal-image-renderer.sh` writes stable captures. |
