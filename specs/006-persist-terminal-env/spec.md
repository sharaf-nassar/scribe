...
### Session 2026-08-02 — Post-startup ordering and AI launches

- **Plain zsh/fish FR-008 correction** → The scribe-ebz follow-up moved restore apply from their pre-rc bootstrap into a self-removing first-prompt initializer. It now runs after user rc/config, deletes the staged file, emits the post-restore baseline, and runs before recurring delta capture. Bash, nushell, and PowerShell retain their post-startup integration-tail consumers. There is no remaining zsh/fish pre-rc defect.
- **Structured AI launch amendment** → AI bash, zsh, and fish tabs apply a staged restore delta after login startup and before `exec`. Bash's server preamble sources the AI-mode integration script, which consumes the file; zsh/fish consume it directly in the server-built preamble after their pre-rc AI guard has declined resident integration. Each path deletes the file exactly once. Nushell, PowerShell, and unknown AI launches have no supported consumer and receive no staged file. Because AI shells exec before a prompt, prompt OSC marks, baseline emission, and per-prompt env-delta emission are intentionally absent from AI tabs.
