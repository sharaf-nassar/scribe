...
| A2-I7 | A wheel over a lane, tab, or open drawer is claimed by the board whether or not it travels, so it never reaches the pane behind the strip. It moves only the lane the pointer is over, vertically only (A2-R1 keeps A2 free of a horizontal axis), clamped to that queue's own rows at both ends. Scroll is pixel-smooth and per workspace and lane: edge fades mask the rows crossing the body's edges so no clipped row reads as a whole one (A2-S6), the `⌄` cue shows only while rows remain below, and the body builds a bounded window of rows rather than the whole queue. The offset is transient view state, reset when its queue's membership changes and cleared with its workspace; no window record restores it. | `scribe-2c6p` | Headless clamp/isolation and bounded-window tests plus a functional wheel run asserting lane travel with zero pane wheel reports at travel and at both clamps. |
