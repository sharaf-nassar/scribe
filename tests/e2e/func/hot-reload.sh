...
# --- Phase 3: Trigger hot-reload (new server with --upgrade) ---
...
echo "PHASE 3 PASS: server hot-reload completed"

# --- Phase 4: Reconnect daemon and reattach to the session ---
scribe-test daemon start
scribe-test session attach "$SAVED_SESSION"
echo "PHASE 4 PASS: reattached to session $SAVED_SESSION after upgrade"

# --- Phase 5: Verify the session is alive — send a new command ---
scribe-test send "$SAVED_SESSION" 'echo after-upgrade\n'
scribe-test wait-output "$SAVED_SESSION" "after-upgrade"
echo "PHASE 5 PASS: command executed after hot-reload"

# --- Phase 6: Verify the background process survived ---
scribe-test send "$SAVED_SESSION" 'jobs\n'
scribe-test wait-output "$SAVED_SESSION" "sleep 600"
echo "PHASE 6 PASS: background process survived hot-reload"

# --- Phase 7: Verify screen content was preserved (not blank) ---
...
if echo "$CELLS" | grep -qF "hot-reload-marker"; then
    echo "PHASE 7 PASS: screen content preserved after hot-reload"
else
    echo "PHASE 7 FAIL: screen content lost after hot-reload"
