...
#
# Validates that:
#   1. OSC 1337 AiState sequences are stripped from visible terminal output
#   2. All four AI states can be sent without corrupting the terminal
...
# ── Phase 1: OSC 1337 AiState=processing is stripped from output ─────────
...
scribe-test send "$SESSION" 'printf "\\033]1337;AiState=state=processing\\033\\\\"; echo ai-phase1-ok\n'
...
if echo "$SNAP1" | grep -q "AiState"; then
    echo "PHASE 1 FAIL: AiState sequence leaked into visible output"
    exit 1
fi
echo "PHASE 1 PASS: processing state sent, OSC stripped from output"

# ── Phase 2: All four states cycle correctly ─────────────────────────────
...
scribe-test send "$SESSION" 'printf "\\033]1337;AiState=state=processing\\033\\\\"; echo state-proc\n'
scribe-test wait-output "$SESSION" "state-proc"

scribe-test send "$SESSION" 'printf "\\033]1337;AiState=state=idle_prompt\\033\\\\"; echo state-idle\n'
scribe-test wait-output "$SESSION" "state-idle"

scribe-test send "$SESSION" 'printf "\\033]1337;AiState=state=permission_prompt\\033\\\\"; echo state-perm\n'
scribe-test wait-output "$SESSION" "state-perm"

scribe-test send "$SESSION" 'printf "\\033]1337;AiState=state=error\\033\\\\"; echo state-err\n'
scribe-test wait-output "$SESSION" "state-err"

echo "PHASE 2 PASS: all four AI states cycled without corruption"

# ── Phase 3: OSC with optional fields ────────────────────────────────────
# The parser accepts optional tool/agent/model/context fields.
scribe-test send "$SESSION" 'printf "\\033]1337;AiState=state=processing;tool=Bash;agent=main;model=claude;context=42\\033\\\\"; echo fields-ok\n'
scribe-test wait-output "$SESSION" "fields-ok"

# Verify no AiState payload leaked.
SNAP3=$(scribe-test snapshot "$SESSION" /dev/stdout 2>/dev/null)
if echo "$SNAP3" | grep -q "AiState"; then
    echo "PHASE 3 FAIL: AiState with fields leaked into output"
    exit 1
fi
...
scribe-test send "$SESSION" 'printf "BEFORE\\033]1337;AiState=state=processing\\033\\\\AFTER\\n"\n'
...
scribe-test send "$SESSION" 'for s in processing idle_prompt permission_prompt processing error; do printf "\\033]1337;AiState=state=$s\\033\\\\"; done; echo rapid-ok\n'
scribe-test wait-output "$SESSION" "rapid-ok"
echo "PHASE 5 PASS: rapid state transitions handled without deadlock"

# ── Phase 6: Session with AI state can be closed cleanly ─────────────────
# This is last because closing a session may affect daemon state.
EXTRA=$(scribe-test session create)
scribe-test send "$EXTRA" 'printf "\\033]1337;AiState=state=processing\\033\\\\"; echo extra-alive\n'
