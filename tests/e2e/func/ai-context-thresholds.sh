...
#
# Validates that the status bar and tab-inline % display correctly across
# the three context threshold bands:
#   Ok     (< 70)  — only status bar shows %, tab suppresses it
#   Warn   (>= 70) — both status bar and tab show %, count >= 2
#   Danger (>= 90) — both status bar and tab show %, count >= 2
# =============================================================================

# ── Phase 1 + 4: Ok band (50%) — status bar only ─────────────────────────────
# At 50% (below warn=70) the status bar renders "50%" but the tab-inline
# suppresses it, so "50%" should appear exactly once in the snapshot.
scribe-test send "$SESSION" 'printf "\\033]1337;ClaudeState=processing;context=50\\033\\\\"; echo ctx-phase1-ok\n'
...
if ! echo "$SNAP1" | grep -q "50%"; then
    echo "PHASE 1 FAIL: context=50 not rendered as 50% in status bar"
    exit 1
fi
echo "PHASE 1 PASS: context=50 (Ok band) rendered as 50% in status bar"

# Phase 4 assertion: tab suppression — 50% should appear at most once.
# (Tab-inline only activates at >= warn threshold; status bar always shows it.)
...
# ── Phase 2: Warn band (72%) — status bar + tab ───────────────────────────────
# At 72% (>= warn=70) both the status bar segment AND the tab inline label
# should render "72%", so it should appear at least twice.
scribe-test send "$SESSION" 'printf "\\033]1337;ClaudeState=processing;context=72\\033\\\\"; echo ctx-phase2-ok\n'
...
if [ "$COUNT2" -ge 2 ]; then
    echo "PHASE 2 PASS: 72% count=${COUNT2} >= 2 (status bar + tab both rendered)"
else
    echo "PHASE 2 FAIL: 72% count=${COUNT2} — expected >= 2 (status bar + tab inline)"
    exit 1
fi

# ── Phase 3: Danger band (91%) — status bar + tab ────────────────────────────
# At 91% (>= danger=90) both the status bar segment AND the tab inline label
# should render "91%", so it should appear at least twice.
scribe-test send "$SESSION" 'printf "\\033]1337;ClaudeState=processing;context=91\\033\\\\"; echo ctx-phase3-ok\n'
...
if [ "$COUNT3" -ge 2 ]; then
    echo "PHASE 3 PASS: 91% count=${COUNT3} >= 2 (status bar + tab both rendered)"
else
    echo "PHASE 3 FAIL: 91% count=${COUNT3} — expected >= 2 (status bar + tab inline)"
    exit 1
fi

# ── Phase 5: Codex Ok band (51%) — status bar only ────────────────────────────
# Codex provider-symmetric test: send CodexState instead of ClaudeState.
# At 51% (below warn=70) the status bar renders "51%" but the tab-inline
...
scribe-test send "$SESSION" 'printf "\\033]1337;CodexState=processing;context=51\\033\\\\"; echo ctx-phase5-ok\n'
...
if ! echo "$SNAP5" | grep -q "51%"; then
    echo "PHASE 5 FAIL: CodexState with context=51 not rendered as 51% in status bar"
    exit 1
fi
COUNT5=$(echo "$SNAP5" | grep -c "51%" || true)
if [ "$COUNT5" -le 1 ]; then
    echo "PHASE 5 PASS: CodexState context=51 (Ok band) rendered as 51% in status bar"
else
    echo "PHASE 5 FAIL: 51% count=${COUNT5} — expected <= 1 (tab should suppress below warn)"
    exit 1
fi

# ── Phase 6: Codex Warn band (73%) — status bar + tab ──────────────────────────
# Codex provider-symmetric test: at 73% (>= warn=70) both the status bar segment
...
scribe-test send "$SESSION" 'printf "\\033]1337;CodexState=processing;context=73\\033\\\\"; echo ctx-phase6-ok\n'
...
if [ "$COUNT6" -ge 2 ]; then
    echo "PHASE 6 PASS: CodexState 73% count=${COUNT6} >= 2 (status bar + tab both rendered)"
else
    echo "PHASE 6 FAIL: 73% count=${COUNT6} — expected >= 2 (status bar + tab inline)"
    exit 1
fi

# ── Phase 7: Codex Danger band (92%) — status bar + tab ─────────────────────────
# Codex provider-symmetric test: at 92% (>= danger=90) both the status bar segment
...
scribe-test send "$SESSION" 'printf "\\033]1337;CodexState=processing;context=92\\033\\\\"; echo ctx-phase7-ok\n'
...
if [ "$COUNT7" -ge 2 ]; then
    echo "PHASE 7 PASS: CodexState 92% count=${COUNT7} >= 2 (status bar + tab both rendered)"
else
    echo "PHASE 7 FAIL: 92% count=${COUNT7} — expected >= 2 (status bar + tab inline)"
    exit 1
fi
