...
mod tests {
    ...
    /// Read the alpha channel of pixel `(col, row)` from an RGBA canvas.
    fn alpha_at(rgba: &[u8], stride: u32, col: u32, row: u32) -> u8 {
        let idx = ((row * stride + col) * 4 + 3) as usize;
        rgba[idx]
    }

    /// `is_box_drawing` covers exactly U+2500–U+259F and nothing outside it.
    #[test]
    fn is_box_drawing_covers_the_procedural_range() {
    ...
    /// Zero-sized cells produce no canvas (caller falls back to the font).
    #[test]
    fn zero_dimensions_return_none() {
        assert!(render('\u{2500}', 0, 8).is_none());
        assert!(render('\u{2500}', 8, 0).is_none());
    }

    /// Characters outside the handled range return `None`.
    #[test]
    fn unhandled_char_returns_none() {
        assert!(render('A', 8, 16).is_none());
    }

    /// The full block (█, U+2588) fills every pixel with opaque white.
    #[test]
    fn full_block_fills_every_pixel_opaque() {
        let (w, h, rgba) = render('\u{2588}', 4, 6).expect("full block renders");
        assert_eq!((w, h), (4, 6));
        assert_eq!(rgba.len(), (4 * 6 * 4) as usize);
        assert!(rgba.chunks_exact(4).all(|px| px == [255, 255, 255, 255]));
    }

    /// The upper half block (▀, U+2580) fills the top `h/2` rows and leaves
    /// the rest transparent — byte-exact against the legacy rasterizer.
    #[test]
    fn upper_half_block_fills_top_half_only() {
        let cw = 4;
        let ch = 8;
        let (_, _, rgba) = render('\u{2580}', cw, ch).expect("upper half renders");
        for row in 0..ch {
            let expected = if row < ch / 2 { 255 } else { 0 };
            for col in 0..cw {
                assert_eq!(alpha_at(&rgba, cw, col, row), expected, "row {row} col {col}");
            }
        }
    }

    /// Shade characters fill the whole canvas with a fixed alpha: light 64,
    /// medium 128, dark 192, foreground white.
    #[test]
    fn shade_characters_use_fixed_alpha_levels() {
        for (c, alpha) in [('\u{2591}', 64u8), ('\u{2592}', 128), ('\u{2593}', 192)] {
            let (_, _, rgba) = render(c, 3, 5).expect("shade renders");
            assert!(
                rgba.chunks_exact(4).all(|px| px == [255, 255, 255, alpha]),
                "char {c:?} should be uniform alpha {alpha}"
            );
        }
    }

    /// A light horizontal line (─, U+2500) draws a thin band through the
    /// vertical centre and leaves the top and bottom rows clear.
    #[test]
    fn light_horizontal_line_draws_centered_band() {
        let cw = 16;
        let ch = 16;
        let (_, _, rgba) = render('\u{2500}', cw, ch).expect("h-line renders");
        // Top and bottom rows are empty.
        assert!((0..cw).all(|col| alpha_at(&rgba, cw, col, 0) == 0));
        assert!((0..cw).all(|col| alpha_at(&rgba, cw, col, ch - 1) == 0));
        // The centre row is filled edge to edge.
        let mid = ch / 2;
        assert!((0..cw).all(|col| alpha_at(&rgba, cw, col, mid) == 255));
    }

    /// A light vertical line (│, U+2502) draws a thin band through the
    /// horizontal centre and leaves the left and right columns clear.
    #[test]
    fn light_vertical_line_draws_centered_band() {
        let cw = 16;
        let ch = 16;
        let (_, _, rgba) = render('\u{2502}', cw, ch).expect("v-line renders");
        assert!((0..ch).all(|row| alpha_at(&rgba, cw, 0, row) == 0));
    ...
    /// three sibling quadrants stay transparent.
    #[test]
    fn lower_left_quadrant_fills_one_quadrant() {
        let cw = 8;
        let ch = 8;
        let (_, _, rgba) = render('\u{2596}', cw, ch).expect("quadrant renders");
        // Lower-left corner pixel set, other three corners clear.
        assert_eq!(alpha_at(&rgba, cw, 0, ch - 1), 255);
        assert_eq!(alpha_at(&rgba, cw, 0, 0), 0);
        assert_eq!(alpha_at(&rgba, cw, cw - 1, 0), 0);
        assert_eq!(alpha_at(&rgba, cw, cw - 1, ch - 1), 0);
    }
}
