//! Driver process singleton enforcement.
//!
...
use scribe_common::socket::{driver_lock_path, driver_socket_path};

/// Result of attempting to become the singleton driver process.
...
pub fn acquire() -> Result<SingletonResult, String> {
    let lock_path = driver_lock_path();
    let socket_path = driver_socket_path();
...
/// Try to connect to an existing driver process and send focus command.
...
pub fn verify_peer_uid(stream: &UnixStream) -> bool {
    ...
    if peer_uid != expected {
        tracing::warn!(peer_uid, expected, "rejected driver connection from different UID");
        return false;
    }
    true
}
...
pub fn read_command(stream: &UnixStream) -> Option<String> {
    // Set a short read timeout to avoid blocking the event loop.
    ...
    if let Err(e) = std::fs::remove_file(socket_path) {
        if e.kind() != std::io::ErrorKind::NotFound {
            tracing::warn!(
                ...
                "failed to remove driver socket: {e}"
            );
        }
    }
}
