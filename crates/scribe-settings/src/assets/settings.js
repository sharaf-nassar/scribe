...
// Show the "Control handoff" row only in single-typist sharing mode, where it
// is the only mode the option governs (feature 015). Reads the active
// sharing-mode segment from the DOM since remote keys are not mirrored into
// currentConfig. Uses `remote-hidden`, mirroring the passive Tailscale notice.
function updateControlAcquisitionVisibility() {
  var row = document.getElementById("remote-control-acquisition-row");
  if (!row) { return; }
  var active = document.querySelector(
    ".segmented-control[data-key='remote.sharing_mode'] .segment-opt.active"
  );
  var mode = active ? active.getAttribute("data-value") : "single_controller";
  row.classList.toggle("remote-hidden", mode !== "shared_single_typist");
}
  ...
  document.querySelectorAll(".segmented-control").forEach(function(ctrl) {
    ...
    opts.forEach(function(opt) {
      opt.addEventListener("click", function() {
        ...
        if (key === "remote.sharing_mode") {
          updateControlAcquisitionVisibility();
        }
...
function loadConfig(config) {
  ...
  // Remote — Window sharing (feature 015)
  setSegmentedValue('remote.sharing_mode', config.remote?.sharing_mode ?? 'single_controller');
  setSegmentedValue('remote.control_acquisition', config.remote?.control_acquisition ?? 'free_claim');
  setStepperValue('remote.participant_limit', config.remote?.participant_limit ?? 0);
  updateControlAcquisitionVisibility();
