...
The `context` field (`Option<u8>`) carries the AI tool's context-window fill percentage (0–100). It is populated by the Claude statusLine producer (`scribe-claude-statusline.sh`) and the Codex context hook (`detect-codex-context.sh`) via context-only OSC 1337 payloads — `ClaudeContext=NN` and `CodexContext=NN` respectively — that the server applies as a partial patch on the live `AiProcessState`. Producers never assert state, only context. Codex reads the hook `transcript_path` and derives the percentage from that rollout's latest `last_token_usage.total_tokens` over `model_context_window`; cumulative `total_token_usage` is ignored because it is session-wide billing usage. Auggie has no producer for `context` and always leaves the field blank. The [[common#Configuration#AI Context Thresholds]] config controls the warn/danger band boundaries used by the prompt bar and tab-bar displays.

State transitions are owned exclusively by the per-provider hook scripts — Claude's `~/.claude/settings.json` hooks (`PreToolUse`, `Notification`, `PostToolUse`, `Stop` via `dist/detect-claude-question.sh`, `UserPromptSubmit`) emit `ClaudeState=<state>` with no payload (occasionally with `conversation_id`), `dist/codex-task-label.sh` and `dist/detect-codex-question.sh` emit `CodexState=<state>` (sometimes with `conversation_id` but never `context`), and `dist/auggie-state.sh` emits `AuggieState=<state>` (or `…;conversation_id=…`) with no `context`. Each such event would otherwise carry `None` for `context`, `model`, `tool`, `agent`, and `conversation_id`, clobbering values set by an earlier same-provider state event. [[crates/scribe-common/src/ai_state.rs#AiProcessState#merge_partial_from_previous]] carries those optional fields forward from the previously-stored state when the new event leaves them unset and the provider matches, so a state-only hook firing between events does not erase the live percentage. Switching providers (e.g. Claude → Codex) skips the merge so cross-provider state does not bleed. The dedicated context channel — `<Provider>Context=NN` parsed into `MetadataEvent::AiContextChanged` and applied by [[crates/scribe-server/src/ipc_server.rs#send_ai_context_change]] — is what keeps the producer/state separation honest: a status-line refresh patches only the percentage and re-broadcasts the existing state, never replacing it.
