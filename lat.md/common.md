...
[[crates/scribe-common/src/protocol.rs#AiLaunchSpec]] is the shared wire shape for AI session intent: provider, [[crates/scribe-common/src/protocol.rs#AiResumeMode|new/resume mode]], and optional conversation id. `CreateSession.ai_launch` is additive and defaults to `None`; when present it is authoritative for server-owned argv while the client dual-writes a legacy `command` for old local servers. Because the field is remote-visible, [[crates/scribe-common/src/protocol.rs#REMOTE_PROTOCOL_VERSION]] is `4` and mixed v3/v4 peers refuse at the exact-version gate. `AiResumeMode` is also the canonical persisted restore-state type, whose serde names stay exactly `New` and `Resume`.
...
`ai_tab_cwd` (: `pane` (default), `project_root`, or `home`) controls where a fresh Claude/Codex AI tab starts. `pane` uses the focused pane's server-reported CWD, `project_root` falls back through pane to home, and `home` sends `cwd: None` for the server's guarded home fallback. The client reads this value from its live-reloaded config for each create; cold-restart replay keeps its persisted cwd.
