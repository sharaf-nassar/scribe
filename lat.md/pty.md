...
The  wraps PTY master file descriptors for zero-copy async I/O using tokio's `AsyncFd`. No thread-per-session overhead; reads and writes are driven by epoll (Linux) or kqueue (macOS).
...
The  is a VTE Perform adapter that runs in parallel with alacritty_terminal's own VTE parser.
...
`ScribeAiLaunch=<provider_id>` is a shell-integration sentinel that pre-arms the  for an AI binary the shell is about to execute.

`<provider_id>` is one of `claude_code` or `codex_code` (matched by ). The shell's preexec hook emits this OSC immediately before invoking `claude` or `codex` so the PTY reader can set `ai_provider` to the correct value before the AI tool itself starts emitting bytes. Without this pre-arm, `<tool> --resume` would send its initial `\x1b[3J` before identifying via the hook channel, slipping through the filter and wiping scrollback. The parser produces an `AiProviderArmed` variant of , which is consumed entirely inside the PTY reader and is not forwarded to the client.

This is the **only** OSC 1337 AI-related payload parsed from the PTY stream. AI tool state, prompt text, task labels, and context-window fill all arrive via the structured hook channel; see .
...
The exit code now reaches the client, which classifies each command `Success`/`Failure`/`Unknown` and surfaces it as a scrollbar tick colour plus an authoritative non-colour status-bar glyph; see .
...
The theme comes from the cached config snapshot, not a fresh load and parse per query, so a 256-index OSC 4 probe costs one config read the first time the process answers a colour query and zero disk reads after that. `ConfigReloaded` drops the snapshot, so a theme edit is reflected by the next query without a server restart.
