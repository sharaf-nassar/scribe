...
The 016 reachability audit (`specs/016-gpui-client-rebuild/reachability-audit.md`) found the crate shipping far more implemented surface than `main.rs` could reach: most library modules were never imported, most `ServerMessage` variants fell into a `_ => {}` arm, and most `LayoutAction` variants fell into a `_ => tracing::debug!` arm. Both catch-alls are gone. Every `LayoutAction` now reaches a real handler, so the swallowed-action counter has been deleted along with its arm; inbound messages the client does not act on go through [[crates/scribe-client-gpui/src/main.rs#unhandled_server_message]], which names the variant, increments a process counter, and logs at `warn`, so an unimplemented surface is observable at runtime rather than silent. [[crates/scribe-client-gpui/src/main.rs#server_message_variant]] holds the exhaustive variant table that supplies those names, which makes adding a protocol variant a compile error until someone decides what the client does with it.
...
Widening the census surfaced five requirements with no live path. Pane dividers are now wired; server-upgrade reattach and the remote connect picker remain missing, while the AI indicator's painted half and the workspace-notes hover preview remain unwired. `ai_indicator` is imported by `main.rs`, so the module ratchet still scores it wired while its painting half has no call site. Module-level reachability is a floor, not a substitute for a per-requirement row.
