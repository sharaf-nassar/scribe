...
Content comes back too, not just layout: the server checkpoints its own state — the same `HandoffState` the hot handoff sends, per-session zstd replays included — to an AEAD-sealed dump on a dirty-gated 30-second interval and once more on SIGTERM/Ctrl+C, and a fresh server loads it and seeds each replayed launch's pre-crash scrollback into its new PTY session before the shell's first byte (see [[server#Server#Crash Recovery Dump]]). Recovery is keyed by the launch id the replay presents, so a killed window's content is never resurrected.
