...
#### AI tabs are plain tabs that exec

An AI tab is not its own session class. [[crates/scribe-server/src/session_manager.rs#build_launch_shell]] owns the whole decision: the ordinary [[crates/scribe-server/src/session_manager.rs#build_shell]] argv with [[crates/scribe-server/src/session_manager.rs#ai_exec_args]] appended, so the provider inherits exactly what a plain tab inherits.

That means the same [[crates/scribe-common/src/shell.rs#default_shell_program|default shell resolver]], the same integration attachment (bash `--rcfile`, zsh `ZDOTDIR`, fish `XDG_DATA_DIRS`), and the same startup files every other tab reads. Only an interactive `-c` command is added: the shell finishes its normal startup, then `exec`s the provider over itself. Two properties follow. The provider becomes the PTY's direct child, so quitting the AI CLI ends the session and closes the tab rather than dropping the user at a stray prompt. And an AI tab can never resolve a different `PATH` than the tab beside it — the earlier design launched a login shell, which on bash reads only the first of `.bash_profile` / `.bash_login` / `.profile` and therefore missed `~/.bashrc`, where most users keep `PATH`; a provider that was on `PATH` in every plain tab was then a command-not-found `exec` that killed the AI tab on sight.

Zsh and fish are the one exception carried in the appended command. Both schedule their restore-delta apply for the first `precmd` so it lands after user rc, and an AI tab execs before any prompt, so the `-c` command consumes and deletes the staged delta itself — still after rc, still before `exec`, satisfying spec-006 FR-008. Bash needs no equivalent because `scribe.bash` applies the delta inline while being sourced, which is already post-rc. Nushell takes `-i -c` because it rejects the grouped short form, and gets no integration under any `-c` variant (its vendor autoload is REPL-only). PowerShell speaks neither `-i` nor `exec`, so it runs the provider through `-NoLogo -Command` and drops the `-File` integration attachment, whose argument has to come last; pwsh exits when that command returns, so the tab still ends with the CLI even without an exec to replace the process. Prompt OSC marks, per-prompt env-delta emission, and baseline emission still install on an AI tab exactly as on a plain one — they simply never fire, because the shell execs before its first prompt. The server-injected absolute `SCRIBE_HOOK_HELPER` is independent of `PATH` ordering, so hook delivery does not depend on which flavor a profile puts first. If the requested AI binary is absent, the shell prints its normal command-not-found diagnostic and the tab exits; no separate client error protocol is added.

[[crates/scribe-server/src/session_manager.rs#ResolvedShell#for_request]] preserves explicit commands and otherwise uses the host's env-first [[crates/scribe-common/src/shell.rs#default_shell_program|default shell resolver]], while [[crates/scribe-server/src/session_manager.rs#build_shell]] keeps bash `--rcfile`, zsh `ZDOTDIR`, fish/nushell vendor autoload, and PowerShell `-File` integration. Feature 018 briefly made AI tabs a second session class with their own login-shell resolution and their own script gates; unifying them back onto this one path deleted that resolver, the `SCRIBE_AI_TAB` mode in all three integration scripts, and the per-shell pre-exec preambles. Resident shells keep the prompt and env-capture hooks an exec-away tab never reaches, so nothing was lost by sharing the path — the AI tab just stops running them.
...
Buffering holds every sink-bound frame, not just `PtyOutput`: `process_pty_chunk` also emits `TrimScrollback` and `ScrollBottom` on the same path, and replaying a scrollback trim against a snapshot that already reflects it would corrupt the client's history. Frames no snapshot can carry (metadata, `SessionExited`, workspace naming) are tagged with no cursor value at all and always flush. A buffer that outgrows the per-connection output budget while waiting sheds its backlog and marks the session replay-dirty instead, so the connection's writer task catches the client up with a fresh full `SessionReplay`.
...
An upgrade receiver still takes no singleton lock — the old server holds it until it exits. That is safe only because the path is never unowned: a normal-mode server launched into this window finds a live server on the far end and refuses to start, which is the guarantee the flock was mistakenly credited with.

Both platforms reach this path. The Debian `postinst` spawns the receiver directly, and on macOS [[crates/scribe-client/src/server_lifecycle.rs#restart_server]] prefers a direct `--upgrade` spawn, falling back to `launchctl kickstart` only when that spawn fails.
...
After a successful `ditto`, the updater starts the newly installed server with `--upgrade`, matching Linux's zero-downtime handoff.

The executable comes from the replacement app bundle through [[crates/scribe-server/src/updater/macos_restart.rs#server_executable]], never from the predecessor's `current_exe()`. A launchd `kickstart -k` is intentionally not used: it kills the old server and starts a normal successor without `--upgrade`, leaving no live process to transfer PTYs from.

The updater waits up to 30 seconds for the handoff to complete, avoiding false restart-required fallbacks when large snapshots take longer to transfer and restore. If the spawn fails or the handoff times out, it broadcasts `CompletedRestartRequired` and leaves the old server and clients alive until the user approves a cold restart from the UI.
...
AI tabs mark their pre-exec integration load with `SCRIBE_AI_TAB=1`. Bash keeps one-shot restore-delta sourcing and deletion but skips its startup-file emulation, prompt/PS1/PS0/DEBUG wiring, baseline emit, and delta-hook registration; it also removes `ENV` plus both AI integration markers before returning to the exec preamble. Zsh and fish load before user rc files, so their AI-mode guards leave the restore file untouched for the post-login preamble, install no prompt or persistence hooks, and remove the AI integration markers. Plain zsh and fish tabs instead register a self-removing first-prompt initializer: it applies and deletes the restore file after user rc/config, emits the baseline, then lets the already-registered recurring delta handler observe that baseline. Handler registration order plus a capture-ready guard prevents the first prompt from persisting rc-only exports as user-session deltas.
