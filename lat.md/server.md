...
Auggie hooks are configured in `~/.augment/settings.json` using documented `SessionStart`, `PreToolUse`, `PostToolUse`, `Stop`, and `SessionEnd` events. `setup-auggie-hooks.sh` accepts Augment's JSON5-style comments and trailing commas, writes back valid JSON, and preserves unrelated hook entries. The `Stop` hook opts into `includeConversationData` so Scribe can best-effort emit `AuggiePrompt` and `AuggieTaskLabel`; Augment does not currently document a prompt-submit hook, so those values arrive when Auggie stops responding rather than at prompt submission time.
