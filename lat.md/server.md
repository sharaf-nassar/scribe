...
`ipc_server.rs` remains the transport and message-dispatch layer for `AttachSessions`, but  owns the reattach sequence itself: attach-entry preparation from live sessions, pre-snapshot Term and PTY resize,  for the zstd-compressed ANSI replay, and the delayed client-writer install. Per-session metadata and per-workspace names travel on the preceding `SessionList` response, so the attach fan-out is just `SessionCreated` + `SessionReplay` per session.
...
The session runs one authoritative grid sized smallest-attached-wins (FR-012): each participant's `Resize` is stored as its viewport (ungated in shared modes), and  recomputes `min(rows)` × `min(cols)` over the attached viewports after a 250 ms debounce, driving the existing `resize_term` + `TIOCSWINSZ` path; because it is a pure function of the participant set it never flaps, and a participant detaching regrows the grid to the next-smallest.
