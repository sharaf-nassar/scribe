...
The engine holds no writer references, so it publishes withdrawn prompt ids on an unbounded stream that [[crates/scribe-server/src/ipc_server.rs#spawn_agent_prompt_dismiss_forwarder]] takes once at startup and broadcasts as `AgentPromptDismiss` to every capable local window — the same layering as the activity forwarder. A prompt is withdrawn on timeout, on policy refresh, and when its last waiter drops. Expiry and last-waiter cancellation leave a short-lived `prompt_id` tombstone: a decision that raced the withdrawal still applies `AlwaysAllow`/`AlwaysDeny` to its capability axis, while no waiter is signalled and no burst decision is cached. Once-only decisions on a tombstone are no-ops.

Display context stays out of the engine. [[crates/scribe-server/src/ipc_server.rs#AgentPromptDelivery]] resolves the origin session's title, cwd and task label and a human target description — target session title and cwd, or action name and window — when the prompt is actually sent, so `PromptKey` and burst parking are unchanged and a request policy never prompted for reads no session state.

`ConfigReloaded` projects the fresh `[agent_api]` table through [[crates/scribe-server/src/config.rs#project_config]] and [[crates/scribe-server/src/agent_api/mod.rs#AgentApiState#refresh_policy]]. Refresh cancels pending prompts, withdraws their dialogs, and clears existing tombstones so a late decision cannot overwrite the config just loaded; it takes effect on the next call without restarting the server. An all-`Deny` policy also releases held activity leases; it is the off state, not a separate master switch.
...
Those two guards are the server-wide lock order: live sessions before workspace manager, never the reverse, and neither held across an `.await`. `CloseWindow` reads the workspace first to resolve the window's session ids, but that read guard is a statement temporary and is gone before the registry guard is taken. Two walks over every live session honour the same rule by staging under the guard and awaiting after it: [[crates/scribe-server/src/ipc_server.rs#detach_sessions]] does its sync sink and pacer work under the read guard and returns the attachments to clear, which are awaited only once the guard drops; [[crates/scribe-server/src/ipc_server.rs#serialize_live_for_handoff]] copies each session's metadata and clones its `Term`, image-seam, and resize-fd handles via [[crates/scribe-server/src/ipc_server.rs#LiveSession#stage_for_handoff]], then locks each `Term` off-guard. Holding the registry across those awaits stalled every `Hello`, `CreateSession`, and close for the length of a disconnect or a whole handoff snapshot.
