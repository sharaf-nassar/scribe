...
[[crates/scribe-server/src/ipc_server.rs#ControllerIdentity]] (`Local`, or `Remote` with device and login names) is tracked per window in [[crates/scribe-server/src/ipc_server.rs#WindowControllers]], kept in lock-step with `connected_clients` so window-listing and the `WindowTakenOver` naming never drift. A claim resolves through [[crates/scribe-server/src/ipc_server.rs#resolve_and_register_claim]].

The claim mode is derived from `Hello.takeover` plus local/remote origin: a takeover swaps the connected window's writer; a local no-takeover claim falls through to today's assign-different-window path; a remote no-takeover claim of a still-connected window yields [[crates/scribe-server/src/ipc_server.rs#ClaimOutcome]]`::LostControl`. All three per-window maps — writer (`connected_clients`), controller (`window_controllers`), and the `clipboard_gating` capability bit — move together under one lock hold, so a near-simultaneous takeover burst resolves to exactly one controller and no stale clipboard or policy state survives the swap (FR-014). A takeover captures the displaced writer so [[crates/scribe-server/src/ipc_server.rs#handle_client_hello]] can send it `WindowTakenOver` after the lock is released; a `LostControl` outcome completes `Welcome` but leaves the current controller untouched and sends the reclaimant an immediate `WindowTakenOver` (never a silent seizure, FR-011). [[crates/scribe-server/src/ipc_server.rs#log_control_transition]] records each transition.

### Control Authorization

Takeover authorization is enforced server-side, not merely signalled: a connection displaced by a takeover is barred from mutating or re-attaching the window it lost (FR-007, FR-011).

The guard is [[crates/scribe-server/src/ipc_server.rs#connection_controls_window]] — the same `Arc::ptr_eq` writer-identity test as [[crates/scribe-server/src/ipc_server.rs#release_window_if_owned]] and [[crates/scribe-server/src/ipc_server.rs#detach_sessions]]: a connection controls a window only while it is still that window's registered writer, even though a displaced connection keeps its stale, never-revoked `attached_ids`. [[crates/scribe-server/src/ipc_server.rs#dispatch_message]] applies it via [[crates/scribe-server/src/ipc_server.rs#requires_window_control]] to every window-mutating or scrollback-reading frame (`KeyInput`, `Resize`, `CloseSession`, `CloseWindow`, `FocusChanged`, `SearchRequest`) — a displaced connection's frame becomes a no-op — and [[crates/scribe-server/src/ipc_server.rs#filter_attachable_sessions]] applies it to `AttachSessions` so a displaced peer cannot re-steal the `PtyOutput` stream and clipboard-bridge routing by simply re-attaching. A local Unix-socket client is always its own window's controller (a local no-takeover claim assigns a *different* window rather than displacing one), so the guard is a no-op locally; a `LostControl` reconnect never registered, so it fails the guard too, as intended. This closes the FR-007/FR-011 enforcement gap where takeover was previously notified but not enforced.
