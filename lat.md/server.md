...
Environment variables are set to TERM=xterm-256color, COLORTERM=truecolor, and TERM_PROGRAM=Scribe on top of the server process environment. On Linux, [[crates/scribe-client-legacy/src/ipc_client.rs#sync_linux_service_environment]] refreshes the user systemd manager's GUI session variables before starting the service so new PTY sessions inherit working clipboard/display access. Packaged user services are enabled under `graphical-session.target`, not `default.target`, so display-manager autostart waits until DISPLAY/XAUTHORITY are available.
...
The attach-time `clipboard_gating: bool` capability bit on `ClientMessage::Hello` and `ServerMessage::Welcome` ([[protocol#Server Messages#Clipboard Variants]]) is recorded on the window's participant entries in [[crates/scribe-server/src/ipc_server.rs#WindowShare]] (feature 015 consolidated the former per-window gating map into the share registry). When the attached client did not advertise gating, or no client is attached, all non-Deny arms short-circuit to the headless deny path (research decision 7): writes drop, prompts deny, reads reply empty.
...
Producers call [[crates/scribe-common/src/screen_replay.rs#build_session_replay]], which snapshots the `Term` via [[crates/scribe-server/src/session_manager.rs#snapshot_term]] (reading the application's DEC private modes from `term.mode()` into the snapshot's `active_dec_modes: Vec<DecPrivateMode>` list), runs [[crates/scribe-common/src/screen_replay.rs#snapshot_to_ansi]] to emit those mode-restoring DECSET sequences plus the scrollback + visible grid + cursor as an ANSI byte stream, and zstd-compresses the result. Consumers (both `restore_from_handoff` and the client's reattach `handle_session_replay`) decompress and feed the bytes through `vte::ansi::Processor::advance` into a freshly-constructed `Term`, so the restored session's grid and scrollback are populated durably — every subsequent attach sees the same content, not just the first.
...
Feature 013 (remote window control) added no handoff fields — the remote listener is re-derived from config by the receiver rather than carried on the wire — so it stayed at v6; see [[server#Remote Control#Handoff Interaction]].
...
The config also carries the feature-013 `[remote]` table; the same reload handler pokes the remote-control supervisor so the listener starts, stops, or rebinds live without a restart — see [[server#Remote Control#Listener Lifecycle]].
...
Path layout is `<state_dir>/restore/env/<window_id>/<launch_id>.envz`, where `<state_dir>` comes from [[crates/scribe-common/src/app.rs#current_state_dir]] (`dirs::state_dir()`, or the platform data dir — `~/Library/Application Support` on macOS, `%APPDATA%` on Windows — where XDG's `state_dir` is undefined, joined with the flavor slug from [[crates/scribe-common/src/app.rs#AppIdentity#slug]]; that fallback keeps state writable off-Linux, where `dirs::state_dir()` is `None` and every state consumer — restore, window state, env store, LAN trust stores, device-identity cert — would otherwise fail closed). This is the same flavor-aware root that backs [[crates/scribe-client-legacy/src/restore_state.rs#RestoreStore#new]]'s `restore/` subtree, so `scribe` and `scribe-dev` installs cannot collide and the env tree lives alongside the existing window-state tree. [[crates/scribe-server/src/env_store/store.rs#env_dir_for]] returns the per-window directory; [[crates/scribe-server/src/env_store/store.rs#envelope_path]] returns the per-launch file path; both return `StoreError::NoStateDir` when no state directory can be resolved.
...
The wire contract is [[protocol#Remote Protocol]] and design decisions are recorded in `specs/013-remote-window-control/` (research D1–D9). The listener is started, stopped, and rebound live off config — the server is NEVER restarted for this feature.

Feature 014 adds a second, Tailscale-free transport — a LAN link over mutual TLS gated by explicit device approval (`specs/014-lan-remote-control/`) — alongside the tailnet path. This splits the former single-transport state into [[server#Remote Control#Per-Transport State]], threads in the new [[crates/scribe-server/src/lan/mod.rs|lan/ module tree]] ([[server#Remote Control#LAN Identity and TLS]], [[server#Remote Control#LAN Discovery and Network Trust]]), and reuses the SAME `serve_connection` dispatch past its [[server#Remote Control#LAN Accept and Approval]] gate. The tailnet path is preserved byte-for-byte.

Feature 015 (`specs/015-multi-machine-sharing/`) moves the owning side beyond single-controller takeover: when the owner's `sharing_mode` permits it, multiple approved machines attach to ONE window at once (tmux/screen -x style), joining additively without displacing anyone (supersedes 013's FR-007 single-controller invariant). The three per-window maps and their fixed-order tri-lock collapse into one [[crates/scribe-server/src/ipc_server.rs#WindowShare]] entry per window; output fans out per participant and input authorization becomes mode-aware. The default `SingleController` mode keeps every 013/014 path byte-identical. See [[server#Remote Control#Sharing]] for the participant-set model, and [[protocol#Remote Protocol]] (v3) for the wire delta.

### Listener Lifecycle

[[crates/scribe-server/src/ipc_server.rs#RemoteControl]] is the shared handle threaded into [[crates/scribe-server/src/ipc_server.rs#IpcServerState]]; [[crates/scribe-server/src/ipc_server.rs#remote_supervisor]] owns the listener and serializes every start, stop, and rebind through one task so config edits never overlap.

At startup the supervisor applies the current `[remote]` config (a no-op when disabled, the default). Each [[protocol#Client Messages#Configuration]] `ConfigReloaded` reaches [[crates/scribe-server/src/ipc_server.rs#handle_config_reloaded]], which pokes [[crates/scribe-server/src/ipc_server.rs#RemoteControl#request_reload]]; the supervisor re-reads config and (per transport) [[crates/scribe-server/src/ipc_server.rs#RemoteControl#apply_tailnet]] starts, stops, or rebinds accordingly. Enabling enumerates the machine's tailnet bind addresses (fail closed if unavailable) and binds one `TcpListener` per address on `remote.port`, spawning an accept task each; disabling severs live connections and drops the sockets; a port or address change rebinds fresh while leaving live connections alone. The supervisor is spawned, not awaited, so a wedged tailscaled never delays local serving.

The one supervisor reconciles BOTH transports on each poke: after `apply_tailnet` it runs [[crates/scribe-server/src/ipc_server.rs#RemoteControl#apply_lan]] against the `[remote.lan]` table and the trusted-network gate, starting, stopping, or rebinding the LAN listener independently ([[server#Remote Control#LAN Accept and Approval]]). Reloads are also poked by the network-change watcher, so a roam off a trusted network takes LAN dormant without a config edit ([[server#Remote Control#LAN Discovery and Network Trust]]).

### Tailnet Identity

[[crates/scribe-server/src/tailnet.rs]] is a minimal hand-rolled Tailscale LocalAPI client plus the same-account authorization policy (research D2). It is the ONLY component that talks to `tailscaled`, and the GUI client never does.

The daemon is reached by a plain HTTP/1.1 request over its Unix socket on Linux, or the sandboxed localhost TCP LocalAPI on macOS. Two endpoints are consumed: `status` yields [[crates/scribe-server/src/tailnet.rs#SelfIdentity]] (own tailnet user id, device name, tailnet IPs) plus the same-account peer list for the picker, and `whois?addr=ip:port` yields the [[crates/scribe-server/src/tailnet.rs#TailnetIdentity]] behind an accepted connection. [[crates/scribe-server/src/tailnet.rs#check_policy]] is the pure rule: authorize iff the peer carries a concrete tailnet user id equal to this machine's own and is not tagged; [[crates/scribe-server/src/tailnet.rs#authorize_peer]] wires status, whois, and policy together. Any LocalAPI failure fails closed as `IdentityUnavailable` (never a wildcard bind, never an unauthenticated accept); a tagged or identity-less peer refuses `Unauthorized` with a `tagged` qualifier for audit. [[crates/scribe-server/src/tailnet.rs#bind_addresses]] enumerates the tailnet IPs the listener may bind to.

[[crates/scribe-server/src/tailnet.rs#SelfIdentity]] also carries a display-only `login_name` — the signed-in account login read from the LocalAPI User map — kept distinct from the `user_id` that anchors authorization: the id gates access, the login only names the account in UI. The local-only transient [[protocol#Remote Protocol#Local Env Query]] (`GetRemoteEnv`) resolves it through [[crates/scribe-server/src/ipc_server.rs#handle_transient_get_remote_env]] for the Settings → Remote panel, failing closed to `{ account: None, tailscale_detected: false }` (FR-015) on any LocalAPI error.

### Accept Path

[[crates/scribe-server/src/ipc_server.rs#remote_accept_loop]] hands each accepted stream to [[crates/scribe-server/src/ipc_server.rs#handle_remote_client]], which reads the preamble with [[crates/scribe-server/src/ipc_server.rs#read_remote_preamble]] (bounded and timed out so a silent peer cannot hold an accept slot), authorizes via [[crates/scribe-server/src/ipc_server.rs#authorize_remote]], gates the version, and answers with [[crates/scribe-server/src/ipc_server.rs#send_handshake_reply]] before any window data flows.

On accept the connection is registered against the 8-connection cap and given a sever channel; a refusal emits an audit line and closes the socket. An accepted connection then runs the SAME [[crates/scribe-server/src/ipc_server.rs#serve_connection]] dispatch as a local client, distinguished only by its writer being a [[crates/scribe-server/src/ipc_server.rs#ClientSink]]`::Remote` (bounded queue) rather than `Local` (direct socket) and by carrying the resolved tailnet identity so the accepted and disconnect audit lines can name the peer.

Admission is layered so a pre-auth peer cannot exhaust memory or slots (FR-013): [[crates/scribe-server/src/ipc_server.rs#remote_accept_loop]] reserves a pending-handshake permit (`REMOTE_PENDING_HANDSHAKE_CAP`, 64) the instant it accepts — before spawning any handler or reading a byte, mirroring the local `MAX_CONNECTIONS` pattern — and `read_remote_preamble` caps the preamble frame at `REMOTE_PREAMBLE_MAX_BYTES` (~8 KiB, far below the shared 64 MiB frame cap) so an unauthorized peer can never force a giant allocation. The authorized-connection cap (`REMOTE_CONNECTION_CAP`, 8) is unchanged and still refuses `Busy` after authorization.

The disable race now returns a typed refusal: [[crates/scribe-server/src/ipc_server.rs#RemoteControl#register_connection]] is reserved BEFORE [[crates/scribe-server/src/ipc_server.rs#send_handshake_reply]], so a connection that raced a live disable is refused `RemoteHandshakeReply { refusal: Disabled }` rather than told `accepted` and then silently dropped (FR-016). After acceptance [[crates/scribe-server/src/ipc_server.rs#establish_client_window]] runs as a loop: an authorized connection may send a read-only `ListWindows` probe before its `Hello` (see [[protocol#Remote Protocol#Window Probe]]) and registers no window for it; every other non-`Hello` first frame still closes.

### Takeover and Control

[[crates/scribe-server/src/ipc_server.rs#ControllerIdentity]] (`Local`, or `Remote` with device/login names) is tracked per participant in a window's [[crates/scribe-server/src/ipc_server.rs#WindowShare]] — feature 015's consolidated registry folding the three per-window maps and their tri-lock into one entry. A claim resolves through [[crates/scribe-server/src/ipc_server.rs#resolve_and_register_claim]].

The claim mode is derived by [[crates/scribe-server/src/ipc_server.rs#claim_mode_for]] from `Hello.takeover`, the window's sharing mode, and local/remote origin. A takeover swaps the window's sole controller. With sharing off, a local no-takeover claim falls through to today's assign-different-window path and a remote no-takeover claim of a still-connected window yields [[crates/scribe-server/src/ipc_server.rs#ClaimOutcome]]`::LostControl`. With sharing on, a no-takeover claim naming a connected window is an additive share join from either transport ([[server#Remote Control#Sharing#Local Additive Join]]). The controller identity, the attached-sink set, and the `clipboard_gating` bit all live on the one `WindowShare` entry and move together under a single write-lock hold, so a near-simultaneous takeover burst still resolves to exactly one controller and no stale clipboard or policy state survives the swap (FR-014). A takeover captures the displaced writer(s) in [[crates/scribe-server/src/ipc_server.rs#ClaimOutcome]]`::Owned`'s `displaced` vec so [[crates/scribe-server/src/ipc_server.rs#handle_client_hello]] can send each `WindowTakenOver` after the lock is released — the sole previous owner in `SingleController`, or EVERY attached participant when a `takeover: true` ends an active share (FR-003). A `LostControl` outcome completes `Welcome` but leaves the current controller untouched and sends the reclaimant an immediate `WindowTakenOver` (never a silent seizure, FR-011). [[crates/scribe-server/src/ipc_server.rs#log_control_transition]] records each transition.

### Control Authorization

Input authorization is enforced server-side, not merely signalled, and is mode-aware: [[crates/scribe-server/src/ipc_server.rs#connection_may_type]] decides whether a connection's gated frame reaches the session, dropping it safely otherwise (FR-006/007/011).

The guard (feature 015, replacing the single-writer `connection_controls_window`) branches on the window share's mode. In `SingleController` it is the legacy `Arc::ptr_eq` writer-identity test — the same one [[crates/scribe-server/src/ipc_server.rs#release_window_if_owned]] and [[crates/scribe-server/src/ipc_server.rs#detach_sessions]] apply — so a displaced connection (which keeps its stale, never-revoked `attached_ids`) is a no-op, byte-identical to feature 013. In `SharedSingleTypist` a gated frame is honored only from the current `SingleTypist` holder, while `Resize` is exempt — accepted from any attached participant as an ungated per-participant viewport report ([[server#Remote Control#Sharing]]). In `FreeForAll` `KeyInput` and `Resize` are admitted from any attached participant, and the lifecycle/focus/search frames (which have no single holder to follow) fall to the owning machine's always-present `ControllerIdentity::Local` participant (spec Assumptions).

[[crates/scribe-server/src/ipc_server.rs#dispatch_message]] applies the guard via [[crates/scribe-server/src/ipc_server.rs#requires_window_control]] to every window-mutating or scrollback-reading frame (`KeyInput`, `Resize`, `CloseSession`, `CloseWindow`, `FocusChanged`, `SearchRequest`), and [[crates/scribe-server/src/ipc_server.rs#filter_attachable_sessions]] applies it to `AttachSessions` so a displaced or unauthorized peer cannot re-steal the `PtyOutput` stream and clipboard-bridge routing by re-attaching. A local Unix-socket client is always its own window's controller, so the guard is a no-op locally; a `LostControl` reconnect never registered, so it fails the guard too, as intended.

### Sharing

Feature 015's owning-side sharing: when the owner's `sharing_mode` permits it, several approved machines attach to one window at once, and the roster, control state, and grid all live in that window's [[crates/scribe-server/src/ipc_server.rs#WindowShare]] (FR-001/002/004).

Design lives in `specs/015-multi-machine-sharing/`. The mode and its options are a snapshot, taken at each mutation, of the owner's [[crates/scribe-common/src/config.rs#RemoteConfig]] keys: `sharing_mode` ([[crates/scribe-common/src/config.rs#SharingMode]] = `SingleController | SharedSingleTypist | FreeForAll`), `control_acquisition` ([[crates/scribe-common/src/config.rs#ControlAcquisition]] = `FreeClaim | RequestAndGrant`), and `participant_limit` (`Option<u32>`, `None` = unlimited), all `#[serde(default)]` so an existing config loads legacy behavior (FR-004/005/018, SC-006). A remote no-takeover claim that the mode admits registers the joiner through [[crates/scribe-server/src/ipc_server.rs#register_additive_join]] as a new [[crates/scribe-server/src/ipc_server.rs#Participant]] — no existing participant disturbed (FR-002); a join beyond `participant_limit` is refused as a lost-control landing ([[crates/scribe-server/src/ipc_server.rs#ClaimOutcome]]`::LostControl` naming the current controller), leaving the active share undisturbed (FR-018).

Input control passes without disconnecting anyone through [[crates/scribe-server/src/ipc_server.rs#handle_control_message]], which routes the v3 `ControlClaim` / `ControlRequest` / `ControlGrant` frames. [[crates/scribe-server/src/ipc_server.rs#resolve_control_acquire]] is the acquisition decision: under `FreeClaim` a viewer becomes holder instantly (the previous holder demoted to a still-live viewer); under `RequestAndGrant` it records a [[crates/scribe-server/src/ipc_server.rs#PendingRequest]] and routes `ControlRequested` to the current holder (or the owner when unheld), resolved by [[crates/scribe-server/src/ipc_server.rs#resolve_control_grant]] on the answering `ControlGrant`. The owning machine can always claim regardless of the option (FR-005/007). A holder's detach or eject leaves control unheld — no silent inheritance (FR-016) — and [[crates/scribe-server/src/ipc_server.rs#broadcast_share_roster]] pushes the full-state `ShareRoster` to every participant on each membership, control, or mode change (FR-008, SC-005).

The session runs one authoritative grid sized smallest-attached-wins (FR-012): each participant's `Resize` is stored as its viewport (ungated in shared modes), and [[crates/scribe-server/src/ipc_server.rs#apply_authoritative_grid]] recomputes `min(rows)` × `min(cols)` over the attached viewports after a 250 ms debounce, driving the existing `resize_term` + `TIOCSWINSZ` path; because it is a pure function of the participant set it never flaps, and a participant detaching regrows the grid to the next-smallest.

Changing any sharing key applies live over `ConfigReloaded` with no restart: [[crates/scribe-server/src/ipc_server.rs#reconcile_shares_for_mode_change]] takes a [[crates/scribe-server/src/ipc_server.rs#SharingSnapshot]] and rewrites active shares immediately (FR-017) — `SharedSingleTypist` demotes all participants to viewers with control unheld, `SingleController` detaches every remote participant with the legacy `WindowTakenOver` displaced notice, `FreeForAll` makes everyone a typist — and cancels any pending control request, informing the requester.

Session-initiated clipboard requests route through [[crates/scribe-server/src/ipc_server.rs#send_clipboard_to_target]]: an OSC 52 write goes to the current control holder, falling back to the owning machine when control is unheld or in free-for-all where no single holder exists (FR-013). Revoking a device or severing a transport ejects only the affected participant via the existing sever→detach path ([[crates/scribe-server/src/ipc_server.rs#detach_client_window]]), which detaches its sessions, drops it from the share, and — through [[crates/scribe-server/src/ipc_server.rs#audit_membership_event]] on the 013 remote-audit target — records the departure as `eject` (versus `leave` for a clean disconnect); the share continues for everyone else (FR-011/015, SC-007).
...
Feature 015 gated [[crates/scribe-server/src/ipc_server.rs#ClaimMode]]`::ShareJoin` on a remote transport, so on one machine a shared window still admitted only its owner — a second local client asking for that window was handed an empty window of its own instead. [[crates/scribe-server/src/ipc_server.rs#claim_mode_for]] now selects `ShareJoin` for either transport whenever `sharing_mode` is not `SingleController`. `SingleController` keeps both legacy arms verbatim, and a `Hello { window_id: None }` — every stock client's handshake — never reaches the connected-window branch at all, so no user-launched client changes behavior.

The consequence that matters is two attached sinks on one session: [[crates/scribe-server/src/ipc_server.rs#connection_may_attach]] admits any participant in a shared mode, so `AttachSessions` adds the joiner's sink through `attach_additive` instead of replacing the incumbent's, and both processes receive the same `PtyOutput`. That is what the visual E2E shared-pane rig runs on ([[test#Visual E2E Tests#Shared-pane rig]]): `scribe-test` keeps observing the very pane the GPUI client renders.

### Flow Control

Each remote connection's writer is a [[crates/scribe-server/src/ipc_server.rs#RemoteSink]] wrapping a bounded output queue (`REMOTE_OUTPUT_QUEUE_BYTES`, 4 MiB) drained by a dedicated task, so a stalled remote consumer never blocks the server's authoritative Term or other clients (research D5, PR-004).

Feature 015 fans that per-connection queue out per participant: a session's [[crates/scribe-server/src/ipc_server.rs#ClientWriter]] holds an [[crates/scribe-server/src/ipc_server.rs#AttachedSinks]] set (one sink in `SingleController`, N in a shared window) whose `attach_additive` / `detach` maintain membership by `Arc::ptr_eq`, and [[crates/scribe-server/src/ipc_server.rs#send_to_client]] iterates the set so one `PtyOutput` frame reaches every attached participant, each buffered on its OWN `RemoteSink` queue. A slow participant's overflow, backlog-drop, and resync are therefore confined to its own queue — the session, the owning machine, and the other participants are never slowed (FR-009, SC-004).

`PtyOutput` frames are coalesced in the queue; on overflow [[crates/scribe-server/src/ipc_server.rs#drop_pty_backlog]] drops that connection's queued output and marks the affected sessions replay-dirty, and [[crates/scribe-server/src/ipc_server.rs#send_resync_replay]] sends a fresh [[protocol#Server Messages#Terminal Output#SessionReplay]] when the link drains (catch-up-to-current, mirroring tmux `%pause`). Control frames — the takeover notice, session-exit, workspace updates, the sever `RemoteDisconnect` — ride an `OutFrame::Message` lane the backlog drop leaves intact, so they are never lost. Clients already treat any `SessionReplay` as full pane-state replacement, so resync needs no new client logic. Local Unix-socket clients keep today's direct-write path; [[crates/scribe-server/src/ipc_server.rs#send_message]] routes both sink kinds, and even the updater broadcast now goes through it so a remote client's queue is honored instead of a blocking socket write.

Beyond that per-`PtyOutput` overflow, the queue also bounds its TOTAL footprint so a stalled link cannot grow unbounded on the `Keep` (control/replay) frames the `PtyOutput` cap does not govern: [[crates/scribe-server/src/ipc_server.rs#enforce_queue_ceiling]] caps total queued bytes (`REMOTE_OUTPUT_QUEUE_TOTAL_BYTES`, 16 MiB) and frame count (`REMOTE_OUTPUT_QUEUE_MAX_FRAMES`) with a shed-then-close policy — it sheds the droppable `PtyOutput` backlog first and, if the queue is STILL over ceiling, marks the connection closed for teardown, letting the client auto-reconnect to a fresh replay (FR-013).

A vanished (FIN/RST-less) peer no longer holds its authorized slot indefinitely: [[crates/scribe-server/src/ipc_server.rs#enable_tcp_keepalive]] sets `SO_KEEPALIVE` plus tuned idle/interval timers (`REMOTE_KEEPALIVE_IDLE` 60 s, `REMOTE_KEEPALIVE_INTERVAL` 15 s) via `socket2`, which maps them to the right per-platform sockopts (`TCP_KEEPIDLE` on Linux, `TCP_KEEPALIVE` on macOS) so a dead TCP path is dropped in a few minutes with no false positives on a live-but-idle viewer. A remote-only idle-read timeout (`REMOTE_IDLE_READ_TIMEOUT`, 30 min) in both the pre-`Hello` and message loops — local reads stay untimed — remains the application backstop for a peer that is TCP-alive but app-silent.

### Disable and Sever

Turning `remote.enabled` off must sever within 2 s (FR-016). [[crates/scribe-server/src/ipc_server.rs#RemoteControl#disable]] stops accepting, then [[crates/scribe-server/src/ipc_server.rs#RemoteControl#sever_all_connections]] fires every live connection's sever channel; each connection best-effort-sends `RemoteDisconnect { reason: Disabled }` under a bounded write timeout and closes.

`enabled` is flipped under the same lock the sever drains under, so a connection accepted mid-disable cannot slip past the sever. Owning-side sessions are untouched — only the remote client writers drop — and the connecting side auto-reconnects or, having received the notice, reports the disable as fact ([[protocol#Remote Protocol#Disconnect and Sever]]). A bulk `severed` audit line records the event.
...
Each transport (tailnet, LAN) owns a [[crates/scribe-server/src/ipc_server.rs#TransportControl]] carrying its own `enabled` flag, connection-cap semaphore, pending-handshake-cap semaphore, and [[crates/scribe-server/src/ipc_server.rs#SeverRegistry]]; [[crates/scribe-server/src/ipc_server.rs#RemoteControl]] holds `tailnet` and `lan` instances plus a shared connection-id allocator. The tailnet caps (`REMOTE_CONNECTION_CAP` 8 / `REMOTE_PENDING_HANDSHAKE_CAP` 64) are unchanged; the LAN transport gets its OWN `LAN_CONNECTION_CAP` (8) / `LAN_PENDING_HANDSHAKE_CAP` (64), so a flood of LAN dialers cannot exhaust tailnet admission. [[crates/scribe-server/src/ipc_server.rs#RemoteControl#register_connection]] takes the transport and, for LAN, the pinned `device_id`, and its disable-race check reads that transport's `enabled` under the same lock the sever drains under — so tailnet behavior is byte-identical.

The `SeverRegistry` keeps a secondary `device_id → {connection ids}` index — populated only for LAN (whose connections carry a Scribe identity; tailnet entries carry `None`) — so a per-device revoke severs just that device's connections via [[crates/scribe-server/src/ipc_server.rs#SeverRegistry#drain_device]] while every other connection and all owning sessions are untouched (FR-010). [[crates/scribe-server/src/ipc_server.rs#RemoteControl#sever_device_connections]] is the per-device revoke consumer; [[crates/scribe-server/src/ipc_server.rs#RemoteControl#sever_all_connections]] still severs one whole transport on disable or dormancy (FR-012).

### LAN Identity and TLS

The LAN link is mutual TLS 1.3 pinned to a per-install device identity. [[crates/scribe-server/src/lan/identity.rs]] mints the identity and [[crates/scribe-server/src/lan/tls.rs]] wraps the stream and enforces the pin; both fail closed when the identity or keyring is unavailable.

[[crates/scribe-server/src/lan/identity.rs#DeviceIdentity]] is a self-signed Ed25519 X.509 cert whose [[crates/scribe-server/src/lan/identity.rs#DeviceIdentity#device_id]] is `SHA-256(SubjectPublicKeyInfo)` — the 32-byte trust anchor and mDNS TXT `id`, stable across a cert re-mint. [[crates/scribe-server/src/lan/identity.rs#load_or_generate]] generates it once on first LAN enable, sealing the private key in the OS keyring (never plaintext on disk); [[crates/scribe-server/src/lan/identity.rs#own_fingerprint]] and [[crates/scribe-server/src/lan/identity.rs#fingerprint_words_for]] derive the word-list SAS shown on the approval prompt and in Settings (research D8). A headless machine with no keyring cannot be an owning-side LAN host in v1 (fails closed).

The CONNECTING side is a spawned `scribe-client` process — a DIFFERENT binary from the `scribe-server` that sealed the key — so it does NOT read the keyring itself. [[crates/scribe-server/src/ipc_server.rs#handle_transient_get_lan_dial_identity]] serves this machine's own identity (public cert + sealed key via [[crates/scribe-server/src/lan/identity.rs#DeviceIdentity#private_key_pkcs8_der]]) over the local-socket-only [[protocol#Remote Protocol#LAN Trust and Discovery Helpers|GetLanDialIdentity]] first frame, and the client rebuilds it with [[crates/scribe-server/src/lan/identity.rs#DeviceIdentity#from_der]] (re-deriving the same `device_id` from the key's SPKI, no keyring touched). This exists because macOS's `keyring` `apple-native` backend uses the legacy `SecKeychain` API, whose per-item ACL trusts only the CREATING binary; the different client binary is denied (errSecInteractionNotAllowed) with no usable prompt, so a direct client-side keyring read fails closed before any TCP. Routing through the server (the sole keychain accessor) is cross-platform — used on all OSes — and needs no entitlements, signing, or FFI. The handler mints on first use (`load_or_generate`), and the reply is refused over any remote transport exactly like `GetLanEnv` (see [[server#Remote Control#LAN Trust Management]]).

[[crates/scribe-server/src/lan/tls.rs#LanTls]] builds the `tokio-rustls` mutual-TLS config — both peers present and verify certs — and [[crates/scribe-server/src/lan/tls.rs#LanTls#accept]] / [[crates/scribe-server/src/lan/tls.rs#LanTls#connect]] run the async handshake. The [[crates/scribe-server/src/lan/tls.rs#PinningVerifier]] fills BOTH rustls verifier roles: it classifies a handshaking peer's [[crates/scribe-server/src/lan/tls.rs#device_id_from_cert]] against the live trusted-device store as [[crates/scribe-server/src/lan/tls.rs#PinDecision]] `Known` (verified) or unknown (pending TOFU, deferred to the app-layer approval gate) and — critically — still delegates the handshake-signature check to `rustls::crypto::verify_tls1{2,3}_signature`, so a peer lacking the private key hard-fails (research D3/D4). The pin check is a synchronous non-blocking store lookup via the [[crates/scribe-server/src/lan/tls.rs#DevicePins]] trait (adapted by [[crates/scribe-server/src/ipc_server.rs#TrustedDevicePins]]), so every approve/revoke is visible on the next handshake with no rebind.

### LAN Discovery and Network Trust

The LAN surface is dormant unless LAN access is enabled AND the machine is on a network the user marked trusted — a defense-in-depth activation gate (research D5), not authentication. [[crates/scribe-server/src/lan/discovery.rs]] runs mDNS and [[crates/scribe-server/src/lan/network.rs]] is the gate.

[[crates/scribe-server/src/lan/discovery.rs#LanDiscovery]] advertises `_scribe._tcp.local.` (port in the SRV record; `id`/`protovers`/`host` in TXT) via [[crates/scribe-server/src/lan/discovery.rs#LanDiscovery#start_advertising]] and browses peers via [[crates/scribe-server/src/lan/discovery.rs#LanDiscovery#start_browsing]], exposing a deduped, subnet-filtered view through a [[crates/scribe-server/src/lan/discovery.rs#LanPeerHandle]] the supervisor publishes for the local-only `ListLanPeers` handler. Advertising stops (mDNS goodbye) on disable, on leaving a trusted network, or on shutdown.

[[crates/scribe-server/src/lan/network.rs#current_network_fingerprint]] reads the physical-LAN [[crates/scribe-server/src/lan/network.rs#NetworkFingerprint]] (default-gateway MAC as the primary anchor plus subnet, via `netdev`), failing closed on a zero/unresolved gateway MAC, no default route, or a VPN-tunnel default route. [[crates/scribe-server/src/lan/network.rs#TrustedNetworksStore]] persists the trusted networks and answers "am I on a trusted network?" ([[crates/scribe-server/src/lan/network.rs#TrustedNetworksStore#is_current_network_trusted]]) plus add/remove/list; [[crates/scribe-server/src/lan/network.rs#lan_activation_snapshot]] is what [[crates/scribe-server/src/ipc_server.rs#RemoteControl#apply_lan]] consults each reload. [[crates/scribe-server/src/lan/network.rs#spawn_network_watcher]] periodically re-evaluates trust and pokes a reload on a status flip, so a roam onto an untrusted network goes dormant promptly without a config edit (FR-018, analysis C5).

### LAN Accept and Approval

[[crates/scribe-server/src/ipc_server.rs#RemoteControl#apply_lan]] binds the LAN listener only while enabled and on a trusted network; each accepted connection then runs TLS → `LanHello` → the device-approval gate → the 013 version gate → the SAME `serve_connection` dispatch.

When the gate passes, [[crates/scribe-server/src/ipc_server.rs#RemoteControl#start_lan]] binds one listener per physical-LAN address and spawns a [[crates/scribe-server/src/ipc_server.rs#lan_accept_loop]]; going dormant or disabling calls [[crates/scribe-server/src/ipc_server.rs#RemoteControl#deactivate_lan]], which aborts the accept tasks, severs live LAN connections, and stops advertising. The accept loop reserves the LAN pending-handshake permit the instant it accepts (mirroring the tailnet [[crates/scribe-server/src/ipc_server.rs#remote_accept_loop]]) and hands each stream to [[crates/scribe-server/src/ipc_server.rs#handle_lan_client]].

[[crates/scribe-server/src/ipc_server.rs#handle_lan_client]] is the full sequence: the mutual-TLS handshake, [[crates/scribe-server/src/ipc_server.rs#read_lan_preamble]] for the bounded `LanHello`, [[crates/scribe-server/src/ipc_server.rs#lan_approval_gate]] for the trust decision, the exact protocol-version gate ([[crates/scribe-server/src/ipc_server.rs#refuse_lan]] with `IncompatibleVersion`), the LAN connection cap, [[crates/scribe-server/src/ipc_server.rs#RemoteControl#register_connection]] recording the sever channel under the `device_id` index (re-checking the disable race), and finally the encrypted stream handed to [[crates/scribe-server/src/ipc_server.rs#serve_connection]] with a bounded [[crates/scribe-server/src/ipc_server.rs#ClientSink]]`::Remote` queue and a `Remote(device label)` controller — identical to 013 past this point.

[[crates/scribe-server/src/ipc_server.rs#lan_approval_gate]] is the trust decision (contract step 4): a [[crates/scribe-server/src/lan/tls.rs#PinDecision]] `Known` device proceeds immediately; an unknown device reserves a bounded, counted hold from [[crates/scribe-server/src/lan/trust.rs#PendingApprovals]] (cap [[crates/scribe-server/src/lan/trust.rs#MAX_PENDING_APPROVALS]], per-hold [[crates/scribe-server/src/lan/trust.rs#APPROVAL_TIMEOUT]] so unapproved dialers cannot occupy admission indefinitely), sends [[crates/scribe-server/src/ipc_server.rs#send_lan_pending]] (`LanApprovalPending`), and raises the prompt on the owning machine's own local clients via [[crates/scribe-server/src/ipc_server.rs#push_lan_approval_request]]. The owning user's `LanApprovalDecision` resolves the hold ([[crates/scribe-server/src/lan/trust.rs#ApprovalTicket#wait]] returns an [[crates/scribe-server/src/lan/trust.rs#ApprovalOutcome]]); on approve [[crates/scribe-server/src/ipc_server.rs#persist_trusted_device]] writes the [[crates/scribe-server/src/lan/trust.rs#TrustedDevicesStore]] pin ([[crates/scribe-server/src/lan/trust.rs#TrustedDevicesStore#approve]]) and the connection proceeds — revealing NO window or session data until then (SEC-001).
...
[[crates/scribe-server/src/ipc_server.rs#handle_transient_list_lan_peers]], [[crates/scribe-server/src/ipc_server.rs#handle_transient_list_trusted_networks]], [[crates/scribe-server/src/ipc_server.rs#handle_transient_add_current_network]], [[crates/scribe-server/src/ipc_server.rs#handle_transient_remove_trusted_network]], [[crates/scribe-server/src/ipc_server.rs#handle_transient_revoke_trusted_device]], and [[crates/scribe-server/src/ipc_server.rs#handle_transient_get_lan_env]] answer the [[protocol#Remote Protocol#LAN Trust and Discovery Helpers|local-only helpers]] from this machine's own view — refused over any remote transport, like 013's `ListRemotePeers`. Add-current-network fingerprints the physical LAN or explains why it cannot ([[crates/scribe-server/src/ipc_server.rs#lan_network_addable_reason]]); revoke calls [[crates/scribe-server/src/ipc_server.rs#RemoteControl#revoke_trusted_device]], which drops the pin and severs only that device's live connection through the `device_id` index (FR-010). The owning-side approval decision arrives as [[crates/scribe-server/src/ipc_server.rs#handle_lan_approval_decision]], which resolves the matching pending hold. `AddCurrentNetworkTrusted` / `RemoveTrustedNetwork` mutate through [[crates/scribe-server/src/ipc_server.rs#RemoteControl#add_current_trusted_network]] / [[crates/scribe-server/src/ipc_server.rs#RemoteControl#remove_trusted_network]] and poke a reload so [[crates/scribe-server/src/ipc_server.rs#RemoteControl#apply_lan]] re-evaluates activation (removing the current network goes dormant).

### Audit Log

Every remote lifecycle event is a structured server-log line on the [[crates/scribe-server/src/ipc_server.rs#REMOTE_AUDIT_TARGET]] tracing target (research D9, FR-017): `accepted`, `refused`, `disconnect`, and bulk `severed`.

[[crates/scribe-server/src/ipc_server.rs#audit_reason]] maps each [[crates/scribe-common/src/protocol.rs#RemoteRefusal]] to its canonical log token (`disabled` / `unauthorized` / `identity-unavailable` / `version` / `busy`) — the same taxonomy the wire refusal uses — with an optional `detail=tagged` qualifier when an `unauthorized` refusal was a tagged node. There is no dedicated audit UI in v1; the server log is the record.

Feature 014 extends the same target with `lan:`-prefixed lifecycle lines (FR-017). [[crates/scribe-server/src/ipc_server.rs#RemoteAudit]]`::Lan` carries the short device id so `serve_connection` emits `lan: accepted` / `lan: disconnect`; [[crates/scribe-server/src/ipc_server.rs#audit_lan_refusal]] maps each [[crates/scribe-common/src/protocol.rs#LanRefusal]] to its token (`declined` / `not-trusted-network` / `disabled` / `version` / `busy`) for `lan: refused`; approvals, declines, and revocations log `lan: approved` / `declined` / `revoked`; and [[crates/scribe-server/src/ipc_server.rs#audit_dormant_reason]] tokenizes a [[crates/scribe-server/src/ipc_server.rs#LanDormantReason]] for the bulk `lan: dormant` line emitted when the transport goes dormant (network untrusted / disabled).

### Handoff Interaction

A zero-downtime upgrade carries NO remote-control state — neither the listener's enabled/bind flag nor any active remote-connection metadata — so [[crates/scribe-server/src/handoff.rs#HANDOFF_VERSION]] is unchanged by this feature (see [[server#Handoff#Version Bumps]]).
