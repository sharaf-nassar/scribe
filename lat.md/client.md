...
Built-in recognizers come from [[crates/scribe-common/src/config.rs#default_smart_selection_rules]] and are restored via `terminal.smart_selection.reset`. The defaults include a `whitespace_word` fallback at VeryLow precision and a `word_no_trailing_period` rule at Low precision so prose words ending with a period (for example `hello.`) select without the period, while higher-precision recognizers (paths, URIs, emails, quoted strings, namespace identifiers) still win when they match.
