...
The band's clusters render as "zones": the left context group and each right-side cluster (repository/session metadata, then system stats, then the clock) keep their own spacing container but paint no fill — every cluster sits flat on the band's base background — split at `Span::zone_break` boundaries the model emits in place of the legacy " │ " inter-cluster hairlines, while segments inside one zone separate with a muted middot. Each stat carries an identity hue from the theme's ANSI palette (CPU blue, MEM magenta, NET cyan, GPU green) tinting its label, and its sparkline bars and readout band by load: the identity hue below 70%, ANSI yellow from 70%, ANSI red from 90% — the same warn/danger split the AI context bands default to — so a calm graph stays recognisably its own while a spike in any graph flares in the shared hot colors. The stats zone is width-pinned: [[crates/scribe-client/src/status_bar.rs#stats_zone_width]] measures the enabled segments' worst-case text (full-height bars, three-digit percentages, four-column rates, a GPU segment only when the model actually emitted one) and reserves that extent, because live samples repaint every couple of seconds and a fallback font can give the block glyphs uneven advances — without the pin, every sample shifted the zone's neighbours. The pinned cluster also paints as one shaped line: [[crates/scribe-client/src/status_bar.rs#stats_text]] joins the spans into a single `StyledText` with per-span color highlights, because the pin is measured by shaping the worst-case text once while a per-span element row lays every span out in its own rounded box — across the cluster's ~40 spans that drift outgrew the pin and clipped the zone's tail. The stable-width metadata cluster sits to the stats zone's left for the same reason. An actionable update CTA renders as an accent-tinted rounded chip (15% alpha fill, deeper on hover) instead of bare text; progress labels stay inert plain text. The balance button and settings gear share one trailing flat cluster of their own while keeping full-band-height hit targets inside it.
