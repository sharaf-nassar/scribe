...
Outbound: [[crates/scribe-client-gpui/src/ipc_bridge.rs#IpcSink]] replaces Zed's `write_to_pty` path, enqueuing `ClientMessage::KeyInput` / `Resize` plus the session-lifecycle messages the tab shortcuts drive (`CreateSession` / `AttachSessions` / `Subscribe` / `RequestSnapshot` / `CloseSession`), the window-lifecycle frames the close dialog, the window-list poll and the focus observer raise (`CloseWindow` / `QuitAll` / `ListWindows` / `FocusChanged`), and the workspace frames the split shell raises (`CreateWorkspace` / `CloseWorkspace` / `MoveSession` / `ReportWorkspaceTree`, see [[client#GPUI Client Spike#GPUI Pane And Workspace Shell#GPUI Workspace IPC]]) onto the ordered IPC-writer channel drained by [[crates/scribe-client-gpui/src/main.rs#run_writer]]. The sink is independent of the inbound drain, so a keystroke is never queued behind an output firehose; because the channel is a single FIFO, a `Resize` enqueued before a `KeyInput` reaches the server first. The GPUI view feeds the sink from [[crates/scribe-client-gpui/src/main.rs#TerminalView#on_key_down]] through [[crates/scribe-client-gpui/src/main.rs#encode_key]], which is the live entry point of the ported [[client#Input#GPUI Input Encoder Port]] rather than a table of its own.
...
[[crates/scribe-client-gpui/src/main.rs#supervise_connection]] retains one window's IPC queues across a local server handoff, redials with bounded backoff, and sends `Hello` plus `ListSessions` before queued UI traffic. The reply follows [[crates/scribe-client-gpui/src/main.rs#on_session_list]], rebuilding the registry, workspace metadata, and tab strip, then [[crates/scribe-client-gpui/src/main.rs#reattach_visible_sessions]] replays every visible pane's connection-local attachment at its retained grid dimensions. LAN and tailnet dials keep their one-shot refusal behavior so a rejected peer never becomes an endless local-looking retry.
...
[[crates/scribe-client-gpui/src/session_lifecycle.rs#PromptMarks]] holds the records — the port of the legacy client's per-`Pane` `command_records` — as absolute scrollback rows plus a [[crates/scribe-client-gpui/src/scrollbar.rs#CommandStatus]]. The record type itself is [[crates/scribe-client-gpui/src/scrollbar.rs#CommandMark]], owned by the scrollbar module because the store and the overlay's ticks are two ends of one record rather than two types that have to be converted between. `A` opens a record at the prompt row and prunes anchors the grid no longer holds, `D` resolves the most-recent still-open record from its exit code (`None` stays `Unknown`, so an unreported exit is never a failure), and `B` / `C` only move the pending input start.

The marks are anchored by the drain, not the reader, because an absolute row only means anything once the output that moved the cursor has been applied. `PromptMark`, `ScrollBottom` and `TrimScrollback` therefore travel the pane's own ordered channel as [[crates/scribe-client-gpui/src/ipc_bridge.rs#InboundEvent]] variants and come back out of [[crates/scribe-client-gpui/src/ipc_bridge.rs#coalesce]] as [[crates/scribe-client-gpui/src/ipc_bridge.rs#PaneOp]] entries interleaved with that pane's output runs; [[crates/scribe-client-gpui/src/main.rs#apply_pane_op]] then applies each in turn against the same grid. The server helps: it sends the `PtyOutput` chunk carrying the OSC before the `PromptMark` it parsed out of it, so the mark lands on the row the shell actually drew.

A `TrimScrollback` is applied there too, and it is two operations, not one. [[crates/scribe-client-gpui/src/terminal.rs#DisplayOnlyTerminal#trim_history]] first replicates the trim on the display grid — the server suppressed the AI session's ED 3, so the sequence never reached the client and the grid would otherwise keep rows the server has already forgotten — and reports how many of *its own* oldest rows that removed. [[crates/scribe-client-gpui/src/main.rs#apply_trim_scrollback]] then shifts every surviving anchor by exactly that count through [[crates/scribe-client-gpui/src/session_lifecycle.rs#shift_absolute_marks_after_trim]], retiring the marks whose rows are gone. The drop is measured on the client's ring rather than taken from the server's reported history, because the server names only the size it kept.

All three jumps share [[crates/scribe-client-gpui/src/main.rs#TerminalView#jump_to_mark]], which resolves the focused session, compares the marks against [[crates/scribe-client-gpui/src/terminal.rs#DisplayOnlyTerminal#viewport_top_abs]], dissolves the split-scroll pin a landing would otherwise outlive, and moves the viewport with [[crates/scribe-client-gpui/src/terminal.rs#DisplayOnlyTerminal#scroll_to_abs]]. `jump_to_failure` with no failed command is a deliberate no-op rather than a jump to the nearest thing (FR-011). A `ScrollBottom` replays the viewport reset a real ED 3 would have caused, which the server stripped from the stream.
...
State lives on the view, not on the element. [[crates/scribe-client-gpui/src/main.rs#ScrollbarSurfaces]] keys a [[crates/scribe-client-gpui/src/scrollbar.rs#ScrollbarHandle]] per *session* rather than per pane, because the fade belongs to the scrollback being scrolled: moving a session between panes carries its thumb with it, and a pane with no session has nothing to scroll. [[crates/scribe-client-gpui/src/main.rs#TerminalView#retire_scrollbars]] drops a closed tab's state alongside its grid.

Geometry is resolved during paint, because only the grid canvas knows the pane's pixel rect and the whole point of the overlay is that it hugs the right edge of the cells instead of reserving a gutter the layout would subtract. [[crates/scribe-client-gpui/src/main.rs#TerminalView#scrollbar_paint]] therefore supplies the *data* — [[crates/scribe-client-gpui/src/scrollbar.rs#ScrollMetrics]] read live off the grid by [[crates/scribe-client-gpui/src/terminal.rs#DisplayOnlyTerminal#scroll_metrics]], the pane's marks cloned out of the shared prompt-mark store, and the theme palette — and [[crates/scribe-client-gpui/src/terminal_element.rs#TerminalElement#paint_scrollbar]] calls [[crates/scribe-client-gpui/src/scrollbar.rs#build_scrollbar_render]] against the real bounds and lowers the result onto rounded GPUI quads. The track spans the full canvas with no tab-bar inset, because this client's tab strip lives in the window titlebar.

The palette comes from the theme, never from config. [[crates/scribe-client-gpui/src/scrollbar.rs#ScrollbarStyle#from_theme]] takes the derived `chrome.scrollbar` slot (whose 40 % alpha doubles as the resting fade ceiling) and the ANSI green/red for the ticks; `appearance.scrollbar_width` and `appearance.scrollbar_color` stay deliberately unread, because both are declared removed keys for the GPUI client and the width is fixed at [[crates/scribe-client-gpui/src/scrollbar.rs#SCROLLBAR_WIDTH]].

The fade is wall-clock, so it is driven from the idle tick rather than from output: [[crates/scribe-client-gpui/src/main.rs#TerminalView#tick_scrollbar_fades]] runs on the same 16 ms wake [[crates/scribe-client-gpui/src/main.rs#drive_redraws]] already uses for the share hint, and repaints while any scrollbar is animating *or* while this tick changed an opacity — the tick that finally reaches zero reports "not visible" while still owing the frame that clears it. [[crates/scribe-client-gpui/src/main.rs#TerminalView#pulse_scrollbar]] reveals the overlay from every path that moves a viewport, including a paging chord that hit the end of the scrollback, because that is exactly when the user wants to see where they are.

The pointer surfaces reuse the module's own hit-testing. [[crates/scribe-client-gpui/src/main.rs#TerminalView#press_grid]] offers a left press to the scrollbar before the mouse reporter, mirroring the winit client's chrome-first ordering: a click on the thumb was never meant for the application below it. A press on the thumb starts a drag, a press elsewhere in the 3x hit zone jumps the viewport to that point on the track, and both land on [[crates/scribe-client-gpui/src/terminal.rs#DisplayOnlyTerminal#scroll_to_offset]] so they share the split-scroll bookkeeping the keyboard path does. Hover is tracked even while an application owns the pointer, because the hover widen is what makes the 6 px thumb grabbable at all.
...
The shell holds no pixel state. Callers pass the grid area's measured pixel rect ([[crates/scribe-client-gpui/src/main.rs#TerminalView#pane_viewport]]), so the layout stays a pure function of the trees plus the window the grid actually got. The rect is reported by a measuring canvas in [[crates/scribe-client-gpui/src/main.rs#TerminalView#render_grid]] because the grid is the one flex-grown band and its height is whatever the chrome bands leave over — the prompt strip comes and goes with the pane's prompts, so no arithmetic on the window size can predict it. Before the first frame reports anything, the fallback is the nominal grid at the current [[crates/scribe-client-gpui/src/terminal_element.rs#GridFont]] metrics, which is exactly the size the window opens at. [[crates/scribe-client-gpui/src/pane_shell.rs#PaneShell#placements]] resolves that viewport into one [[crates/scribe-client-gpui/src/pane_shell.rs#PanePlacement]] per leaf, which [[crates/scribe-client-gpui/src/main.rs#TerminalView#render_panes]] lowers onto absolutely positioned children whose offsets and sizes are *fractions* of the grid area — so the ratios survive any window size without the view measuring device pixels. The focus ring is drawn only once a window actually has more than one pane, and takes the owning region's accent, so an unsplit window paints exactly as before.

[[crates/scribe-client-gpui/src/pane_shell.rs#PaneShell#dividers]] resolves each region's pure divider geometry over that same viewport, and [[crates/scribe-client-gpui/src/main.rs#TerminalView#render_dividers]] paints the resulting 1px quads above the grids. The grid pointer path claims the 4px divider hit band before terminal selection, maps motion through [[crates/scribe-client-gpui/src/divider.rs#drag_ratio]], then writes the ratio through [[crates/scribe-client-gpui/src/pane_shell.rs#PaneShell#set_pane_ratio]] and republishes both pane sizes.
...
`workspace_split_*` opens the region client-local (only the server may allocate a `WorkspaceId`, and `CreateWorkspace` carries none) and immediately sends [[crates/scribe-client-gpui/src/ipc_bridge.rs#IpcSink#create_workspace]]. [[crates/scribe-client-gpui/src/pane_shell.rs#PaneShell#split_workspace]] queues the region as awaiting a server workspace, so the answering `WorkspaceInfo` can be matched to the region that asked by the FIFO order of the one ordered writer channel — which is the only correlation available, since the request names nothing.

That answer arrives on the reader thread, where [[crates/scribe-client-gpui/src/main.rs#on_workspace_info]] splits it in two. The display name goes straight into the shared [[crates/scribe-client-gpui/src/chrome_metadata.rs#ChromeMetadata]] the status bar renders its workspace segment from — the same store `WorkspaceNamed` writes, so the two channels cannot disagree — and an absent name clears it. The id, accent and project root are parked for the GPUI thread, because a region is an entity the reader must never touch. [[crates/scribe-client-gpui/src/main.rs#TerminalView#adopt_workspace_info]] drains them at the top of the reconcile pass and [[crates/scribe-client-gpui/src/pane_shell.rs#PaneShell#apply_workspace_info]] folds each onto its region: a known id is a metadata refresh, an unknown one re-keys the oldest waiting region, and anything else is reported as unclaimed rather than silently renaming an unrelated region.

Draining before the session adoption below is what makes [[crates/scribe-client-gpui/src/ipc_bridge.rs#IpcSink#move_session]] work. The session a workspace split seeds is necessarily created through the workspace the tab strip was pointing at, because the new one does not exist yet; once the pane that adopts it turns out to sit in another region, [[crates/scribe-client-gpui/src/main.rs#TerminalView#follow_session_to_region]] tells the server so and re-files the strip entry, so a later split seeds its session in the region the user is actually looking at.

Collapsing a region sends [[crates/scribe-client-gpui/src/ipc_bridge.rs#IpcSink#close_workspace]]. Both removal paths report it — a deliberate `close_pane` through [[crates/scribe-client-gpui/src/pane_shell.rs#ClosedPane]]`::Removed` and an exited session through [[crates/scribe-client-gpui/src/pane_shell.rs#RetiredPanes]] — and only regions the server itself minted are ever named, because the server has never heard of a client-minted id.

Every one of those mutations ends in [[crates/scribe-client-gpui/src/main.rs#TerminalView#report_workspace_tree]], which sends [[crates/scribe-client-gpui/src/ipc_bridge.rs#IpcSink#report_workspace_tree]] with the tree [[crates/scribe-client-gpui/src/pane_shell.rs#PaneShell#wire_tree]] serializes. The topology comes from the [[crates/scribe-client-gpui/src/workspace_tree.rs#WorkspaceTree]] that owns it and the per-region payload from the shell's own pane trees, because the GPUI shell keeps panes in [[crates/scribe-client-gpui/src/pane_tree.rs#PaneTree]] entities rather than in the workspace model's tab list; a region maps to one tab whose pane tree is the region's whole split, which is the wire shape the winit client restores from. Panes still waiting for a session are pruned rather than serialized under a synthetic id, so a reconnect cannot restore a pane pointing at a session that never existed. Because the report is called from paths that fire far more often than the topology moves, the throttle is an exact equality check against the last reported value — which is sound precisely because that value *is* the wire payload.
...
The cell count divides real pixels by the live cell box, which is why the viewport above is measured rather than derived from the font: a rect stated in the font's own cells moves its numerator and denominator together, so every font size yields the same `cols`x`rows` and a zoom step would leave the freed pixels dead while telling the server nothing new. [[crates/scribe-client-gpui/src/main.rs#TerminalView#sync_grid_geometry]] closes the other half of that loop — it compares the measured area against the last published size and republishes exactly once when a window resize or a chrome band moved the grid's boundaries.

What arms that comparison is [[crates/scribe-client-gpui/src/main.rs#TerminalView#grid_area_probe]], the invisible canvas that measures the band. The rect is written during *prepaint*, after the render that built the canvas has already run, so the single repaint a window resize buys still reads the pre-resize area and — with nothing scheduling another frame — the panes would be re-laid on screen while every PTY kept its old size. The measuring write therefore reports whether the rect moved ([[crates/scribe-client-gpui/src/terminal_element.rs#record_grid_area]]) and defers a call back into `sync_grid_geometry` when it did, which keeps the publish on the view rather than mid-paint while guaranteeing it sees a measured area. Gating on a *moved* rect is what keeps it a single republish instead of a per-frame `Resize` storm.
...
Nothing is performed on the IPC reader thread. arboard is a window-thread resource and the FR-019 focus gate can only be judged against a live window, so [[crates/scribe-client-gpui/src/main.rs#on_clipboard_message]] folds `ClipboardPromptRequest`, `ClipboardBridgeWrite`, and `ClipboardBridgeReadRequest` into the shared [[crates/scribe-client-gpui/src/clipboard.rs#ClipboardBridge]] — a parked prompt plus a bounded job queue — and the window-lifecycle tick drains it through [[crates/scribe-client-gpui/src/main.rs#TerminalView#poll_clipboard]]. A frame arriving before the capability was negotiated is dropped, exactly as the winit client drops one: without that bit the server should not have sent it. The queue is bounded at [[crates/scribe-client-gpui/src/clipboard.rs#MAX_PENDING_BRIDGE_JOBS]] because a PTY program can emit OSC 52 far faster than a 200 ms tick drains.

The confirmation modal resolves through the same [[crates/scribe-client-gpui/src/dialog.rs#DialogOutcome]] path every other dialog uses: [[crates/scribe-client-gpui/src/main.rs#TerminalView#answer_clipboard_prompt]] correlates the choice back to the parked `request_id` and sends it through [[crates/scribe-client-gpui/src/ipc_bridge.rs#IpcSink#clipboard_answer]], the one seam the two spec-010 client answers leave by. The reply is not optional — the server holds the PTY-side program until the id resolves — so Esc and a backdrop click send a deliberate `DenyOnce` rather than nothing, and an `Always*` choice additionally persists the axis through [[crates/scribe-client-gpui/src/clipboard.rs#persist_policy_axis]].

Copy and paste are the user-facing half. A drag over the grid drives [[crates/scribe-client-gpui/src/terminal.rs#DisplayOnlyTerminal#begin_selection]] / [[crates/scribe-client-gpui/src/terminal.rs#DisplayOnlyTerminal#extend_selection]] at the granularity [[crates/scribe-client-gpui/src/mouse_state.rs#MouseClickState#record_press]] classifies (click / double / triple), and the range is projected onto the painted viewport by [[crates/scribe-client-gpui/src/selection.rs#viewport_spans]] and painted by [[crates/scribe-client-gpui/src/terminal_element.rs#TerminalElement#apply_selection]] in the theme's selection colours, under the find-match accent so a match inside a selection still reads as a match. Settling a drag publishes the text to the X11 primary selection when `copy_on_select` is on; the `copy` chord runs the same text through the AI cleanup transforms into the system clipboard; typing clears the selection, because the highlighted region describes content the shell is about to overwrite.

Every paste — the chord, a middle click, and the context menu's Paste row — is requested through the spec-011 [[crates/scribe-client-gpui/src/paste.rs#PasteGate]], so risky content parks behind [[crates/scribe-client-gpui/src/dialog.rs#PasteConfirmationDialog]] and resumes on the exact original bytes when confirmed. Delivery goes through [[crates/scribe-client-gpui/src/paste.rs#paste_chunks]], which splits the payload into frames the server's `KeyInput` limit accepts while keeping a single DEC 2004 marker pair across the whole paste.
...
[[crates/scribe-client-gpui/src/main.rs#TerminalView#open_remote_connect]] resets the transport-free [[crates/scribe-client-gpui/src/remote.rs#RemoteConnect]] state, requests both local discovery lists, and [[crates/scribe-client-gpui/src/main.rs#TerminalView#sync_remote_connect]] folds the reader-owned snapshots into its peer step. [[crates/scribe-client-gpui/src/remote_picker.rs#remote_picker_overlay]] renders the picker above ordinary overlays. Selecting a tailnet peer runs `RemoteHandshake` then `ListWindows`; selecting a LAN peer builds its local identity, completes mutual TLS plus `LanHello` approval, then runs the same probe. Selecting a listed window launches a new client with the chosen dial environment, leaving the local window untouched. The visual E2E proves the overlay paints a discovered peer and sends the tailnet probe on the TCP wire ([[test#Visual E2E Tests#Tailnet remote control]]).

The picker starts its probes through [[crates/scribe-client-gpui/src/main.rs#TerminalView#probe_remote_windows]] on the app-global Tokio bridge, rather than GPUI's foreground executor: TCP timers and sockets therefore always have a Tokio reactor, while every result and LAN approval notice returns through a GPUI task before changing the view. This keeps both transport handshakes asynchronous without letting a network task touch GPUI state.
...
[[crates/scribe-client-gpui/src/titlebar.rs#TitlebarView]] is the `gpui::Entity` that assembles the chrome as `div` elements: a `WindowControlArea::Drag` move region, the workspace-badge pill, the tab strip (active accent underline, per-tab close button revealed on hover, AI activity dot, context-% suffix, and drag-reorder slide), the equalize and gear icons, and the min/maximize/close window controls. Each interaction mutates state and emits a [[crates/scribe-client-gpui/src/titlebar.rs#TitlebarEvent]] the shell acts on — the gear's `OpenSettings` is subscribed in [[crates/scribe-client-gpui/src/main.rs#TerminalView#build_titlebar]] and opens the settings window ([[settings#GPUI Settings Window#In-app entry points]]); [[crates/scribe-client-gpui/src/titlebar.rs#TitlebarView#update_drag]] reorders tabs live as the cursor crosses a neighbour, and the window controls drive the platform window through GPUI's `WindowControlArea` hit regions. [[crates/scribe-client-gpui/src/titlebar.rs#pane_title_pill]] builds the semi-transparent per-pane title pill the shell overlays on a split pane; [[crates/scribe-client-gpui/src/titlebar.rs#WindowControlKind]] names the three window-control buttons. The spike wires the titlebar above the terminal grid in [[crates/scribe-client-gpui/src/main.rs#TerminalView]] so the visual E2E harness (`tests/e2e/visual/titlebar.sh`) can screenshot the assembled bar and its interaction checklist.
...
The settings row's destination is another top-level window: it lowers onto `KeyAction::OpenSettings` and reaches [[crates/scribe-client-gpui/src/main.rs#TerminalView#open_or_focus_settings]] ([[settings#GPUI Settings Window#In-app entry points]]). Rows whose destination surface is not ported yet — the remote picker, the update dialog, and the clipboard/selection trio — are still *routed*: they reach a dispatcher and are named and counted by [[crates/scribe-client-gpui/src/main.rs#unroutable_action]] instead of being discarded at the subscription. That warning is the difference between "not built yet" and "silently dead".
