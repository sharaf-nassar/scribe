...
An exit that drains a workspace also drops that workspace from the first-seen order, so a long-lived client that opens and closes many workspaces does not grow the order without bound; a session re-created in that workspace lands at the end, not at the old slot.
...
It also advertises `Hello.ai_background_wait = true`, so the server sends `WaitingForBackground` rather than its `Processing` downgrade.
...
A drag may also cross the titlebar/lower-bar boundary. [[crates/scribe-client/src/main.rs#TerminalView#update_tab_subtree_drag]] resolves the other region's bar and insertion slot while the existing titlebar or lower-bar marker remains the native GPUI drag. Same-region swaps keep their existing live behavior; once the pointer enters another region, release first restores any provisional source reorder and sends one capability-gated `MoveWorkspace` with `WorkspaceMoveOperation::MoveTab`. Nothing is re-filed locally. The server's full `SessionList` and tree are authoritative, so cancellation, an unsent request, and every typed refusal leave both regions byte-for-byte as they began.
...
The GPUI client ports cold-restart recovery: after a server crash the bootstrap window rebuilds its windows, workspaces, tabs, and panes from persisted snapshots and re-creates each saved session at the correct geometry.
 ...
 persists one TOML snapshot per window under `$XDG_STATE_HOME/scribe/restore/windows/<window_id>.toml` plus a shared `index.toml`, all hardened to `0700`/`0600` because launch bindings can carry prompt text and provider conversation IDs. A bootstrap lock serialises multi-process index mutations and stale locks (>30 s) are reclaimed. Every snapshot write goes through a private temp file that is `sync_all`ed before the rename: rename is only atomic for data already on disk, and a power cut after an unsynced rename publishes an empty inode that cold restart replays as nothing.  atomically claims the first replayable  entry, skips non-replayable and unreadable entries, and reports how many windows remain. Those are fanned out as `--restore-child` processes via  only once the server's first `SessionList` proves it lost its sessions, because a server that kept them restores its own windows through `Welcome`'s `other_windows` and fanning out both would open every window twice; each child claims exactly one more entry and, gated by , never fans out again. A claim is non-destructive: the snapshot file stays on disk as the window's last good layout, its id parked in the index's `claimed` list beside a claim timestamp so it cannot be claimed again while the claim is fresh, and it is retired only when the claiming window durably writes a fresh snapshot of its own, or the user explicitly closes the window. A claim older than [[crates/scribe-client/src/restore_state.rs#CLAIM_TTL_MS|CLAIM_TTL_MS]] (15 min) whose window never flushed that superseding snapshot names a claimant that crashed or wedged mid-replay, so the next claim scan ([[crates/scribe-client/src/restore_state.rs#reclaim_stale_claims|reclaim_stale_claims]]) moves it back to the front of the claimable list — without the expiry, one mid-replay crash parked the layout in `claimed` forever and it was never restorable again. A claim whose timestamp is missing (an older client rewrote the index) is re-stamped at the scan instant, restarting its TTL rather than reading as immediately stale. A claim the window never consumed — the server kept its sessions, so nothing was replayed — is released rather than retired, because a claim the server refused names another live window's layout and deleting it is how a layout gets lost.
...
Geometry is read back off the live window by [[crates/scribe-client/src/main.rs#TerminalView#capture_geometry|capture_geometry]] through [[crates/scribe-client/src/window_state.rs#geometry_from_bounds|geometry_from_bounds]]. The size comes from GPUI's cached bounds — already logical pixels, so no scale-factor division is needed — but on X11 the origin must not: the cached origin is raw `ConfigureNotify` data, and a reparenting window manager can leave it frame-relative. Mutter answers a resize with the client's offset inside its frame and a move with an origin shifted by that same offset, which is how persisted records came to hold the frame offset `(14,49)` and a later restore landed the window at the primary monitor's top-left with its titlebar under the GNOME panel. [[crates/scribe-client/src/main.rs#TerminalView#resolve_root_origin|resolve_root_origin]] therefore takes the origin from the root-relative [[crates/scribe-client/src/monitor.rs#logical_window_origin|logical_window_origin]] (`translate_coordinates` scaled to logical pixels), re-querying only when the cached reading moves so a steady window costs no X11 round trips per paint; [[crates/scribe-client/src/main.rs#TerminalView#verify_restored_position|verify_restored_position]]'s landing check reads the same answer, which is what stopped it reporting a frame-offset "landing" for a window sitting on its saved monitor. It runs from an `observe_window_bounds` subscription (the GPUI equivalent of winit's `Moved`/`Resized`) and again on every paint, so a window that is opened and never touched still persists the size it came up at. Both files are written on a 500 ms debounce (`RESTORE_DEBOUNCE`), since a drag-resize emits a bounds change per frame and a split re-reports the tree several times while sessions arrive.

The snapshot is marked stale from , the single funnel every layout change already passes through. Prompt-history changes dirty it too: prompt rows ride the snapshot's launch records, so [[crates/scribe-client/src/main.rs#AiChrome#take_prompts_dirty|a dirty flag on AiChrome]] is drained by the restore poll into the same debounced flush — without it a prompt typed right before a crash stayed off disk until the next split, close, or tab switch dirtied the layout.  serialises the live shell into the ported format: the shell keeps panes in  entities while `snapshot_window_restore` is written against , so a scratch layout is filled from the live trees — one region becomes one tab whose pane tree is the region's whole split, the same shape reported to the server — and panes still awaiting a session are pruned exactly as they are on the wire. A window with nothing replayable in it is *removed* from the store rather than saved blank, so the next cold start cannot claim it and replay an empty window forever.
...
[[crates/scribe-client/src/main.rs#TerminalView#fill_pending_panes|TerminalView::fill_pending_panes]] drains replay answers. The ordinary [[crates/scribe-client/src/main.rs#TerminalView#reconcile_panes|reconcile pass]] adopts one arriving session per tick, which is enough for a split but not for a replay: five `CreateSession` frames come back faster than five ticks, so four panes would stay empty while their sessions lived on as tabs with nowhere to render. During a replay its active-session branch is gated off: `active_session` is overwritten by the newest arrival, while the restored panes are pending in launch order. `fill_pending_panes` owns every pairing until that queue drains.

That FIFO is sound only during replay. [[crates/scribe-client/src/restore_replay.rs#rebuild_layout_from_snapshot|Snapshot rebuilding]] constructs launches workspace-major; [[crates/scribe-client/src/tab_session.rs#TabSessions#push|TabSessions::push]] appends tabs within each region and creates regions in first-arrival order; and [[crates/scribe-server/src/ipc_server.rs#handle_list_sessions|handle_list_sessions]] only sends `SessionList` as a `ListSessions` reply, never as a mid-replay broadcast. Outside replay the pairing would be wrong — a split's pending pane must get the session that split asked for, not whichever older tab happens to have no pane.

A snapshot carries one pane tree per *tab*, so the replay pairs launches per tab tree rather than per region. [[crates/scribe-client/src/restore_replay.rs#ReplayLaunch|ReplayLaunch]] marks the first launch of each saved tab as `is_tab`; that one is sent as a `PendingCreate::Tab` and recreates the strip entry, and the rest of that tab's launches are `PendingCreate::Pane` answers that fill panes inside it. Until each tab's answer arrives its tree is keyed by the snapshot's placeholder session id, and `PaneShell::sync_tabs` re-keys it onto the real session the first time the strip names that tab — matched by the same per-region FIFO order the launches were queued in. Every pane still takes its session through `adopt_session`, so a replayed pane's persisted prompt rows are seeded exactly once.
...
A fresh claim shields its snapshot — nothing further is claimable — but a claim aged past `CLAIM_TTL_MS` is reclaimed at the next claim scan and the same snapshot replays again, ahead of younger unclaimed windows.

A claim whose stamp an older client's index rewrite dropped is re-stamped "now" and keeps shielding its snapshot until that restarted TTL expires, so a stampless claim never reads as immediately stale.
...
What it is handed is the rect the pane *paints* into, produced by [[crates/scribe-client/src/main.rs#TerminalView#painted_pane_rect|painted_pane_rect]]. Three bands live inside a placement rect and none of them belong to the PTY: a lower region's tab bar (already taken off by [[crates/scribe-client/src/pane_shell.rs#PaneShell#content_rect|content_rect]] at the live resolved `tab_height + tab_bar_padding` before placements are returned), the one-pixel hairline border every pane card draws plus the 10px content padding inside it (GPUI hands taffy both as box insets, so the child's content box is 22px smaller on each axis), and the pane-internal prompt strip. Reserving neither the border nor the strip is how the client came to report more columns than it renders.
...
The pane split tree is  (binary `Leaf`/`Split` nodes, ratios clamped 0.1-0.9, spatial-overlap directional focus with edge wrap). The workspace split tree is , whose `WorkspaceSlot` carries tabs, active tab index, accent color, name, and project root. `TabState` drops the winit client's `selection` field until terminal selection is ported in Phase B. Its `restored_focused_pane` is snapshot-only: `PaneShell::snapshot_layout` writes it and the restore replay reads it, while the live focused pane is the shell's own `focused` map. The name says so because a same-named `focused_pane` invited a runtime read that returned whatever the last snapshot recorded.

#### Pane Tree Model

 is a `gpui::Entity` wrapping a `LayoutTree`. Every structural mutation emits `PaneTreeEvent::Changed` and calls `notify()`.

 re-equalizes every ratio after a split lands, so all panes in the tab immediately rebalance to equal space, while  promotes the closed pane's sibling into the freed extent and preserves ratios outside the changed subtree. `close` refuses to remove the sole root leaf.  remains the explicit balance action,  clamps to 0.1-0.9, and `find_pane_in_direction` resolves directional focus (with edge wrap) without mutating or emitting.
...
Reported mutations include workspace split, tab add/remove, , workspace ratio change (clamped 0.1-0.9), in-place slot edits via `update_slot`, and workspace-pill rearrangement through [[crates/scribe-client/src/workspace_tree.rs#WorkspaceTree#rearrange_workspace]]. [[crates/scribe-client/src/workspace_layout.rs#WindowLayout#remove_tab]] settles the surviving selection through the shared [[crates/scribe-common/src/protocol.rs#active_tab_index_after_departure]] rather than a clamp of its own: this model's `active_tab` rides the reported tree the server persists, and the old clamp-only rule kept the *index* while the list shifted, so removing a tab before the shown one silently re-filed the selection and the wrong tab came back after a reconnect. On reconnect the restore path pushes tabs (each auto-activating the last) and then replays `active_tab_index` through `set_active_tab` to restore the originally focused tab, matching the winit client's post-pass.

[[crates/scribe-client/src/workspace_layout.rs#WindowLayout#rearrange_workspace]] applies the shared `WorkspaceTreeNode` extract/insert/swap operations, then lowers the edited topology back over the existing client-owned slots. Edge moves therefore re-equalize ratios; center swaps preserve shape and ratios; tab state, pane layouts, metadata, active-tab selection, and the moved workspace's focus stay attached to their IDs.
...
### GPUI Pane And Workspace Shell
 ...
 is what makes the two split trees above reachable from the running binary: it owns the window's one  plus one dormant  per strip tab, keyed by that tab's root session. A workspace region renders only its active tab's tree.
...
The shell holds no pixel state. Callers pass the grid area's measured pixel rect () plus the live resolved tab-row height, so the layout stays a pure function of the trees, chrome input, and the window the grid actually got. The rect is reported by a measuring canvas in  because the grid is the one flex-grown band and its height is whatever the chrome bands leave over — the prompt strip comes and goes with the pane's prompts, so no arithmetic on the window size can predict it. Before the first frame reports anything, the fallback is the nominal grid at the current  metrics, which is exactly the size the window opens at.  resolves that viewport into one  per leaf, shrunk through [[crates/scribe-client/src/pane_shell.rs#PaneShell#card_rect]] into its pane card: adjacent panes keep 6px of window ground between them (3px per side) and every pane keeps a 6px margin to its region's content edge. The inset lives inside `placements` — the one per-frame resolver — so paint, published PTY sizes, dividers, and hit-tests shrink together by construction.  lowers those cards onto absolutely positioned children whose offsets and sizes are *fractions* of the grid area — so the ratios survive any window size without the view measuring device pixels. Every pane card carries its own theme-background fill (skipped default-bg cells reveal the card, never the ground), an 8px corner radius, a hairline border, and 10px of content padding between that border and the glyphs — subtracted again in `painted_pane_rect` so the PTY is never told about columns the padding consumed; the focused pane's border takes the workspace's effective accent ([[crates/scribe-client/src/main.rs#TerminalView#region_accent]]) — the layered chrome mock's bright focus ring — while resting panes wear a whisper edge, the divider tone at 40% of its own alpha, so a resting seam reads as a card edge rather than a drawn line. The radius rounds the card's own fill and border, but children paint under GPUI's rectangular content mask, so a full-background application's corner cells can square off the curve — an accepted ceiling until corner radii are threaded into the grid's corner cell-bg quads. The prompt strip paints its own background under that same mask, so its outer corners take the card radius themselves (top or bottom per `terminal.prompt_bar_position`) rather than squaring off the corner they sit in. The grid band behind the cards paints the theme background stepped to 80% — [[crates/scribe-client/src/main.rs#pane_ground]], delegating to the shared [[crates/scribe-common/src/theme.rs#ground_tone]] that also derives the tab strip's `tab_bar_bg`, so the strip and the gaps are one surface — and the gaps read as a layer beneath the terminal surfaces rather than as seams in one flush surface. A black or near-black background has nothing left to darken, so below a small brightness floor the ground lightens by a fixed step instead: the layer split survives in both directions, and only which side is darker flips. A pane whose session is still in flight — a restore replay, or a fresh split awaiting `SessionCreated` — paints [[crates/scribe-client/src/main.rs#pane_skeleton]] instead of an empty grid: quiet bars that set the card's layout expectation, breathing on the CI trace's 1.2s cadence because waiting is the same kind of liveness, rendered static under the shared animation policy, and replaced wholesale by the session's first frame.

 resolves pane and workspace-region divider geometry over that same viewport, but no divider line is painted — the gaps stay bare window ground, per the layered chrome mock, and a workspace divider's full 4px-tolerance hit band carries GPUI's native left-right or up-down resize cursor as the only visible affordance. The grid pointer path claims either divider before terminal selection, maps motion through the matching ratio helper, then writes the ratio through  and republishes both adjacent pane sizes.

Workspace pills now route through [[crates/scribe-client/src/workspace_drag.rs#WorkspaceDrag]], a pure idle → armed → dragging/tear-armed lifecycle. Every frame re-tests the last delivered pointer against live workspace rects and divider hit bands, so ghost and zone previews are level-triggered rather than stale hover caches. Release returns any actionable tree edit while resetting the drag to idle; the caller then uses [[crates/scribe-client/src/pane_shell.rs#PaneShell#rearrange_workspace]] and the existing `ReportWorkspaceTree` path. Escape, blur, disappearance, source/self drops, dividers, chrome, and tear-armed release clear the drag without a tree report.

[[crates/scribe-client/src/workspace_drag.rs#WorkspaceDragMotion]] carries only transient feedback after that lifecycle changes: old zones fade out, live zones fade in, successful drops settle the ghost onto the moved region, and cancelled/no-op releases snap it back to the grab point. Both durations resolve through [[crates/scribe-client/src/animation.rs#AnimationSettings]], stay at or below the 150ms `MAX_TRANSITION`, and collapse to no transient layer when `appearance.animations` is false or `SCRIBE_DISABLE_ANIMATIONS` is truthy. The drag source leaves the pill press in GPUI's bubble path so native `on_drag` can record it; the titlebar tracks that press separately to suppress compositor window movement without stopping GPUI's threshold listener.

[[crates/scribe-client/src/workspace_drag.rs#DragProbe]] is constructed once from `SCRIBE_DRAG_PROBE`. When disabled it stores no timestamps and emits no tracing. When enabled, each delivered workspace-drag pointer update stamps ingestion and a priority-3 deferred paint callback emits `input_to_paint_us`, closing the measurement at overlay paint rather than render-tree construction.

The command palette adds five client-local workspace actions. Its four distinct directional labels carry one `FocusDirection`-backed action that calls [[crates/scribe-client/src/pane_shell.rs#PaneShell#move_focused_workspace_in_direction]], which uses the same extract/insert operation as its matching drag edge but does not wrap at a window edge; a no-neighbor action leaves the tree unchanged and logs feedback. The new-window action enters the same correlated transfer coordinator as a tear-out, with the focused workspace and source size but no cursor placement. They are palette-only: no `KeybindingsConfig` or settings surface is added.

 closes a pane inside the tab's own tree; a tab's *sole* pane answers  `LastPane` and the caller falls through to closing the tab, which closes every session in that tab's tree. A region collapses only once its last tab is gone:  is the shared removal path, so an exited session's pane, an emptied tab and a tabless region all retire the same way a deliberate close does. A tab whose own session exits while its splits are still running re-anchors onto a surviving pane — the same rule `prune_workspace_node` applies to the persisted tree — and the strip takes that pane as the tab through [[crates/scribe-client/src/tab_session.rs#TabSessions#promote_pane]], so live split sessions can never be stranded in a tree the strip no longer reaches.
...
Three things can be out of step. The root region starts on a client-minted `WorkspaceId` because the shell exists before the first `Welcome`, and adopts the server's through  once a `SessionList` names one. A pane whose session exited is retired by . And a split's `CreateSession` answer is identified by `IpcSink::claim_pending_create`'s ordered FIFO: `PendingCreate::Pane` is recorded through [[crates/scribe-client/src/tab_session.rs#TabSessions#insert_pane]] and never reaches [[crates/scribe-client/src/tab_session.rs#TabSessions#insert_active]], so it is adopted straight into the pane the split queued. `PendingCreate::Tab` answers alone enter the strip — including the seed session of a `workspace_split_*`, which is a tab of its new region. Each strip tab owns its own pane tree; [[crates/scribe-client/src/pane_shell.rs#PaneShell#sync_tabs]] gives every tab of every *visible* region a tree and points each region at the tab the strip shows, and switching tabs swaps the whole tree and streams all of its panes rather than adopting a tab into the outgoing focused pane. The active-session adoption is gated by [[crates/scribe-client/src/main.rs#tab_adoption_candidate]]: only a session assigned to no tab tree at all — shown or dormant — is ever adopted, because an assigned session has a home and showing it is a tab switch. Without that gate, focus-follows-mouse could re-activate the outgoing session in the frame a fresh tab's empty tree took its region, adopt that still-assigned session into the new tab's own pane, and orphan the new session — a "new" tab painting the old content, with clicks on it looking dead because the strip had nothing left to change. Sessions named in a server tree's `pane_trees` are filed as pane sessions before the list rebuilds the strip on reconnect, while sessions in `SessionList` but in no tree remain ordinary tabs. The invariant is: **a pane split never adds a strip tab; tabs own pane trees**.

The exit path is region-scoped end to end. [[crates/scribe-client/src/tab_session.rs#TabSessions#remove]] refocuses onto the nearest surviving tab of the exited tab's own region and stops there; a region that runs out of tabs is dropped rather than handing its selection to a strip neighbour. The same region-local departure clamp handles an optimistic `MoveSession` re-file and an authoritative `SessionList` re-file, including an atomic tab-subtree transfer: its source retains the shown session by identity, or when that session departs keeps its successor at the removed slot (the predecessor at the end), rather than resetting to tab 0. Pane sessions are re-filed from the authoritative tree before that list is folded in, so the tab root and every split child converge together without any `MoveSession` sequence. The optimistic split-seed move still appends and selects the tab in its target region. The reader changes the window-global `active_session` only when that exited session was the attached one: it attaches the same-region refocus target, or clears the pointer when there is none. Either way the exit bumps the redraw generation itself: a background tab's exit reaches neither the attach nor the status path that used to bump it, so its tab strip repainted only on the next unrelated tick. [[crates/scribe-client/src/pane_shell.rs#PaneShell#retain_sessions]] keeps a region's emptied tree alive when its workspace still holds tabs, so a hidden tab is never orphaned by its shown sibling's exit; only a tabless region collapses and is closed on the server. After such a collapse the reconcile pass re-points an empty or tabless `active_session` to the shell's surviving focused pane before adoption. `sync_tabs` then swaps the region onto the tab the strip refocused, and [[crates/scribe-client/src/main.rs#TerminalView#fill_empty_region_panes]] hands each still-empty pane *its own tab's* session — tab-scoped, so a refill can never paint one tab's session into another tab's pane — streaming it via attach/subscribe without touching the active session ([[crates/scribe-client/src/main.rs#TerminalView#stream_session]]) so an unfocused region repopulates without stealing the keyboard.
...
Every one of those mutations ends in , which sends  with the tree  serializes. The topology comes from the  that owns it and the per-region payload from the shell's tab-owned pane trees: `session_ids` contains only strip tabs, and each matching `pane_trees` slot carries that tab's split while its child sessions remain out of the strip. Panes still waiting for a session are pruned rather than serialized under a synthetic id, so a reconnect cannot restore a pane pointing at a session that never existed. Because the report is called from paths that fire far more often than the topology moves, the throttle is an exact equality check against the last reported value — which is sound precisely because that value *is* the wire payload.

A committed same-window tab move is the one later `SessionList` that can change workspace filing without adding a session. [[crates/scribe-client/src/main.rs#park_server_topology]] compares the incoming workspace IDs with the reader's prior registry and parks that tree; [[crates/scribe-client/src/main.rs#TerminalView#adopt_server_topology]] then rebuilds the two regions from the authoritative leaf payload, restores tree order and each `active_tab_index`, and re-focuses the pane named by the still-live `active_session`. This is the same adoption path reconnect and handoff use, so the live commit and both recovery paths cannot drift.
...
Typing is only the input-driven half of that invariant. Selection ranges are absolute grid lines read straight off the live `Term` on every access, so [[crates/scribe-client/src/terminal.rs#DisplayOnlyTerminal#advance_output]] also drops the selection whenever a committed frame actually changes visible state — a full-screen redraw such as a TUI's collapse control, streaming output, `clear`, a vim redraw — because nothing else notices when a redraw overwrites the cells a stale range still points at; left alone, the highlight keeps painting whatever the pane draws over those rows next, and `selection_text()` keeps handing that new text to `copy_on_select` and the `copy` chord instead of what was actually highlighted. [[crates/scribe-client/src/terminal.rs#DisplayOnlyTerminal#begin_selection]] sets a `selection_dragging` guard so a repaint that lands mid-gesture cannot erase the selection a live drag is still building, and [[crates/scribe-client/src/main.rs#TerminalView#release_over_grid]] clears that guard unconditionally, before its own first-refusal chain, because a Shift-drag over a mouse-tracking application can end with the release itself forwarded to the application rather than reaching `finish_selection`. [[crates/scribe-client/src/terminal.rs#DisplayOnlyTerminal#trim_history]] clears the selection outright, rather than shifting it, whenever a trim actually drops rows: alacritty's `Line` addressing has no notion of a row that no longer exists, so a selection anchored above the new history floor would otherwise index straight past the ring buffer's surviving length on the next read.
...
The band's clusters render as "zones": the left context group and each right-side cluster (repository/session metadata, then system stats, then the clock) keep their own spacing container but paint no fill — every cluster sits flat on the band's base background — split at `Span::zone_break` boundaries the model emits in place of the legacy " │ " inter-cluster hairlines, while segments inside one zone separate with a muted middot. Each stat is a chip of `label graph readout`: a semibold label tinted by the stat's identity hue from the theme's ANSI palette (CPU blue, MEM magenta, NET cyan, GPU green), a painted graph, and a readout right-aligned in fixed cells. The graphs are quads, not glyphs. The model emits them as [[crates/scribe-client/src/status_bar.rs#SpanKind]]`::Graph` spans carrying a `Bar` per sample (a `0..=1` level and a colour), and [[crates/scribe-client/src/status_bar.rs#graph_row]] paints each as fixed-width bars rising from a hairline baseline in the hue. The block-glyph run (`▁▂▃`) it replaces was a few pixels of smear at the baseline, shaped by whichever fallback face supplied the glyphs. MEM is a `SpanKind::Gauge` instead: a one-sample bar graph was noise, while a horizontal gauge reads as the fraction it is. A short history left-pads with idle stubs in the dim label colour, so a graph keeps its fixed width from the first sample and a missing sample never reads as a real zero. Bars and readouts band by load: the hue (bars at 72% alpha, so a calm graph sits behind its readout) below 70%, solid ANSI yellow from 70%, ANSI red from 90% — the same warn/danger split the AI context bands default to — so a calm graph stays recognisably its own while a spike in any graph flares in the shared hot colors. Network rates are never banded: there is no "too much" bandwidth. The zone's width is stable by construction (fixed bar counts at any given window width, fixed readout cells in the terminal's monospace font), which retired the earlier worst-case-text pin and single-`StyledText` paint the glyph run had needed. The stable-width metadata cluster sits to the stats zone's left. An actionable update CTA renders as an accent-tinted rounded chipAn actionable update CTA renders as an accent-tinted rounded chip (15% alpha fill, deeper on hover) instead of bare text; progress labels stay inert plain text. The balance button and settings gear share one trailing flat cluster of their own while keeping full-band-height hit targets inside it. [[crates/scribe-client/src/status_bar.rs#StatusBarMetrics]] holds one reference geometry for the 36px default band: 14px text, 12px semibold labels, 16px readouts, 5px bars with 2px gaps in a 22px box, 36px between chips, 20px between zones, 14px edge, 28px controls. A graph's full LENGTH is set by its sample count, never by the band: CPU and GPU carry `CPU_SPARK_WIDTH` (16) bars and each network direction `NET_SPARK_WIDTH` (8), which must match the sampler's history caps; only a narrow window shortens them (below). Every band tall enough to hold the chip renders it at exactly those sizes; only a band shorter than `MIN_FIT_HEIGHT` (24px, the graph box plus its baseline rule under the top border) scales it down, and a band taller than the reference grows it so the extra height is filled rather than padded. Scaling the type with the band instead — the rule this replaced — meant the design existed at exactly one height and every other band, including the common 28px one, got a miniature of it with 11px text and 17px graphs. The trailing controls take that fixed `control` width rather than their glyph advance, so their hit rects are derivable: the E2E scripts that click them (`tests/e2e/visual/settings-entry.sh`, `window-chrome-bands.sh`) compute the offsets from the same numbers, and a unit test pins those two click points inside the rects at both the 8px floor and the default. The band also shapes in the terminal's own font family rather than the generic `monospace`: cosmic-text has no face by that name, so GPUI fell through to its sans-serif fallback stack and painted the bar proportionally with hairline spaces. [[crates/scribe-client/src/status_bar.rs#render]] takes the family and the metrics through `StatusBarGeometry`, and [[crates/scribe-client/src/status_bar.rs#measure_left_budget_cols]] measures the right group — every span at its own size, plus the graphs' painted widths and the zone spacing — in that same face. The band is `whitespace_nowrap`, so nothing folds onto a second line; a squeezed window degrades the right group instead of clipping it. Every render, [[crates/scribe-client/src/status_bar.rs#fit_right_group]] measures the left group, the centred CTA and the trailing controls in the band's own face and hands the right group the width that remains, and [[crates/scribe-client/src/status_bar.rs#fit_right]] fits it in two stages: the longest graphs lose their oldest bar together (CPU and GPU come down to the network graphs' length, then all four shorten in step) until every graph is at the `MIN_SPARK_WIDTH` four-bar floor, and only then do sections drop from the right, one chip or metadata segment at a time (GPU, network, MEM, CPU, then host, session count, branch), until the group fits. Graphs stay at the floor while sections are dropped rather than re-growing into the freed space, so a resize never bounces them. The left group is the only element that yields after that (it is `min_w_0` and clips at its own edge), and the controls cluster is `flex_none`, so the balance button and gear keep their corner at any width. The fitted right group carries over an OSC 8 hover rebuild, and [[crates/scribe-client/src/status_bar.rs#measure_left_budget_cols]] budgets the hover URI against it. The normative mock is `.impeccable/mocks/status-bar-stats.html`, drawn at the real band size in the ayu chrome: the glyph run it replaces, then the shipped chip layout idle and under load.
...
See [[client#Client#URL Detection#OSC 8 Hover Preview]] for the OSC 8 hover preview behavior.
...
[[crates/scribe-client/src/dialog.rs#AgentConsentDialog]] shows the capability, target, and caret-escaped caller label marked “caller-supplied,” plus the warning that Scribe cannot verify the caller. The server-resolved `AgentPromptContext` supplies the rest: a “Requested by” line naming the origin session's title, cwd and task label, and a target line carrying the resolved description instead of the opaque session id. An absent context — an older server, or a caller with no live origin — renders “unknown session” and falls back to the raw target. Deny once is the default focus; Escape and backdrop dismissal deliberately send `DenyOnce`. The four buttons reuse `ClipboardDecision`, so the wire carries allow/deny once and always without a second decision enum.

[[crates/scribe-client/src/main.rs#AgentPromptSlot]] is the reader/foreground handoff: the reader parks the request, the foreground publishes the id it is displaying, and a `ServerMessage::AgentPromptDismiss` reaches either side. A withdrawal drops a still-parked prompt outright and flags a displayed one, which [[crates/scribe-client/src/main.rs#TerminalView#close_withdrawn_agent_prompt]] closes on the next lifecycle tick without sending any decision. Dismissals are broadcast, so an id this window never showed is ignored.
...
The one exception is feedback with no other surface at all: [[crates/scribe-client/src/main.rs#TerminalView#push_toast]] stacks up to three transient chrome-toned chips at the grid band's bottom-right, and [[crates/scribe-client/src/main.rs#TerminalView#copy_selection]] is its only emitter — nothing else in the window changes when text lands on the clipboard, so "Copied N lines" is the copy chord's sole acknowledgment. A per-toast timer owns removal (motion preferences may change how a toast looks, never whether it leaves), each timer removes only its own id so a newer toast cannot be taken down by an older one's expiry, and one status node announces the newest message while the chips stay decorative. Arrival is the only animated half: a chip fades and slides in over one capped [[crates/scribe-client/src/animation.rs#AnimationSettings#transition]], renders at its end state under reduced motion, and leaves instantly either way. The bar's own surfaces — connection, update, command status — stay where they are; the toast stack must not become a second status bar.
...
[[crates/scribe-client/src/window_chrome.rs#tab_bar_height]] is the one effective tab-row formula: `appearance.tab_height + appearance.tab_bar_padding`; [[crates/scribe-client/src/window_chrome.rs#default_window_size]] adds it and `appearance.status_bar_height` to the grid's own extent at the metrics it is *painted* with — the live `GridFont`, not the integer cell size reported to the server, because the painted metrics decide where the last row lands.  resolves font and both chrome metrics from live `[appearance]` config and hands the result to `Bounds::centered`. At shipped defaults the 28 + 8 tab row and 24px status band make the window 1008x741 rather than 960x680.
...
and paints the same failure notice as a server result. Card-drag paint and its
guarded write queue both derive A2-BD6's native-lane verb matrix from
[[crates/scribe-client/src/beads_board.rs#card_drop_verb]], so a painted accepted
target always has a writable verb.
...
way the tab does. That drawer `on_hover` also reports `HoverSource::Control`
for the board itself: the drawer `occlude`s, and GPUI's `BlockMouse` makes
every hitbox behind an occluding one report `is_hovered() == false`, so a
hovered board would otherwise read the pointer entering its own drawer as the
pointer leaving the board and take the drawer down mid-transfer. `Control` is
the source for exactly that -- an element inside the board that takes hover
away from it -- where `Board` would be cleared again by `board_shell`'s own
leave in the same pointer move. Beside it sits
...
Already-admitted is the load-bearing word. [[server#Server#Beads Flow source cache#Flow graph admission]] refuses a cycle, a disconnected member, an external blocker, and an over-bound epic before the wire, and [[protocol#Client Messages#Beads epic graph]] carries that decision as a typed outcome rather than an empty option. So the client has exactly two cases — a graph it can lay out, or Lanes — and this module carries no degenerate-shape handling. Its geometry is pinned against the normative mock by [[test#Test Harness#GPUI Client Headless Suites#Beads Flow visual contract]] and its pure logic by [[test#Test Harness#GPUI Client Headless Suites#Flow layout and paint-path guard]].

[[crates/scribe-client/src/beads_flow.rs#IndexedGraph#new]] validates the bounded graph and [[crates/scribe-client/src/beads_flow.rs#longest_path_ranks_indexed]] then assigns every issue its maximum blocker distance from a root. Unknown endpoints, duplicate ids, empty or over-bound graphs, and an in-memory cycle fail as [[crates/scribe-client/src/beads_flow.rs#FlowLayoutError]] instead of leaking malformed geometry or looping. `bd` itself refuses cycles, but this boundary remains defensive because the ranking algorithm requires acyclicity.

[[crates/scribe-client/src/beads_flow.rs#ExpandedGraph#add_edge]] inserts one virtual node in every intermediate rank crossed by a skip edge. [[crates/scribe-client/src/beads_flow.rs#ExpandedGraph#barycenter_pass]] then makes one downward pass, ordering each rank by the mean predecessor position with stable input order as the tie break. [[crates/scribe-client/src/beads_flow.rs#layout_flow]] places only real nodes after that ordering; dummies remain internal to rank ordering and never enter the exported layout.
...
[[crates/scribe-client/src/beads_flow.rs#render]] lowers an admitted graph and its layout into the compact 197px strip: the 34px Flow band, rank ruler, 214×24 nodes, orthogonal one-pixel wire segments, clipped-edge fades, and a horizontal position bar. The strip's own floor (with its resize grip) is painted once by the enclosing [[crates/scribe-client/src/beads_board.rs#render]], not duplicated inside the Flow renderer -- both lanes and Flow share that one floor, at the bottom of the same board rect, so a second one behind it would only ever repaint hidden pixels. It reads only named [[crates/scribe-client/src/beads_board.rs#BeadsBoardColors]] slots, so every Flow surface follows the live theme. The strip carries no origin of its own: it fills the slot inside [[crates/scribe-client/src/beads_board.rs#board_shell]], which has already positioned itself absolutely over its region, so `FlowRender` takes the viewport *width* it clamps the scroll and position bar against and never a rect. A renderer that positioned itself at the board's own `rect.x`/`rect.y` applied that offset a second time -- a no-op in the origin region, where both are zero, and in any other region it painted the whole strip outside the board's own clip, so that region kept showing A2 while its graph sat in the store. `FlowRender` takes the cursor, horizontal offset, trace classes, workspace-owned focus/action controls, and the band's own `FlowBandControl` as explicit seams: mode entry and exit, scrolling, panel retargeting, trace selection, and liveness remain outside the renderer. A missing node control is a typed [[crates/scribe-client/src/beads_flow.rs#FlowRenderError]], preventing a painted node from silently losing its AccessKit button role, Tab stop, Enter/Space activation, or blockers/dependents description. [[crates/scribe-client/src/main.rs#flow_node_control]] marks each stable control handle as a tab stop, and the terminal focus repair recognises a focused node so the next repaint cannot return it to the PTY. The epic chevron paints as inert text with no id, role, or handler -- by construction unreachable to a pointer, the keyboard, or AccessKit, since GPUI never builds an accessibility node for an element with no [[crates/scribe-client/src/beads_flow.rs#render|`.id()`]]. The band's own `← LANES`, the mode pair's `LANES`, and every graph node are real Buttons; the mode pair's `FLOW` member is a `Role::Tab` with `aria_selected(true)` and no handler of its own, a selected-state indicator rather than a control.
...
[[crates/scribe-client/src/beads_board.rs#PendingFlow]] is the fence that makes a late reply safe. It holds the epic and the cursor as *latest intent* rather than per request, and its *presence* is the fence: mode exit, workspace loss, `NotDetected`, and a reconnect that withdrew the capability each drop the entry, so a reply that outlived its request finds nothing to match and cannot re-enter Flow. A reply naming a different epic is refused by the `epic_id` check. Two clicks on one epic therefore collapse to the second: [[crates/scribe-common/src/protocol.rs#BeadsEpicGraph]] carries graph content and no cursor, so honouring whichever reply arrives first against the newest intent lands on the card the reader actually asked for, and its twin is spent against the same fence. Neither a generation counter nor a request id on the wire would add anything: two replies for one epic cannot differ while the graph is frozen, and an earlier monotonic generation was removed once it proved to be compared against nothing.

[[crates/scribe-client/src/beads_board.rs#BeadsBoards#apply_epic_graph]] lays the graph out once and stores it in [[crates/scribe-client/src/beads_board.rs#FlowView]]. Board polling continues underneath and never touches it — a strip that re-ranked itself under the pointer would move the node a click was travelling towards. Only a text-scale change re-lays it out, through `relayout_flows`, and a graph that no longer fits the row budget returns to lanes rather than clipping nodes out of sight. Successful opens also append their workspace to an open-order queue, moving an already-open workspace to the tail; cleanup follows exit, region loss, capability loss, and relayout drops.

Escape reaches [[crates/scribe-client/src/beads_board.rs#BeadsBoards#exit_latest_flow]] only after the detail panel has declined the key, so a focused panel always dismisses before the strip changes mode. It exits the queue's tail — the genuinely most recently opened Flow, not the numerically largest workspace UUID — and Escape never strands a reader in lanes with the panel they were reading still open. The band's own `← LANES` and the mode pair's `LANES` reach the identical `exit_flow` call through [[crates/scribe-client/src/main.rs#flow_band_control]]'s one `on_exit` callback, each behind its own stable `FocusHandle` -- [[crates/scribe-client/src/main.rs#TerminalView#flow_band_for]] hands out the cached pair while a strip is in Flow and a throwaway pair otherwise, since one `BeadsBoardRender` shape serves lanes and Flow alike but only Flow ever mounts it. [[crates/scribe-client/src/beads_flow.rs#flow_exit_control]] builds both controls, preserving their separate ids, labels, and focus handles. Pointer and Enter/Space both reach it: GPUI turns a focused, `on_click`-bearing element's Enter/Space key-up into a synthesized click by itself, so the control's own `on_key_down` only has to stop the keystroke from reaching the PTY, not call the callback a second time. The wheel is claimed by the Flow strip alone: in lanes the same gesture belongs to the lane bodies underneath. Either wheel axis drives the one axis Flow has, clamped to the graph, because a rank too wide for the row budget fails layout instead of growing a vertical scrollbar. Claiming and clamping are separate: the strip stops propagation for every wheel it handles and only refreshes when the offset actually moved, so wheeling at either end of a graph cannot fall through to the pane behind the board -- the same rule scribe-uu2y set for a swallowed press's release.

The clamped offset also decides the edge fades and the position bar: [[crates/scribe-client/src/beads_flow.rs#edge_fades]] paints the left fade only once `scroll_x` has moved off zero and the right fade only while `scroll_x` is still short of the same `max_scroll` the scrollbar clamps to, so a graph that fits the strip shows neither, one scrolled to the origin shows only the right fade, one scrolled to its end shows only the left, and everywhere between shows both. Both fades render as a `linear_gradient` from the board's own ground colour to that ground at zero alpha, never a literal mock colour, so they move with the theme like every other Flow surface.

Tab and Shift+Tab auto-scroll the landed node into view (A3-I6). [[crates/scribe-client/src/beads_board.rs#BeadsBoards#scroll_flow_node_into_view]] moves the offset only as far as the clip needs -- flush with the node's left edge if it entered clipped on the left, flush with its right edge if clipped on the right, untouched if it was already fully visible -- rather than centring or homing to either end. [[crates/scribe-client/src/main.rs#TerminalView#sync_flow_focus]] is what calls it: the same per-frame focus poll that feeds the hover trace also tracks, per workspace, which node held focus last frame, and calls the scroll only on the rising edge into a *different* node. Without that edge check every frame would re-home the view while a node already held focus, fighting any wheel scroll the reader took since. A pointer click reaching this same path is harmless, not special-cased away: a clicked node is by construction already on screen, so the clip check finds nothing to do.
...
Hover and keyboard focus raise the identical trace: Tab can land a Flow node's focus on a different node than the one the pointer still rests on, so [[crates/scribe-client/src/beads_board.rs#BeadsBoards#set_flow_hover]] takes a `FlowHoverSource` (`Pointer` or `Focus`) alongside the issue id, the same per-source bitmask [[crates/scribe-client/src/beads_board.rs#BeadsBoards#hover_lane]] already uses for a collapsed lane's drawer. Entering a *different* node than the one currently tracked replaces it outright -- only one node is ever traced -- but entering or leaving a source on the node already tracked merges into its existing bitmask instead, so a pointer leaving while focus still holds the same node (or the reverse) cannot erase a trace the other source still wants open. [[crates/scribe-client/src/main.rs#TerminalView#sync_flow_focus]] is the poll: GPUI has no per-element focus-in/out callback, only `Window::on_focus_in`/`on_focus_out` subscriptions a free-function board renderer cannot register, so every visible Flow node's `is_focused(window)` is read once per frame, the same reason [[crates/scribe-client/src/main.rs#TerminalView#sync_lane_tab_focus]] polls the collapsed-lane tabs' keyboard focus instead of subscribing.

`set_flow_hover` still returns whether anything changed, but only `hovered_issue_id` -- the field the renderer actually reads -- decides that answer; a source joining or leaving an already-traced node moves the bitmask without moving `hovered_issue_id`, so it schedules no repaint, the same way re-entering an already-traced node from one source repaints nothing. A leave is honoured only for the node that owns the current trace: pointers cross borders in an arbitrary order, so an unfiltered leave from the node just departed would erase the trace the newly entered node had already set. Because the whole trace is one value behind one refresh, entering and leaving each restore every node and wire in a single frame rather than easing elements back independently — there is no per-element animation to interrupt, so a reduced-motion setting changes nothing about the final frame.
...
Tabs wear a browser-style raised card: an active, hovered, or flashing tab's fill is an absolute child inset `TAB_CARD_TOP_GAP` (5px) from the strip's top edge ([[crates/scribe-client/src/titlebar.rs#TitlebarView#tab_card]]), rounded at its top corners and running to the band's bottom edge, while resting tabs paint no fill and sit flat on the strip. No hairline closes the strip off — the ground-tone band meets the work area directly — and because the active card wears `tab_bar_active_bg` (the pane fill), the active tab reads as opening into the content below it, the way a browser tab meets its toolbar. The strip paints the window-ground tone itself: `tab_bar_bg` derives through [[crates/scribe-common/src/theme.rs#ground_tone]], the same function behind [[crates/scribe-client/src/main.rs#pane_ground]], so the titlebar band and the pane gaps are one surface and the active card — wearing the pane fill — reads as a pane against both, the layered chrome mock's two-surface model (`--ground` under `--pane`). Active splits into two tiers ([[crates/scribe-client/src/tab_bar.rs#TabShown]]): every region's shown tab keeps the raised card — its open-tab marker — but only the window-focused region's shown tab (`Focused`, promoted in [[crates/scribe-client/src/main.rs#TerminalView#partition_tab_strip]] from the shell's focused workspace) wears the mock's mark: a 2px tick riding the card's top edge, inset 8px from each side, in the workspace's effective accent — quiet gray until a project names the workspace — plus the bright title and standing close button. Other regions' shown tabs go quiet (dim title, hover-only close), so the tick answers "where is my keyboard" while the card answers "which tab is open here".
...
In a multi-workspace window the titlebar hosts tabs only for regions on the window's top row. [[crates/scribe-client/src/main.rs#TerminalView#partition_tab_strip]] splits the decorated strip: tabs of a region stacked below go to that region's own in-region bar (below), everything else stays in the titlebar, and each titlebar position keeps the [[crates/scribe-client/src/tab_session.rs#TabAddress]] of the tab it rendered so Select/Close/Reorder events resolve back to a region and a position inside it ([[crates/scribe-client/src/main.rs#TerminalView#titlebar_slot]]). [[crates/scribe-client/src/main.rs#TerminalView#apply_group_badges]] then marks every titlebar run with `TabData::group_region_x` and `TabData::group_accent`. A region's tabs are contiguous because a region owns its tab list, so no regrouping pass runs before a paint and opening a tab in an earlier region cannot produce a repeated region edge. Top-row regions' left edges are distinct by construction, so titlebar groups always absolutely align over their regions — the old duplicate-edge flowed fallback is gone with the stacked tabs that caused it. No group hairline closes a run off either: region identity lives in the pill dot, the focused tab's tick, and the pane focus ring — and every region tint (those marks, workspace drag chrome, and the CI bar) resolves through [[crates/scribe-client/src/main.rs#TerminalView#region_accent]]: a named workspace takes the badge's own name-hashed palette color while an unnamed one takes [[crates/scribe-client/src/main.rs#TerminalView#muted_workspace_accent]] — the tab-separator hue lifted to a clearly visible 0.5 alpha, because at the separator's own 0.18 the focus ring all but vanished. Region chrome (including the focused tab's tick and the pill dot) therefore always wears exactly the workspace pill's color: quiet gray until a project is entered, re-tinting the moment a Beads project names the workspace. Every top-row region in a multi-workspace window receives a clickable [[crates/scribe-client/src/tab_bar.rs#GroupBadge]]: named regions keep their configured-palette label and accent, while missing or cleared names use the neutral `workspace` label with the muted tab-separator accent. Both badge forms render as a rounded chip sharing the tab card's vertical rhythm — inset `TAB_CARD_TOP_GAP` from the strip's top and flush to the band's bottom, 6px horizontal padding, 6px radius — with a rounded hover layer on the label, so pills and tab cards sit on one line. Every pill wears one neutral chip fill (the theme foreground at 5% alpha, the layered chrome mock's `--fill`, [[crates/scribe-client/src/tab_bar.rs#pill_fill]]) and its workspace color lives in a leading 7px square identity dot ([[crates/scribe-client/src/titlebar.rs#workspace_pill_dot]]); a detected Beads board swaps the dot for the clickable board icon drawn in that same workspace accent. A region with no tabs renders that same badge independently at its region edge, so it remains reachable while its seed session is in flight; its click focuses the retained empty pane. The label half of both badge forms is also a `WorkspaceDragMarker` source with an empty native ghost and grab cursor; its press still stops propagation, while the nested Beads icon keeps separate handlers and never registers the workspace marker. The rightmost group extends to the window edge, crowded tabs shrink to `TAB_MIN_WIDTH`, and a badge beside tabs selects its group's first tab. Single-workspace chrome is unchanged.
...
A region below the top row carries the legacy client's per-workspace tab bar instead: [[crates/scribe-client/src/pane_shell.rs#PaneShell#region_bar_rects]] reserves the same live resolved tab-row height at the region's top (every pane-geometry consumer receives that value and shrinks through the same content-rect rule, so panes, dividers and hit-tests agree), and [[crates/scribe-client/src/main.rs#TerminalView#render_region_tab_bars]] paints the partition's [[crates/scribe-client/src/main.rs#RegionBarData]] as the same named-or-neutral pill followed by tabs with the titlebar's content chrome (flash blend, AI dot, context suffix) minus keyboard tab stops; the lower bars keep the flat full-bleed fill rather than the titlebar's raised card and its accent tick, so their active fill alone is the mark against the lifted strip ground. A bar tab draws its right-hand separator only against a neighbour in its own bar; the last tab paints none, so no divider abuts the next region's workspace pill across a region boundary. The lower pill carries the titlebar badges' neutral chip, identity dot, and label hover layer, so upper and lower workspace pills read as one component; like the titlebar strip, the bar draws no bottom hairline. The lower pill label registers the same dedicated workspace drag marker as the titlebar labels and keeps its Beads child outside that grab surface. Empty lower regions keep their pill and focus their retained pane on click. Both tab renderers use the same compact `text_xs`, full-height, vertically centered row. `tests/e2e/visual/titlebar.sh` measures the real glyph box in both default 36 px bars, while `tests/e2e/visual/window-chrome-bands.sh` measures 16, 60, and 80 px rows through live reload. A bar tab is "active" when its session is the one the region shows ([[crates/scribe-client/src/pane_shell.rs#PaneShell#region_shown_session]]), independent of window focus — but only the focused region's shown tab takes the `Focused` tier's bright title and standing close button, in both bars; clicking selects by session id, and the bar-set shape is logged on change as the scripted oracle (`tests/e2e/visual/workspace-split.sh`).
...
### Strip shares the window ground

Verifies `TabBarColors` passes the theme's `tab_bar_bg` token through unchanged and that the token equals `ground_tone(theme.background)`, while the active card keeps the raw `tab_bar_active_bg` pane fill.

That identity is the mock's two-surface model: the strip and the pane gaps are one ground, and the active tab is a pane surface — there is no third strip-only color.
...
### Multi-workspace pills cover unnamed and zero-tab regions

Verifies pill data preserves named labels and configured accents, gives unnamed multi-region workspaces a neutral label and muted accent, and retains standalone top and lower-bar pills without tabs. Single-workspace unnamed chrome remains pill-free.
...
The GPUI client renders server-owned GitHub Actions state inside each matching workspace region. A collapsed band starts at 40px and grows when its workflow cells wrap.
...
### Cells wrap onto extra rows

The collapsed cell cluster is a grid, not a single flex row: a run with more workflows than its region is wide grows the band by 28px per extra row instead of overlapping names and tracks.

[[crates/scribe-client/src/ci_bar.rs#measure_layout]] shapes labels in the configured terminal font at their rendered sizes, rather than requesting a generic `monospace` face. It subtracts band padding, gaps, state and metadata from the region width, including either the owner's open/dismiss controls or the viewer's read-only chip. The branch label truncates at 160px; its SHA and elapsed suffix remain separate.

[[crates/scribe-client/src/ci_bar.rs#CiBandLayout#fit]] uses the longest visible cell name plus an 11px live-mark allowance to choose a preferred slot width, clamped between 72px and 200px. That width determines how many columns fit, not a maximum rendered column width. Cells then share equal-width grid columns across the fewest rows needed, balancing six cells as three and three when four columns would fit. Names truncate within their columns. [[crates/scribe-client/src/ci_bar.rs#CiBarModel#slots]] counts the `+N more` counter as its own slot; zero available width still yields one column, and an empty model retains one row.

[[crates/scribe-client/src/main.rs#TerminalView#sync_ci_run_strips]] builds and measures each visible band once per frame against its region rect, storing both in [[crates/scribe-client/src/main.rs#CiBandFrame]]. Reservation and [[crates/scribe-client/src/main.rs#TerminalView#render_ci_run_bars]] consume that same frame, so they agree on row count as region widths or labels change. [[crates/scribe-client/src/ci_bar.rs#CiBandLayout#height]] yields 40px for one row plus 28px for each additional row.
...
[[crates/scribe-client/src/main.rs#TerminalView#sync_ci_run_strips]] reserves the sum of each region's measured collapsed heights and any expanded panels, capped to preserve three terminal text rows. [[crates/scribe-client/src/pane_shell.rs#PaneShell#ci_band_rect]] clips each band's offset and height to that reservation and yields no rect once the strip is spent. Painting and pane geometry use the same clipped area. Width changes that alter wrapping therefore update the owning region's PTY sizes without reserving space in neighboring regions.

The band sits above a pinned Beads board when both are visible. Its bottom hairline uses the owning workspace accent at 28% alpha — a quiet ownership mark rather than a highlighted edge — except failure and stale replace it with their semantic theme colors at full strength.

### Expanded job trace

Pointer, Enter, or Space toggles an accessible trace panel below the collapsed
band's measured grid, whether its cells occupy one row or several.
...
[[crates/scribe-client/src/main.rs#ci_trace_model]] builds detail only for an
expanded band. For stale runs it uses the latest workflow update timestamp,
falling back to the current time only when no workflow supplies one.
...
reserves the measured collapsed height plus panel inside only the owning region.
Closing the panel returns its space while retaining the wrapped collapsed band.
...
Driving [[crates/scribe-client/src/main.rs#AiChrome#apply_state_change]] with the edges one real turn emits leaves the label live while `Processing`, frozen at the prompt-to-finish figure once the AI stops, unmoved by the further idle edges an idle provider keeps sending, ticking again on the return to `Processing`, and still ticking through a `WaitingForBackground` edge. The assertions read the label off [[crates/scribe-client/src/prompt_bar.rs#build_model]] through [[crates/scribe-client/src/main.rs#AiChrome#visible_prompts]] — the pair the render pass itself uses — so a stamp that never reaches the strip fails the test.
...
Find-in-scrollback is the client surface that is nothing but a server round trip: the client is display-only, so the scrollback being searched lives on the server and every settled query travels as `SearchRequest` and returns as `SearchResults`.
 ...
 is the overlay itself, a port of the winit  with its quad painter replaced by GPUI elements. It mounts inside the focused pane's grid slot ([[crates/scribe-client/src/main.rs#TerminalView#compose_pane_content]]) rather than the window root, and anchors a box to that pane's own top-right corner — the pane whose scrollback the query actually searches — as a single row: a `/query` field, a fixed-width inline `n/m` counter, and the three pointer controls (scribe-1mpq). It owns no session and no sink, so an edit — , ,  — clears the stale matches and schedules a `::QueryChanged` rather than searching anything. Clearing before the reply lands is deliberate: the grid must never contradict the query field, even for one frame.
...
The query is a real single-line editor model: [[crates/scribe-client/src/search.rs#SingleLineEditor]] owns UTF-8 text, a grapheme-boundary caret byte index, and a normalized selection range. Typed and pasted text replaces the selection; Left/Right and Shift extend by grapheme, Home/End move to line edges, and Ctrl or Option movement/deletion uses `UnicodeSegmentation::unicode_word_indices` for Unicode-safe word starts across punctuation and underscores; Backspace/Delete never split a combining sequence or emoji family. [[crates/scribe-client/src/search.rs#find_editor_visual]] lowers that model onto a `StyledText` selection wash and a measured 2px caret. The caret blink belongs to the overlay entity itself, uses [[crates/scribe-client/src/search.rs#CURSOR_BLINK_INTERVAL|530 ms]], follows live `appearance.cursor_blink` reloads, and stops when the overlay entity is dropped.

While the overlay is up it owns the keyboard. [[crates/scribe-client/src/search.rs#find_input_action]] resolves Escape, Enter / Shift+Enter and Up/Down match cycling before the GUI edit table, then covers select-all, copy, cut, standard paste, configured copy/paste bindings, Shift+Insert, word and line motion, word deletion, and printable replacement. [[crates/scribe-client/src/main.rs#TerminalView#handle_find_overlay_key]] performs clipboard reads/writes directly against the local editor; find paste never enters the PTY paste-confirmation gate and every key remains consumed, so no edit chord or pasted byte can leak to the pane. Cycling the current match emits `FindOverlayEvent::MatchCycled` before it notifies the overlay, so keyboard and pointer controls reach one shell-side path. `TerminalView::scroll_focused_pane_to_current_find_match` reads the selected grid row and uses the pure `scroll_delta_to_current_match` helper to centre an off-viewport hit, then hands that `Scroll::Delta` to `scroll_session`; find therefore gets the same pane snapshot, split-scroll, scrollbar, and repaint bookkeeping as every other terminal scroll. The projection rebases the server's live-screen-relative rows by the display offset before calling `visible_highlights`, whose viewport-only contract remains unchanged, so the new viewport paints the current match rather than an unrelated live-screen row. GPUI's overlay notify still dirties the shell through the ancestor chain, so no second shell-level `cx.notify()` is needed.

That router ownership does not add focus state. [[crates/scribe-client/src/search.rs#FindOverlayColors]] always takes the theme's active tab text for typed input, muted tab text for the empty placeholder, and an opaque theme-accent 2px border; the active panel retains the [[crates/scribe-client/src/search.rs#FIND_OVERLAY_BG_ALPHA|98%]] opacity floor so it reads as the keyboard-owning surface rather than dim chrome.

The key table was the only way in until scribe-1mpq added three accessible pointer controls — previous, next, close — calling the same [[crates/scribe-client/src/search.rs#FindOverlayView#prev_match]] / [[crates/scribe-client/src/search.rs#FindOverlayView#next_match]] / [[crates/scribe-client/src/search.rs#FindOverlayView#dismiss]] the key table already drove, so a pointer-only user can cycle matches and close the overlay without knowing a single binding. Each control keeps `Role::Button`, `aria_label`, and `aria_description`, but [[crates/scribe-client/src/search.rs#find_control|find_control]] deliberately does not copy [[crates/scribe-client/src/ci_bar.rs#action_button|ci_bar::action_button]]'s `track_focus`, `focus_visible`, or Enter/Space `on_key_down`: scribe-1mpq shipped all three as unreachable dead code (click never focused the control, it was never a tab stop, and the key table consumes every key including Tab while find is up), so the row advertised a focus ring and a keyboard path nobody could reach. scribe-2yw1 replaced that with the pointer-affordance-plus-tooltip contract Zed's own find bar uses (`search/src/search_bar.rs:40-67` at this repo's pinned rev): a click focuses the overlay's own `focus_handle` — the query-field equivalent — instead of the button, and a `.tooltip()` per control names the keystroke that does the same thing ("Next match - Enter", "Previous match - Shift+Enter", "Close - Esc"). AccessKit's `Action::Click`, auto-registered by `on_click` regardless of tab order, still activates every control for assistive tech; WCAG 2.1.1's own normative note only requires an equivalent keyboard path to exist, not that every pointer target be focusable, and Escape/Enter/Shift+Enter/Up/Down already cover all three actions. Control geometry (size, gaps, padding, counter width) lives in named constants shared by [[crates/scribe-client/src/search.rs#find_control]] and its headless click tests. A narrow pane gives the box a zero minimum width and clips its right-aligned row, so only the query text gives way from the start edge while the fixed counter and three `flex_none` controls stay full-size and clickable. The backdrop's dismiss-on-any-click handler is gone with it: with an explicit close control and Escape both still working, swallowing every left click in the pane was only ever a cost — it turned a click meant to place a text selection under find into an unwanted dismiss instead. The box and each control stop propagation on the matching mouse-up too, not just the down (scribe-uu2y): the overlay paints inside the pane's own `grid_slot`, so an unswallowed release still resolves to a real cell, and [[crates/scribe-client/src/main.rs#TerminalView#forward_mouse_release]] cannot tell a leaked release from one whose press it actually forwarded — a mouse-tracking application would see a button go up that never went down. The rule stays local to the element that ate the press rather than teaching the grid release path to recognise the overlay, which would just be a second place to keep the pairing correct.
...
[[crates/scribe-client/src/context_menu.rs#ContextMenuView]] renders the right-click menu. [[crates/scribe-client/src/context_menu.rs#build_menu_items]] assembles Copy / Paste / Select All, then any detected URL, file, OSC 8 copy-address, and configured smart-selection actions. Copy requires a selection; OSC 8 takes precedence over heuristic URLs.

[[crates/scribe-client/src/main.rs#TerminalView#open_context_menu]] derives additional rows only from the cell under the pointer and matching configured rules. Blank grid without matching actions has exactly the three standard rows. Demo entries and test payloads must never be injected by the app; E2E tests supply their own data through real user inputs.
...
Splitting a pane automatically re-equalizes all ratios in the tree so every pane rebalances to equal space the moment the split lands. Closing one promotes its sibling into the freed extent; ratios stay unchanged on close.
...
Splitting a workspace automatically re-equalizes all workspace ratios through [[crates/scribe-client/src/workspace_layout.rs#WindowLayout#equalize_all_workspace_ratios]] — which weights each split by the leaf count on either side — so every region rebalances to equal space the moment the split lands. Removing a workspace promotes its sibling into the freed extent and preserves the other ratios. Restoration (`split_workspace_with_id`, `from_tree`) preserves reported ratios and never equalizes.
...
GPU-rendered tab bar in  generating  from  using the same glyph atlas as the terminal grid.
 ...
 is derived from `ChromeColors` and holds background, active background, text, separator, gradient-top, and accent color values.  carries per-tab title, active flag, optional AI indicator color, and an optional transient `tab_flash` intensity (a short theme-accent blend over the tab background that self-decays over `TAB_FLASH_SECS` ≈0.45s via the same animation/redraw envelope as the scrollbar fade, additive over active/hover styling and the AI indicator). The strip background is the flat window-ground tone (`tab_bar_bg`, no gradient). Every region's shown tab paints its raised card in the pane fill with rounded top corners; the window-focused region's shown tab additionally wears a 2px tick riding the card's top edge inset 8px from each side, in the workspace's effective accent — quiet gray until a project names the workspace — with the bright title, while other regions' shown tabs keep a dim title and hover-only close. Resting tabs draw a boundary mark only against a resting neighbour — a 13px vertically-centred 1px absolute child rather than a left border, so every mark is the same height and layout never shifts by a border pixel; it drops beside the active or hovered tab, whose own fill marks its edges, and at a group's first tab where the pill is the boundary. A tab's close control collapses to zero width while hidden (it stays mounted for focus stability), so a resting tab spends that room on its title. An AI state dot (from `TabData.ai_indicator`) is rendered in the tab when a session has an active AI state. For provider task-label sessions, the title prefers the last hook-emitted task label while that label is active, then falls back to the normal shell title. Tab titles are truncated to fit the available column width. In multi-workspace mode every region displays a pill: named workspaces use the configured-palette label and accent, while missing or empty names use the neutral `workspace` label and muted tab-separator accent. Empty top-row regions carry the existing `GroupBadge` as a standalone pill; lower bars retain it even with zero tabs. `TabData::group_region_x` independently marks each group's first tab, preserving adjacent group boundaries and absolute alignment with the regions below; `TabData::group_accent` still styles region hairlines and active-tab accent ticks. The pill quad and per-cell backgrounds both span `space + name + space + trailing-gap` so the accent fills the full `badge_columns` allocation up to the next tab boundary — the gap cells are emitted with `pill_bg` rather than the chrome bg, otherwise the cell-bg pass would punch through the underlying quad and leave a visible strip between the badge and the first tab.
...
Off Linux the guard carries an uninhabited field, so [[crates/scribe-client/src/x11_focus.rs#X11FocusGuard]] cannot be constructed at all rather than merely being empty — `from_window_handle` already returns `None` there. That makes the platform-neutral `poll`, `clear_reactivation_debounce`, `should_suppress_key`, and `window_id` discharge `self` by matching a value that cannot exist, instead of silently ignoring it. Keeping `window_id` platform-neutral is also what lets [[crates/scribe-client/src/main.rs#TerminalView#x11_window_id]] stay a single `Option::map` with no `cfg` split: an off-Linux branch that ignored `self` and returned `None` failed the macOS clippy job under `unused_self`.
...
Mouse events are processed for text selection, scrollbar interaction, divider drag, tab drag, workspace-pill drag, prompt bar interactions, and context menus.
...
#### Moving a workspace into an existing window

A workspace can also land in a window that already exists.

[[crates/scribe-client/src/workspace_transfer.rs#WorkspaceMoves]] owns that correlated half of [[protocol#Client Messages#Workspace move]], and it is deliberately thinner than the tear-out coordinator: a move opens no window, and the server answers a commit with one full `SessionList` per surviving window, so success needs no foreground work at all. Only refusals and requests that never reached the wire produce feedback. `WorkspaceMoveEligibility` is resolved against live state at the moment the frame would go out rather than when the affordance was built, so an absent capability or a destination that has closed since is a pure no-op.

[[crates/scribe-client/src/main.rs#TerminalView#workspace_move_siblings]] is the single source of eligible destinations, used by both the palette and the pointer. It enumerates this process's other GPUI windows, keeps the connected ones whose `Welcome` named a window, and reduces each to the region it currently focuses; a duplicate launch focuses the running client rather than starting a second process, so that enumeration is the user's whole window set and needs no round trip that could go stale. Ordering is by window id, not window-map iteration.

The palette is the platform-neutral path. [[crates/scribe-client/src/command_palette.rs#workspace_move_entries]] offers each eligible window exactly two deterministic rows — one insert at the destination region's right edge, one centre swap — and the swap row carries [[crates/scribe-client/src/workspace_drag.rs#swap_hint_text]] as a dimmed explanatory line, which is the same sentence the pointer's centre zone paints. It is part of the row rather than a hover tooltip because a keyboard-driven palette never produces a hover.

Pointer targeting is X11-only. `TerminalView::cross_window_pointer_supported` gates it: X11 publishes every window's mapped, root-relative frame and keeps delivering drag motion past the source surface, while Wayland exposes neither a global cursor nor client window positions and macOS has no sanctioned native proof for the shipped pill, so both keep the palette as their complete cross-window path. Whether the pointer has even left this window is decided from the size GPUI already holds, so an in-window rearrange issues no X11 round trip; only once it is outside does [[crates/scribe-client/src/monitor.rs#mapped_window_bounds]] measure the frame that turns a window-local pointer into a root position. [[crates/scribe-client/src/main.rs#TerminalView#adopt_guest_drop]] then hands that root position to each sibling, which resolves it through the same viewport, divider veto, and [[crates/scribe-client/src/workspace_drag.rs#zone_at]] rule the local drag uses and paints the result with the very same zone element — one preview surface and one `AnimationSettings` for both windows, not a second overlay system. A resolved sibling suppresses tear arming, so the release commits a move instead of detaching.

A centre target explains itself through [[crates/scribe-client/src/workspace_drag.rs#swap_hint]], which returns wording only for `Center` and takes the destination region's usable name or generic wording when it has none. The sentence lives on the zone element itself, and only the fading-*in* live target carries it: an edge transition, cancel, Escape, or blur drops the wording on the frame it happens rather than letting it ride out the highlight's own fade, so no residue survives either path. `tests/e2e/visual/workspace-cross-window.sh` drives both palette rows plus measured X11 edge and centre drops, while `workspace-drag-tearout.sh` retains the same-window preview matrix.

Nothing local moves on release. [[crates/scribe-client/src/main.rs#TerminalView#begin_workspace_move]] sends ids and one operation; a refusal leaves both windows unchanged. [[crates/scribe-client/src/main.rs#ServerTopology#from_session_list]] recognizes a live ownership change when a later list adds sessions or re-files known sessions into different workspaces. [[crates/scribe-client/src/main.rs#park_server_topology]] parks its tree, live session set, and origin before rebuilding the tab strip. Ordinary refreshes and source departures do not overwrite a pending destination layout; the source still collapses through pane retirement.

[[crates/scribe-client/src/main.rs#ServerTopology#should_adopt]] always accepts a parked live ownership change. The reconnect authorship cache cannot veto it: moving out and back can reproduce an older reported tree exactly while the local layout no longer contains that workspace. Ignoring that return left tabs without a rendered region and let a later incomplete report persist the omission. Carrying the origin across the reader/foreground boundary lets [[crates/scribe-client/src/main.rs#TerminalView#adopt_server_topology]] restore the requested edge or swapped slot, active tabs, and pane sessions without mistaking a valid return for an echo. The first list of a connection may retain an authored layout only when all live workspace IDs already have local regions. This also restores a return committed just before its destination lost the stream. Cold-restart replay keeps its own precedence. No IPC schema or capability changes are needed.

When a move empties its source window, the server closes that window after the acknowledged commit. [[crates/scribe-client/src/window_lifecycle.rs#WindowLifecycle#on_window_closed]] accepts a frame naming this window even without a pending close, so an emptied shell cannot remain or create replacement sessions.
...
`focused_pane_size` describes the outgoing tab, and the two are not interchangeable: a per-tab prompt strip removes rows from only the tab that owns it. [[crates/scribe-client/src/main.rs#TerminalView#switch_tab]] therefore assigns and focuses the selected session locally first, then [[crates/scribe-client/src/main.rs#TerminalView#stream_session]] resolves [[crates/scribe-client/src/main.rs#TerminalView#placed_pane_size]] from that session's measured placement through the same [[crates/scribe-client/src/main.rs#TerminalView#painted_pane_rect]] and [[crates/scribe-client/src/main.rs#TerminalView#grid_size_for]] used by normal publication. It reshapes and caches that grid before sending `AttachSessions`; the server's first `SessionReplay` is already authoritative, and the following publish sees the cached size rather than issuing a corrective `Resize` / `RequestSnapshot` round trip.

Hidden-but-open tabs also keep their `pane_sizes` entry: [[crates/scribe-client/src/main.rs#TerminalView#publish_pane_sizes]] retains every session still in the tab strip and only [[crates/scribe-client/src/main.rs#TerminalView#detach_session]] (close or exit) drops one. That memory is the second source `stream_session` falls back on, so a switch back re-attaches at the grid the session already has — the server-side pre-snapshot resize and the follow-up publish are then both no-ops. This is the terminal-standard rule (tmux resizes a pane only when its size actually changed, commit 5864d71); attaching at the outgoing tab's size instead resized the PTY off its real grid and back on every switch, a SIGWINCH storm plus a doubled replay. The reader-thread attach paths obey the same rule: the strip-reconcile [[crates/scribe-client/src/main.rs#attach_session]] and reconnect [[crates/scribe-client/src/main.rs#reattach_visible_sessions]] both announce the session's own display-grid dimensions and use the focused size only for a session this window has never painted.
...
#### Startup announces no speculative grids

A restored window's first frames measure a layout the restore is about to replace, so nothing derived from them may reach a PTY.

On X11 a maximized or fullscreen record opens *windowed* at its restore rect — [[crates/scribe-client/src/main.rs#x11_creation_bounds]] strips the state so GPUI emits no toggle — and the real state is asserted after the map. Publishing during that transient resized every PTY to the pre-maximize rect (a 3×3-section window published 39-column, 5–10-row grids) and requested a full replay of each, then did both again when the state landed: multiple seconds of visible refresh-and-resize churn per start. [[crates/scribe-client/src/main.rs#RestoreRuntime#size_restore_in_flight]] — true while `pending_state` or `state_target` hold work, both bounded by the assert landing or [[crates/scribe-client/src/main.rs#STATE_ASSERT_ATTEMPTS]] — now gates three paths: [[crates/scribe-client/src/main.rs#TerminalView#publish_pane_sizes]] returns without announcing (the render loop retries every frame, so the first settled frame publishes once), [[crates/scribe-client/src/main.rs#TerminalView#stream_session]] attaches with a zero `TerminalSize`, and the reader's [[crates/scribe-client/src/main.rs#attach_session]] does the same for any session that has no display grid yet. A zero size is the Codex reattach contract generalized: `attach_flow::send_attach_replay` skips its pre-snapshot resize unless `has_grid()`, so the replay arrives at the session's current — last known, correct — dimensions, and the deferred-grid entry makes the first settled publish announce the real grid. For a typical restart the measured grid equals the one the server already has, so that publish is a server-side no-op with no `RequestSnapshot`: startup costs one replay per pane, tmux's resize-only-on-change rule applied to the whole pipeline.
...
Priority order: PermissionPrompt > WaitingForInput > IdlePrompt > Error > WaitingForBackground > Processing. Each state has configurable color, pulse frequency, tab indicator, and pane border settings. Error state decays over a timeout. Attention states (IdlePrompt, WaitingForInput, PermissionPrompt) clear on keystroke. Both `IdlePrompt` and `WaitingForInput` share the same `waiting_for_input` indicator config (color, pulse, timeout). `WaitingForBackground` has its own `waiting_for_background` entry, a steady orange by default; it is not an attention state, so a keystroke never clears it, and a configured pulse follows the bounded attention envelope.
...
`WaitingForBackground` is not an attention state, so a run that parks on background work never notifies; its wake-up turn is `Processing` again, and that turn's settle does.
...
The status band reads `appearance.status_bar_height` (default 36px) once at startup and on every chrome-geometry reload, so its border-box height, startup reservation, and flex-grown grid cutover use one live metric. The settings gear renders at the band's far right and opens the settings window on click — the pointer-based settings entry point, moved out of the titlebar. Beside it, a balance button (`⊞`) renders whenever the window holds two or more panes and resets every workspace-region and pane split to equal space via [[crates/scribe-client/src/main.rs#TerminalView#equalize_layout]]. Update availability and progress also render here, centered in the empty span between the left and right segments — see  — so the CTA stays visible on narrow windows and steps down to a shorter `↑ Update` label, then disappears entirely, only when the empty span cannot hold it. The update CTA, balance button, and gear use full-band hit targets clipped to the configured bar. Clicking the update segment opens the in-app confirmation dialog. On a narrow window the right side yields before the controls do: the sparklines shorten to a four-bar floor, then sections drop from the right (see [[client#Client#GPUI Client Spike#GPUI Status Bar Port]]), and the balance button and gear stay in the corner.
...
The  refreshes every 2 seconds via sysinfo. CPU and network history are kept in rolling buffers (16 and 8 entries) for sparkline rendering. GPU detection on Linux reads AMD sysfs or NVIDIA sysfs/nvidia-smi.

Those caps are the graphs' full length: `status_bar`'s `CPU_SPARK_WIDTH` and `NET_SPARK_WIDTH` paint one bar per entry and must match them, so the 2 s refresh puts 32 s of CPU and GPU history and 16 s of each network direction on the band. A narrow window paints only the newest bars ([[crates/scribe-client/src/status_bar.rs#fit_right]]).
...
[[crates/scribe-client/src/terminal_element.rs#TerminalElement#with_annotation]] accepts an optional [[crates/scribe-client/src/terminal_element.rs#AnnotationPaint]] without redoing placement or colour math. A visible paint pass overlays opaque band cells, synthetic-italic message glyphs beside an upright ANSI-red mark, and dimmed box joinery outside the band; it never mutates the `Content` snapshot, so dismissing it repaints the original cells. It also draws a fixed 2px ANSI-red rule for every wrapped failed-link segment, through the same [[crates/scribe-client/src/terminal_element.rs#paint_segment_rule]] raster the hover rule uses — the two share that quad loop and nothing else, so their state, triggering, and dismissal stay separate and each keeps its own colour and thickness. `None` leaves the pass out of idle frames; [[crates/scribe-client/src/main.rs#TerminalView#sync_link_feedback_for_panes]] supplies one paint value per pane only while its annotation is visible.

[[crates/scribe-client/src/main.rs#TerminalView#press_opens_link]] captures the clicked segment, all wrapped run segments, and a [[crates/scribe-client/src/terminal.rs#ContentAnchor]] before calling the observed URL, path, or OSC 8 opener. [[crates/scribe-client/src/main.rs#PendingOpen]] is keyed by pane and carries `{generation, anchor, target, cancel}`; replacing that pane's entry cancels the older bounded worker without disturbing another pane. [[crates/scribe-client/src/main.rs#PendingOsc8Open]] carries either the observed click anchor or a fire-and-forget origin through the disallowed-scheme dialog, so **Open Anyway** preserves Ctrl+click feedback while context-menu activation keeps its old behavior.

[[crates/scribe-client/src/main.rs#TerminalView#finish_link_open]] accepts only the current generation, revalidates the anchor revision, classifies the typed outcome, and traces instead of painting when the pane, session, revision, or annotation geometry is stale. [[crates/scribe-client/src/main.rs#TerminalView#sync_link_feedback_for_panes]] revalidates visible annotations on every pane frame, so any non-empty PTY output to the clicked session (which bumps the content revision) removes only that pane's annotation; a focus or tab change neither clears the state nor paints it over a different session. Its returned [[crates/scribe-client/src/terminal_element.rs#AnnotationPaint]] values feed `with_annotation`, preserving the underlying `Content` cells.

[[crates/scribe-client/src/main.rs#TerminalView#attach_root_interactions]] observes key-down and mouse-down in GPUI's capture phase and wheel events in the ordinary bubble phase, calling [[crates/scribe-client/src/main.rs#TerminalView#dismiss_link_annotations]] without stopping propagation. The triggering input therefore continues to its modal, control, or PTY target. [[crates/scribe-client/src/main.rs#TerminalView#render_status_bar]] appends every currently painted annotation message to the existing `Role::Status` accessibility label; no focus-change handler participates in dismissal.
...
[[crates/scribe-client/src/main.rs#TerminalView#route_osc8_activation_from]] shares [[crates/scribe-client/src/url_detect.rs#is_allowed_osc8_target]] between Ctrl+click and `ContextMenuAction::OpenOsc8Url` activation. Absolute filesystem targets such as `/tmp/preview.png` bypass scheme confirmation because they have no URI scheme. They reach the OS handler verbatim, including spaces and colons in filenames; observed opens retain absolute-path metadata for failure feedback. This does not broaden the URL prefix allowlist or automatically allow other scheme-less targets. Heuristic `ContextMenuAction::OpenUrl` actions retain their existing prefix guard.

Plain clicks are forwarded to the mouse-reporting application rather than handled by Scribe's Ctrl+click router. For manual verification after rebuilding the client, print an OSC 8 link with an absolute image target using `printf '\033]8;;/tmp/preview.png\033\\Mobile preview\033]8;;\033\\\n'`. Ctrl+click and right-click "Open URL" must open the image without a scheme dialog; a `custom://host/preview` target must still prompt. No scan or render work is added; the new path check runs only during activation.
...
Claiming is non-destructive ([[crates/scribe-client/src/restore_state.rs#RestoreStore#claim_first_window]]): the claimed id moves to the index's `claimed` list beside a timestamp — not claimable again while the claim is younger than the 15-minute [[crates/scribe-client/src/restore_state.rs#CLAIM_TTL_MS|CLAIM_TTL_MS]], which rate-limits a crash loop to one replay of the same snapshot per TTL while letting a claimant that crashed mid-replay return the layout to service — while the file itself survives as the window's last good layout. It disappears only when a replacement is durably on disk: a fresh snapshot save clears the claim through [[crates/scribe-client/src/restore_state.rs#RestoreStore#upsert_index]], and the claiming window retires the old file after its first successful post-claim flush (or on an explicit quit or close). Non-replayable and unreadable entries are still pruned at claim time. A reconnect that finds the server kept its sessions therefore no longer destroys the last good snapshot — the failure mode that lost layouts in the v0.0.23→v0.1.0 update, where one reconnect both flattened the window and deleted the only snapshot of the real layout.
