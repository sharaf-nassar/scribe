...
[[crates/scribe-client/src/terminal_element.rs#TerminalElement#with_annotation]] accepts an optional [[crates/scribe-client/src/terminal_element.rs#AnnotationPaint]] without redoing placement or colour math. A visible paint pass overlays opaque band cells, synthetic-italic message glyphs beside an upright ANSI-red mark, and dimmed box joinery outside the band; it never mutates the `Content` snapshot, so dismissing it repaints the original cells. It also draws a fixed 2px ANSI-red rule for every wrapped failed-link segment. `None` leaves the pass out of idle frames; [[crates/scribe-client/src/main.rs#TerminalView#sync_link_feedback_for_panes]] supplies one paint value per pane only while its annotation is visible.
