...
While the overlay is up it owns the keyboard:  consumes every keystroke (Escape closes, Enter / Shift+Enter and the arrows cycle, Backspace and Delete edit, printable characters extend), so nothing leaks to the PTY and the find chord cannot reopen the overlay underneath itself. Cycling the current match notifies only the overlay entity, not the shell view that actually paints the grid's highlights: GPUI's `mark_view_dirty` walks the ancestor chain on every notify, so the overlay's own notify already dirties the shell too (scribe-2yw1 deleted the redundant shell-level `cx.notify()` `handle_find_overlay_key` used to make after every keystroke, once the pointer path — which never made that second call — proved the single notify was enough).
