...
[[crates/scribe-client/src/window_chrome.rs#tab_bar_height]] is the one effective tab-row formula: `appearance.tab_height + appearance.tab_bar_padding`. [[crates/scribe-client/src/window_chrome.rs#chrome_height]] adds the fixed status bar (GPUI lays divs out border-box, so each hairline is inside its band's number), and [[crates/scribe-client/src/window_chrome.rs#default_window_size]] adds the grid's own extent at the metrics it is *painted* with — the live `GridFont`, not the integer cell size reported to the server, because the painted metrics decide where the last row lands.  resolves both font and tab geometry from the live `[appearance]` config and hands the result to `Bounds::centered`. At the shipped defaults the 28 + 8 tab row makes the window 1008x741 rather than 960x680.
