...
Those caps are the graphs' length: `status_bar`'s `CPU_SPARK_WIDTH` and `NET_SPARK_WIDTH` paint one bar per entry and must match them, so the 2 s refresh puts 32 s of CPU and GPU history and 16 s of each network direction on the band.
