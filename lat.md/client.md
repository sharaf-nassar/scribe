...
Tab actions: new, Claude Code new/resume, Codex new/resume, Auggie new/resume, close, next, prev, select 1-9. The legacy `new_claude_*` action names remain in config and code and map to Claude Code, while `new_codex_*` and `new_auggie_*` open their providers. Those AI-tab shortcuts start the selected CLI through the user's login shell with `-lic` and `exec`, resolving the shell from `SHELL` first and then the account database so Finder-launched macOS apps still inherit the expected PATH and rc files without first rendering a normal shell prompt. Also: pane splits, pane focus/cycling, workspace splits/cycling, copy, paste, settings, find, zoom, and equalize.

### Command Palette

The command palette is a GPU-rendered action picker for common window actions, profile switching, and explicit Claude Code, Codex, and Auggie tab actions, opened from a dedicated keybinding and reusing the normal layout-action handlers.
...
During hot restart reattach, `SessionList` does not carry prompt fields. The cold-restart snapshot's [[crates/scribe-client/src/restore_state.rs#LaunchRecord]] persists `first_prompt`, `latest_prompt`, `prompt_count`, and `latest_prompt_at` (Unix-epoch seconds via [[crates/scribe-client/src/restore_replay.rs#system_time_to_unix_seconds]]) so the elapsed-time counter keeps counting up from the original prompt time after a restart. [[crates/scribe-client/src/main.rs#App#apply_snapshot_prompt_state]] reads the saved snapshot, converts the epoch seconds back to `SystemTime`, copies prompt state (including the timestamp) to matching panes by `conversation_id`, then triggers a layout resize if the bar becomes visible.
...
The bottom portion height starts from the cursor-position heuristic in [[crates/scribe-client/src/split_scroll.rs#compute_pin_rows]]: `(screen_lines - cursor_line + 3)` rows clamped to `[3, screen_lines/2]`. When an active prompt is present, [[crates/scribe-client/src/split_scroll.rs#compute_active_prompt_pin_rows]] instead expands the pinned region to span from the latest `PromptStart` mark (falling back to `input_start`) through the live bottom, still leaving at least three rows for the top portion. This keeps the full AI prompt chrome and multiline input block intact while scrolled up, instead of guessing with a fixed extra-row margin alone.

`compute_active_prompt_pin_rows` returns `None` when the stored `prompt_top_abs` is past the current live bottom, which signals a stale mark left over from a scrollback shrink and lets the caller fall back to the cursor heuristic instead of collapsing the pin to three rows. To keep the marks themselves valid, `handle_trim_scrollback_event` calls [[crates/scribe-client/src/pane.rs#shift_absolute_marks_after_trim]] with the dropped-row count after each [[crates/scribe-common/src/protocol.rs#ServerMessage]]`::TrimScrollback`, so each Codex/Claude redraw cycle does not progressively shrink the pinned region as duplicate redraw frames are trimmed away.
