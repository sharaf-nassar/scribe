...
Nothing local moves on release. [[crates/scribe-client/src/main.rs#TerminalView#begin_workspace_move]] sends ids and one operation; a refusal therefore leaves both windows byte-identical. A commit is adopted from the authoritative post-move `SessionList`: [[crates/scribe-client/src/main.rs#park_server_topology]] parks a later list only when it names a session this window has never held — the destination side of a move — so the source keeps collapsing through the pane-retire path a tear-out already used, and [[crates/scribe-client/src/main.rs#TerminalView#adopt_server_topology]] still owns the authorship decision. When the move empties its source window the server closes that window itself and says so after the acknowledged commit; [[crates/scribe-client/src/window_lifecycle.rs#WindowLifecycle#on_window_closed]] obeys a frame naming this window even with no close pending, which is what stops an emptied shell outliving its sessions without opening a replacement window or a login shell.
