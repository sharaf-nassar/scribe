...
Tabs wear a browser-style raised card: an active, hovered, or flashing tab's fill is an absolute child inset `TAB_CARD_TOP_GAP` (5px) from the strip's top edge ([[crates/scribe-client/src/titlebar.rs#TitlebarView#tab_card]]), rounded at its top corners and running to the band's bottom edge, while resting tabs paint no fill and sit flat on the strip. The strip's bottom hairline is itself an absolute child rather than a container border, so the card painted after it covers the line under the active tab — and because the active card wears `tab_bar_active_bg` (the terminal background), the active tab reads as opening into the content below it, the way a browser tab meets its toolbar. The strip ground is lifted a fixed `STRIP_LIFT` over the theme's `tab_bar_bg` token in [[crates/scribe-client/src/tab_bar.rs#TabBarColors]] — menus, dialogs, and the beads board reuse that token as a quiet ground and keep it — so the dark card clearly reads against the band, and the card's own contrast is the active mark: there is no accent tick on the active tab.
...
### Strip ground lifts over the theme token

Verifies `TabBarColors` lifts the strip background and its gradient/hover tone a fixed `STRIP_LIFT` above the theme's `tab_bar_bg`/`tab_bar_gradient_top` tokens while the active card keeps the raw `tab_bar_active_bg` fill.

That lift is the contrast that makes the raised card read against the band without lightening the other chrome surfaces (menus, dialogs, beads board) that reuse the theme token as a quiet ground.
