...
They pin every corner's normalized nearest-edge result with horizontal tie precedence, center resolution, 4px zone hysteresis, the verified 8px arm / strictly-over-24px disarm rule including out-of-bounds delivery, preview clearing while tear-armed, target restoration after re-entry, capability-disabled arming, actionable commit selection, Escape/blur/disappearance cancellation, self/divider/tear/non-target no-ops, and a clean next arm. Motion tests additionally pin both requested durations under `MAX_TRANSITION`, zero duration from both `appearance.animations = false` and a truthy `SCRIBE_DISABLE_ANIMATIONS`, identical animated/static end frames, no transient layers on the zero-duration path, bounded ghost placement, and a fully inert probe while its env gate is off.
