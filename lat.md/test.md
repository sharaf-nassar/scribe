...
No `SCRIBE_RESTORE_ENV_DELTA_FILE` may survive into that environment either, since the launch consumes the staged delta on its way through. The harness-local suspend-capable `pi` stand-in is what makes any of that observable — it dumps its argv and environment, then stays alive so the tab has a live process in it. The CWD assertion is the reason the phase types a `cd` first and reports it by hand: this image ships no shell-integration scripts, so nothing else emits an OSC 7 and the pane's directory has to be something other than the server's `$HOME` fallback or the two are indistinguishable. The tab count is asserted as *exactly one* rather than "at least one", because a chord reaching two handlers is the failure this row exists to catch.

#### Launch-only Ctrl+Z routing and lifecycle

Production routing tests cover protected press, 128 held repeats and paired release across focus/modifier changes in legacy and Kitty event modes. Native shells retain exact 0x1a; configured actions and extra modifiers retain precedence.

`handle_root_key_down_ctrl_z_press_repeat_and_release` in [[crates/scribe-client/src/main.rs]] invokes the live root router and release handler against an outbound IPC sink. `handle_root_key_down_ctrl_z_preserves_native_boundaries_and_owners` in [[crates/scribe-client/src/main.rs]] covers unknown/manual origin, modified chords, configured action and overlay ownership. The headless host supplies detached configuration and no native X11 handle, but runs the production handlers and registered root listeners.

`dispatch_server_message_ai_launch_origin_reconciles_every_inventory_and_exit` in [[crates/scribe-client/src/main.rs]] covers creation fallback, attach echo isolation, every inventory refresh, observed Pi downgrade and true session removal. `pending_create_origin_tracks_replay_values_and_rolls_back` in [[crates/scribe-client/src/ipc_bridge.rs]] checks fresh/resume and legacy Pi outgoing intent plus enqueue rollback. Server launch, attach, live handoff and map/legacy-decoder regressions retain the same independent origin. Current handoff transport is named-map-only; positional decoder fixtures do not expand accepted versions. The origin reconciliation, rejected-create and lost-acknowledgement reconnect tests reuse the common typed-launch fixture `suspend_test_launch`.

`dispatch_server_message_rejected_ai_then_plain_ctrl_z_stays_native` and `dispatch_server_message_rejected_plain_then_ai_origin_stays_unknown` in [[crates/scribe-client/src/main.rs]] dispatch server rejection and successive old-shape create replies before invoking the real keyboard router. They require exact native 0x1a and no true or false origin from the skewed FIFO. `serve_connection_lost_create_ack_reconnect_keeps_only_proven_origin` uses framed in-memory streams through the production reader and writer: a fully written create loses its acknowledgement, queued creation still reaches the next connection, unknown origin stays native, and retained session origin plus server true/false inventory remain authoritative.

`just e2e-visual visual/tab-window-chords.sh` now uses a raw-input Pi stand-in that records receipt of Ctrl+Z and suspends its own foreground job group if it receives it. Press/hold/release must leave the exact launched session responsive to subsequent input, then ordinary provider exit must finalize it. Existing wrapper marker, argv and CWD checks remain. Separate real Pi/Claude keyboard-only checks require disposable isolation and record versions; the stand-in is not a substitute for that acceptance.
...
`ctrl+Prior` carries the region hops because the shipped `workspace_focus_left` / `workspace_focus_right` defaults are `ctrl+alt+left` / `ctrl+alt+right`, which openbox — the window manager the visual container has to run — grabs for its own desktop switching, so they never reach the client here (see [[lat.md/test#Test Harness#Visual E2E Tests#Pane and workspace layout is live]]).
...
The inbound half is injected rather than provoked, because that is what makes it an assertion about the reader rather than about the split: a `WorkspaceInfo` carrying a display name must repaint the status bar's left group, and the same frame with `name: null` must repaint it back to the original pixels. The injected accent is the server's own, echoed out of the recorded frame, so the name is the only thing that moves between captures. ImageMagick captures the focused Scribe window directly before cropping its left status-bar quarter, so another restored window cannot redirect the oracle and the bar's own 2 s sparkline resample cannot swamp the comparison.

### Workspace split bars and strip integrity

`tests/e2e/visual/workspace-split.sh` drives both split directions and verifies region tab bars plus session-exit focus integrity.

The bar oracle is the client's own `lower-region tab bars changed` log line — emitted by the strip partition whenever the bar set's shape changes, because the in-region bars paint no other externally observable trace. After `ctrl+alt+-` the latest state must name one bar with one tab, a `ctrl+shift+t` in that region must grow it to two, a delayed exit after focus moves to the right region must shrink it back to one, and exiting the attached last tab must report `none` alongside the server-side `CloseWorkspace`. Assertions read the *latest* state line, since earlier states stay in the log.

The integrity oracle counts `moved a session into another workspace region` lines: exactly one per split (each split's seed is created through the previous region's workspace and re-filed once its pane adopts it) and never one more — a growing count is the exit-refocus path dragging a surviving session across a workspace boundary. Before the delayed background exit the right pane hides its shell cursor and captures a stable frame; its terminal-grid body crop must have zero differing pixels afterwards, while `pane adopted a session` grows at most once for the background region's sibling and then stops — a tab owns its own pane tree, so the region usually swaps onto the sibling's already-populated tree and adopts nothing, and what the assertion is really guarding is the re-adoption loop. That crop takes the divider position as a percentage rather than assuming half the window: stacking a second region into the left column re-equalizes the tree by leaf count, so the divider sits at two thirds and a half-window crop reaches across it into the left column's own tab bar, which legitimately repaints when that region loses a tab. The attached last-tab collapse must add no adoption line at all, and a long command after collapse must change the surviving left pane by at least the configured routing threshold. Clicks warp the pointer window-relative but press through XTEST, because GPUI reads pointer input via XInput2 and never sees the synthetic `XSendEvent` buttons `xdotool click --window` produces — typing works over synthetic events, clicks silently vanish.
...
Layout-restore only ever moves the window, and a reparenting window manager answers a move with the synthetic, root-relative `ConfigureNotify` ICCCM requires — the one case that cannot expose a parent-relative reading. A resize instead earns a real `ConfigureNotify` whose `x`/`y` are relative to the WM frame, and GPUI's X11 backend stores that value verbatim without checking whether the event was synthetic (its `set_bounds` ignores the origin when the size changed, but a same-size event with a bogus origin is adopted whole). The hypothesis that this happens under the openbox test WM was disproven — but it was then confirmed in production under Mutter, whose event stream left records holding the literal frame offset `(14,49)`, and the next start landed those windows at the primary monitor's top-left with their titlebar under the GNOME panel. Capture no longer persists the cached origin at all: [[crates/scribe-client/src/main.rs#TerminalView#resolve_root_origin|resolve_root_origin]] reads the root-relative [[crates/scribe-client/src/monitor.rs#logical_window_origin|logical_window_origin]] instead, so the probe's invariant — the offset must not move on a resize — now guards that reading. Under openbox the offset settles at the constant frame delta, because `xdotool` reports the frame origin while the record holds the client origin.
...
Verifies one region's wire payload names every tab of that region, in strip order, with each tab's own split at that tab's index — the report the server persists as the window's layout.

Reporting only the visible pane is what lost tab order and the active tab across a restart, so the assertions are about what is *left out*: another region's tabs are excluded, a lone pane carries no pane tree of its own (the reader rebuilds it from the tab's session id), a split round-trips back through `wire_tab_pane_tree` to the same shape, and a split's extra pane session stays inside `pane_trees` — `wire_tree_pane_sessions` finds it there, so a reconnect re-files it as a pane instead of promoting it into the strip.

### A pane split never adds a strip tab

The regression oracle for the model this shell shape exists to restore: splitting a one-tab region must not move that region's tab count.

The split runs the way the running client runs it — `split_focused_pane` queues a pane, the reader files the answering session with `TabSessions::insert_pane`, and the pending pane claims it. The assertion is made on the wire payload, not on pixels: the reported leaf keeps its single `session_ids` entry with `active_tab_index` still 0, while `pane_trees[0]` becomes a `Split`. Both panes render, and `close_tab_sessions` returns both sessions, so closing that one tab closes everything the split created.

### Tab switch swaps the whole per-tab tree

Selecting a tab must swap the region's entire layout rather than adopt the incoming session into the outgoing tab's focused pane.

A split tab and a plain tab share one region. `TabSessions::select` at the split pane's would-be position answers `None` — three sessions are live but only two are tab positions, which is the digit/next/prev half of the invariant — and selecting the split tab brings back *both* of its panes at once, while selecting the plain tab leaves exactly one rendered and the split's panes dormant.

### A tab re-anchors onto its surviving split

A tab is keyed by the session it was opened with, so that session exiting must not strand the splits it made.

After a split, only the split's session stays live. `retain_sessions` closes the dead pane, re-keys the tab onto the survivor, and reports it in `promoted_tabs`; `TabSessions::promote_pane` then gives it the strip entry. The region still shows exactly one tab, one pane, and reports that survivor as the tab the server persists — the same collapse rule `prune_workspace_node` applies to a stored tree.
...
#### A server-closed source window exits without asking

Verifies the other frame [[crates/scribe-client/src/window_lifecycle.rs#WindowLifecycle#on_window_closed]] must obey: an unsolicited `WindowClosed` naming *this* window.

The server closes an emptied source window itself when the last workspace it held moves into another window, and it sends that frame after the acknowledged commit. There is no pending close to match, so the window id is the only evidence, and refusing it would leave an emptied shell running with no sessions. Both the exit and the closed latch are asserted, since the IPC backend reads the latch after the GPUI thread has drained the exit. Before `Welcome` names this window there is nothing to match, so an early frame still closes nothing.
...
They pin every corner's normalized nearest-edge result with horizontal tie precedence, center resolution, 4px zone hysteresis, the verified 8px arm / strictly-over-24px disarm rule including out-of-bounds delivery, preview clearing while tear-armed, target restoration after re-entry, capability-disabled arming, actionable commit selection with immediate idle reset, the shared cancel reset used by Escape/blur/disappearance, self/divider/tear/non-target no-ops, and a clean next arm. Motion tests additionally pin both requested durations under `MAX_TRANSITION`, zero duration from both `appearance.animations = false` and a truthy `SCRIBE_DISABLE_ANIMATIONS`, identical animated/static end frames, no transient layers on the zero-duration path, bounded ghost placement, and a fully inert probe while its env gate is off.

`tests/e2e/visual/drag-latency-probe.sh` is the disposable X11/Xvfb + Lavapipe measurement path. It relaunches only the isolated E2E client with `SCRIBE_DRAG_PROBE=1` and a `RUST_LOG` that names `scribe::drag_probe=info` explicitly — the entrypoint's default `scribe_server=info,scribe_client=info` filter drops the probe target entirely — creates two regions, drives a pill sweep, extracts `input_to_paint_us` from the client log, and prints nearest-rank min/p50/p90/p95/p99/max against the 16.7ms budget. It is deliberately outside suite inventory because timing from software Vulkan is evidence for the named run, not a portable CI assertion.

[[crates/scribe-client/src/workspace_transfer.rs]] tests cover sole-workspace and absent-capability no-send decisions, every typed refusal including `EnvironmentRebindFailed`, same-id timeout/disconnect retry gated by one `can_retry` predicate, first-send and retry-send failure recovery, and one-open idempotency while late or duplicate results share the ignored path. [[crates/scribe-client/src/workspace_transfer.rs#WorkspaceMoves]] adds the existing-window half: an absent capability and a stale target each refuse before anything reaches the wire, a second request cannot claim the in-flight slot, every typed `WorkspaceMoveRefusal` settles with its own message, a commit is deliberately silent because the post-move session list is what both windows adopt, and late or foreign results are ignored. Its recovery split is asymmetric on purpose — a request that never reached the writer reports a plain send failure, an admitted one reports an interrupted move and defers to the server's ledger. [[crates/scribe-client/src/workspace_drag.rs#swap_hint]] pins wording to centre targets only, with blank names falling back to the same generic sentence, and [[crates/scribe-client/src/workspace_drag.rs#move_operation_for]] pins each zone to its own insert edge or the swap. [[crates/scribe-client/src/command_palette.rs#workspace_move_entries]] pins the two deterministic rows per eligible window, the byte-identical swap explanation on the swap row alone, and the empty list a single-window desktop produces. [[crates/scribe-client/src/window_lifecycle.rs#WindowLifecycle#on_window_closed]] pins the server-initiated close: a frame naming this window exits it with no close pending, while an unnamed window and another window's id both stay ignored. Binary tests pin the claimed target's `initial_session: false`, `open_restored_window` status return, inherited target size, cursor anchoring, compositor placement, and open-failure recovery copy. [[crates/scribe-client/src/ipc_bridge.rs#IpcSink#transfer_workspace]] verifies all three correlated ids reach the ordered writer queue unchanged.

[[crates/scribe-client/src/workspace_layout.rs#WindowLayout#rearrange_workspace]] tests pin client-side lowering over the shared tree operations: edge extraction/insertion re-equalizes all region ratios; center swap preserves nested ratios and shape. Interactive split re-equalizes the same way; interactive close instead promotes the sibling and preserves unaffected ratios. Slot metadata, active tab, pane-owned payload, and source focus stay attached. Directional palette-move tests pin non-wrapping nearest-neighbor selection, no-neighbor feedback/no-op, focused-source movement, and equality with the matching drag edge. [[crates/scribe-client/src/workspace_tree.rs#WorkspaceTree#rearrange_workspace]] adds the entity boundary assertion that one successful rearrangement emits one report; [[crates/scribe-client/src/workspace_tree.rs#WorkspaceTree#move_focused_workspace_in_direction]] covers the equivalent palette path. [[crates/scribe-client/src/command_palette.rs#base_entries]] pins all five workspace rows as visible client-local actions. Existing titlebar drag-reorder suites remain the regression oracle for tab marker isolation and window-move behavior.
...
leaf-count contract, waits for a serialized-message substring with its requested
bounded timeout, and reads fields from the first matching post-reset frame;
